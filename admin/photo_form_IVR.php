		  ...
		  //Unlink the previuos image
		   ...
		   //unlink($filename);
			...
			if(@move_uploaded_file($_FILES['photo']['tmp_name'], $target_path))
			{
				...
				$photo_url= "https://registration.gjepc.org/images/ivr_image/photograph/".$photograph_fid;

				$sql="update ivr_registration_details set photograph_fid='$photograph_fid' where eid='$id'";
				$result=$conn ->query($sql);
				if($result){	
					$updateGlobal = "UPDATE globalExhibition SET photo_url='$photo_url',isDataPosted='N',isDataPosted_Offline='N',`face_status`='P' WHERE `registration_id`='$registration_id' AND `visitor_id`='$id' AND `participant_Type`='INTL'";
