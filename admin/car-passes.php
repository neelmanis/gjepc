...
if($_REQUEST['Reset']=="Reset")
{
	...
	$_SESSION['vehicle_no']="";  
  header("Location: car-passes.php?action=view");
} else { 
...
<?php 
if(($_REQUEST['action']=='reset') && ($_REQUEST['unique_code']!=''))
{
	$unique_code = filter(intval($_REQUEST['unique_code']));
	$checkData = "SELECT isDataPosted FROM gjepclivedatabase.globalparking WHERE 1 and id='$unique_code'" ;
	$resultData = $conn->query($checkData);
	$rows = $resultData->fetch_assoc();
	$isDataPosted = $rows['isDataPosted'];

    $sql = "update gjepclivedatabase.globalparking set isDataPosted='N',isSubmitted='0' where unique_code='$unique_code'";
...
<div id="main">
    <div class="content">
        ...
        <div class="content_head"><a href="car-passes.php?action=view">
            <div class="content_head_button">Car Passes List</div></a>
            <!-- <a href="car-passes.php?action=add"><div class="content_head_button">Add Pass</div></a> -->
            <!--  -->
        </div>
                        ...
                        <tr>
                            ...
                            <td>
                                <select name="category_search" id="category_search" class="input_txt">
                                    ...
                                    <?php if ($_SESSION['curruser_login_id'] != 221) { ?>
                                    ...
                                    <option value="S" <?php echo isset($_SESSION['category_search']) && $_SESSION['category_search'] == "SV" ? 'selected' : ''; ?>>Service</option>
                                </select>
                            </td>   
                        </tr>
                        <tr>
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0" class="detail_txt" >
                    <tr class="orange1">
                        ...
                        <?php
                            ...
                            if ($_SESSION['curruser_login_id'] == 221) {
                                $sql = "select * from globalparking as a where agency_category != 'E'";
                            } else {
                                $sql = "select * from globalparking as a where 1";
                            }
                            ...
                            if ($rCount > 0) {
                                ...
                                <tr>
                                    ...
                                    <td><?php echo filter($rows['registration_id']); ?></td>
                                    ...
                                    <td>
                                        <a  href="car-passes.php?action=update&uniquecode=<?php echo $rows['unique_code'] ?>" title="UPDATE"><img src="https://gjepc.org/admin/images/edit.gif" border="0"> 
                                        </a>&nbsp;
                                        <a href="car-passes.php?action=reset&unique_code=<?php echo $rows['unique_code'];?>"><img src="images/reset.png" title="Reset" border="0" width="16" height="16" /></a>  
                                    </td>
                                    <?php if ($rows['status'] != "") { //$rows['status'] = "Y"?>
                                        <td><a href="https://registration.gjepc.org/car-parking-qr-code.php?action=generatePass&unique_code=<?php echo $rows['unique_code']; ?>" target="_blank"> Download</a></td>
                                    <?php } ?>
                                </tr>
            ...
            <div class="pages_1">Total Number of Application: <?php echo $rCount; ?>
                <?php echo pagination($limit, $page, 'car-passes.php?action=view&page=', $rCount); //call function to show pagination?>
            </div>        
        <?php } ?> 
                        ...
                        <!-- <tr>
                            ...
                            <td>
                                <select name="category" id="category">
                                    ...
                                    <option value="S" <?php echo isset($update_category) && $update_category == "SV" ? 'selected' : ''; ?>>Service</option>
                                </select>
                            </td>
                        </tr>  -->
                        <tr>
...
<script language="javascript">
    //$(document).ready(function(e){
        ...
        $(".gate").change(function(e){
            ...
            $.ajax({
                url: "car-passes-ajax.php?action=getGround",
                method:"POST",
                dataType:"json",
    ...
    //     }   
    //     $.ajax({
    //         url: "car-passes-ajax.php?action=updateStatus",
    //         method:"POST",
    //         dataType:"json",
