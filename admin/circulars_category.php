...
{
	$sql="delete from circulars_category where id=?";
	$stmt = $conn -> prepare($sql);
	$stmt->bind_param("i", intval($_REQUEST['id']));
	if($stmt->execute()){
	echo"<meta http-equiv=refresh content=\"0;url=circulars_category.php?action=view\">";
	}
}
...
if(($_REQUEST['action']=='active') && ($_REQUEST['id']!=''))
{
	...
	$sql="update circulars_category set status='$status' where id='$id'";
	$stmt = $conn -> prepare($sql);
	if($stmt->execute()){
		echo"<meta http-equiv=refresh content=\"0;url=circulars_category.php?action=view\">";
	}
...
if($_REQUEST['action']=='save')
{
	...
	$sql="INSERT INTO circulars_category (cat_name,order_no,status) VALUES ('$cat_name', '$order_no','1')";	
	$stmt = $conn ->query($sql);
	echo "<meta http-equiv=refresh content=\"0;url=circulars_category.php?action=view\">";
	} else {
		echo "<script langauge=\"javascript\">alert(\"Sorry Invalid Token Error.\");location.href='circulars_category.php?action=add';</script>";
	}
}
...
if(($_REQUEST['action']=='update')&&($_REQUEST['id']!=''))
{
	...
	$sql="update circulars_category set cat_name='$cat_name',order_no='$order_no' where id='$id'";	
	$stmt = $conn ->query($sql);
	echo"<meta http-equiv=refresh content=\"0;url=circulars_category.php?action=view\">";
}
?>
...
<div class="breadcome_wrap">
	<div class="breadcome"><a href="admin.php">Home</a> > Manage Circulars Category</div>
</div>

<div id="main">
	<div class="content">
    	<div class="content_head"><a href="circulars_category.php?action=add"><div class="content_head_button">Add Circulars Category</div></a>  <a href="circulars_category.php?action=view"><div class="content_head_button">Manage Circulars Category</div> </a></div>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="detail_txt" >
    ...
    $i=1;
	$result = $conn ->query("SELECT * FROM circulars_category where 1".$attach." ");
    ...
    ?>  
 	<tr <?php if($i%2==0){echo "bgcolor='#CCCCCC'";}?>>
        ...
        <td ><a href="circulars_category.php?action=edit&id=<?php echo $row['id']?>"><img src="images/edit.gif" alt="Edit" width="15" height="15" border="0" /></a></td>
        <td ><a href="circulars_category.php?action=del&id=<?php echo $row['id']?>" onClick="return(window.confirm('Are you sure you want to delete?'));"><img src="images/delete.png" title="Delete" border="0" /></td>
 	</tr>
	<?php
	$i++;
	   }
...
if(($_REQUEST['action']=='add') || ($_REQUEST['action']=='edit'))
{
  ...
  {
		$action='update';
		$result2 = mysql_query("SELECT *  FROM circulars_category  where id='$_REQUEST[id]'");
...
<table width="100%" border="0" cellspacing="0" cellpadding="0"  >
  	<tr class="orange1">
    <td colspan="2"> &nbsp;Add Circulars Category</td>
    </tr>
