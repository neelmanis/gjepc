...
<body>
    ...
    try {
      ...
      //base64_encode($mpdf->Output($tmp.''.$filename.'.pdf', \Mpdf\Output\Destination::FILE));
...
<div><input type="submit" name="print_pdf" action="print_pdf" value="Download Parichay Card"  class="input_submit" /></div>
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="detail_txt" >
  <tr class="orange1">
  	<td>Select</td>
    <td width="8%">Date</td>
  ...
  <tr>
  	<?php if($rows['company_approval'] == "Y") {?>
  	   <td><input type="radio" name="person_id" value="<?php echo $rows['person_id']; ?>"  /></td>
	<?php } else { ?>
		<td><input type="radio" name="person_id" value="<?php echo $rows['person_id']; ?>"  disabled/></td>	
	<?php } ?>
