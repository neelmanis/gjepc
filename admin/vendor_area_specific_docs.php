...
{   $request_id = $_REQUEST['id'];
	$sql="delete from vendor_area_specific_docs where id=$request_id";
	$stmtd = $conn -> query($sql);
	
	if($stmtd){	echo"<meta http-equiv=refresh content=\"0;url=vendor_area_specific_docs.php?action=view\">"; }
}
...
<div class="breadcome_wrap">
	<div class="breadcome"><a href="admin.php">Home</a> > Manage Vendor Area Spec Doc</div>
</div>
...
<div id="main">
	<div class="content">
        ...
        <div style="float:right; padding-right:10px; font-size:12px;"><a href="vendor_area_specific_docs.php?action=view">Back to Vendor Area Spec Doc</a></div>
        <?php }?>
        </div>
...
if(($_REQUEST['action']=='add') || ($_REQUEST['action']=='edit'))
{
  ...
  if(($_REQUEST['id']!='') || ($_REQUEST['action']=='edit'))
  {
		$action='update';
		$request_id  = $_REQUEST['id'];
		$sql3 = "SELECT * FROM vendor_area_specific_docs where id=$request_id";
      ...
      $area=getVendorAreaName($row2['area'],$conn);
		  ...
		  $documentArray = explode(",",$document);
			$status=$row2['status'];			
		}
  }
?>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0"  >
   ...
   <tr>
      ...
      <td>
        <?php   
		$queryDocument = "SELECT * FROM vendor_documents WHERE status=1 AND type='v'"; 
		$query = $conn -> prepare($queryDocument);
		$query->execute();			
		$result2 = $query->get_result();
		while($document = $result2->fetch_assoc()){ ?>
        <input type="checkbox" name="document[]" id="document" value="<?php echo $document['id'];?>" <?php if(in_array($document['id'],$documentArray)){echo "checked";}?>> <?php echo $document['name'];?>
          <br>
        <?php  } ?>
      </td>
    </tr>
