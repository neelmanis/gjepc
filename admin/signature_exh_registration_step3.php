...
if($action=='UPDATE')
{
	...
	header("Location: signature_exh_registration_step4.php?id=$gid&registration_id=$uid");
	}
}
?>
...
<title>IIJS-Signature &gt; Exhibitor Registration &gt; Step-3</title>
...
$(document).ready(function(){
  ...
  woman_entrepreneurs=$('[name=woman_entrepreneurs]:checked').val();
	...
	$.ajax({ type: 'POST',
					url: 'ajax.php',
					data: "actiontype=calculatePayment&&selected_area="+selected_area+"&&section="+section+"&&selected_premium_type="+selected_premium_type+"&&woman_entrepreneurs="+woman_entrepreneurs+"&&category="+category+"&&country="+country+"&&last_yr_participant="+last_yr_participant,
					dataType:'html',
					beforeSend: function(){
...
<script>
  $("#section").live('change',function(){
	  ...
	  $('#grand_total').val('');
	   ...
	   $.ajax({ type: 'POST',
					url: 'ajax.php',
					data: "actiontype=selectArea&&section="+section,
					dataType:'html',
					beforeSend: function(){
							},
					success: function(data){
...
//echo "select * from exh_registration where uid='$registration_id' and `show`='IIJS Signature 2018' and last_yr_section_flag='1' order by exh_id desc limit 0,1";
	...
	$query1=$conn->query("select * from exh_registration where uid='$registration_id' and `show`='IIJS SIGNATURE 2023' order by exh_id desc limit 0,1");
...
<tr>
    <td><strong>Last year Signature details :</strong></td>
    <td>
...
<?php $query_premiere=$conn->query("select * from exh_registration where uid='$registration_id' AND `show`='IIJS PREMIERE 2022'");
	$result_premiere=$query_premiere->fetch_assoc();
	$num_premiere=$query_premiere->num_rows;
	if($num_premiere>0)
	{
		$option_premiere=$result_premiere['options'];
		$category_premiere=$result_premiere['category'];
		$selected_area_premiere=$result_premiere['selected_area'];
		$section_premiere=$result_premiere['section'];
		$selected_scheme_type_premiere=$result_premiere['selected_scheme_type'];
		$selected_premium_type_premiere=$result_premiere['selected_premium_type'];
	?>
<tr>
    <td><strong>Last year Premiere details :</strong></td>
    <td>
	<strong>Section :</strong> <?php echo strtoupper($section_premiere);?><br />
    <strong>Area :</strong> <?php echo strtoupper($selected_area_premiere);?> <br />
	<strong>Category type :</strong> <?php echo $category_premiere?> <br />
    <strong>Scheme type :</strong> <?php echo $selected_scheme_type_premiere?> <br />
    </td>
</tr>	
   <?php }
	?>
<tr >
    ...
    <td><input type="hidden" name="exh_id" id="exh_id" value="<?php echo $exh_id?>"/>
        ...
        <?php /*} else { */?>
        <?php if($num_premiere>0){ ?>
        <input type="radio" id="option" class="bgcolor" name="option" value="Same stall position as off IIJS premiere 2022" <?php if($option=="Same stall position as off IIJS premiere 2022"){?> checked="checked" <?php } ?> />  Same stall position as off IIJS premiere 2022<br/>
      <?php } ?>
