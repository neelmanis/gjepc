...
if($_REQUEST['company_action']=='save_document')
{
		...
		{
    		echo "<script langauge=\"javascript\">alert(\"Sorry you have select Invalid file.\");location.href='iijs_employee_directory.php?action=view';</script>";
			exit;
		}
		elseif($file_name != '')
		{
			if($_FILES["upload_company_document"]["type"] == "application/pdf" || $_FILES["upload_company_document"]["type"] == "application/PDF")
			{
				...
				$stmt->execute();
				echo"<meta http-equiv=refresh content=\"0;url=iijs_employee_directory.php?action=view\">";
				}
				else
				{
				echo "<script langauge=\"javascript\">alert(\"Sorry you can not upload this  file.\");location.href='iijs_employee_directory.php?actions=companyedit&regid=$registration_id';</script>";
				return;
				}
			} else
			{
			 echo "<script langauge=\"javascript\">alert(\"Sorry you have select Invalid file.\");location.href='iijs_employee_directory.php?actions=companyedit&regid=$registration_id';</script>";
			}		
		}
}
?>
...
if(($_REQUEST['action']=='update')&&($_REQUEST['id']!=''))
{
	...
	$is_mobile_change = filter($_REQUEST['is_mobile_change']);
   ...
   if($approval == "Y"){ $disapprove_reason = ""; }
	...
	if(isset($is_mobile_change) && $is_mobile_change == 'Y')
	{
		$globalCheck = "SELECT mobile FROM globalExhibition WHERE `mobile`='$sendMobile'";
		$checkUniqueResult = $conn->query($globalCheck);
	    $countUnique = $checkUniqueResult->num_rows;
		if($countUnique>0){
		echo "<script> alert('Mobile No Already Exist In Global');</script>";
		echo "<meta http-equiv=refresh content=\"0;url=iijs_employee_directory.php?action=viewReg&regid=$registration_id\">"; exit;
		} else {
		$sqlLogs = "INSERT INTO visitor_approval_logs SET post_date=NOW(), registration_id='$registration_id',visitor_id='$id',admin_id='$adminID',admin_name='$admin_name',type='visitor_mob_change',action='$approval',reason='mobile change'";
		$resultLogs = $conn ->query($sqlLogs);
		
		$updateGlobal = "UPDATE globalExhibition  SET `mobile`='$sendMobile',`isSecondary`='N',`days_allow`='Y' WHERE `registration_id`='$registration_id' AND `visitor_id`='$id' AND participant_Type='VIS'";
		$updateGlobalResult = $conn->query($updateGlobal);
		
		$dirCheck = "SELECT mobile FROM visitor_directory WHERE `mobile`='$sendMobile'";
		$checkDirResult = $conn->query($dirCheck);
	    $countDir = $checkDirResult->num_rows;
		if($countDir>0){
		echo "<script> alert('Mobile No Already Exist In Visitor Directory');</script>";
		echo "<meta http-equiv=refresh content=\"0;url=iijs_employee_directory.php?action=viewReg&regid=$registration_id\">"; exit;
		} else {
		$updateDir = "UPDATE visitor_directory SET `mobile`='$sendMobile',`badge_id`='N',isSecondary='N' WHERE `registration_id`='$registration_id' AND `visitor_id`='$id'";
		$updateDirResult = $conn->query($updateDir);
...
if($face_isApplied =="Y")
{
    ...
    if($checkCount >0){
        if($approval =="Y"){
         ...
         $updateGlobal = "UPDATE globalExhibition SET photo_url='$photo_url',isDataPosted='N',`face_status`='Y' WHERE `registration_id`='$registration_id' AND `visitor_id`='$id' AND (`participant_Type`='VIS' OR `participant_Type`='IGJME' ) ";
        	$resultStatusUpdate = $conn->query($updateGlobal);
        } else {
 			$updateGlobal = "UPDATE globalExhibition SET isDataPosted='N',`face_status`='D' WHERE `registration_id`='$registration_id' AND `visitor_id`='$id' AND (`participant_Type`='VIS' OR `participant_Type`='IGJME' ) ";
        	$resultStatusUpdate = $conn->query($updateGlobal);
        }
		$website = "IIJS PREMIERE 2022";
		$photolink = "https://registration.gjepc.org/visitor_badge.php";
		$messagev = "Dear $name, your photo update request has been approved, kindly download your badge from $photolink for $website Team GJEPC";
...
else if($approval=='D' && $countHistory == 0 ) //Send mail For DisApproved Visitor
{
	$url = "https://registration.gjepc.org/single_visitor.php";
...
/***  Emailer End **/
echo"<meta http-equiv=refresh content=\"0;url=iijs_employee_directory.php?action=viewReg&regid=$registration_id\">";
}
}
?>
...
{	
	...
	if($num>0)
	{
		...
		echo '</script>';		
	echo "<meta http-equiv=refresh content=\"0;url=iijs_employee_directory.php?action=view\">"; exit;
	}
	
	$sqlreg = "UPDATE `registration_master` SET company_pan_no='$company_pan_no', company_gstn='$company_gstn',company_type='$company_type', state='$state',address_line1='$address_line1',address_line2='$address_line2',city='$city',pin_code='$pin_code', `approval_status`='$approval_status', `disapprove`='$disapprove' WHERE id='$registration_id'";
...
if($_REQUEST['Reset']=="Reset")
{
  ...
  header("Location: iijs_employee_directory.php?action=view");
} else
{
...
<div id="main">
	<div class="content">
        ...
        <div class="content_head_button" style="float:right; padding-right:10px; font-size:12px;"><a href="iijs_employee_directory.php?action=view">Back</a></div> 
        ...
        <a href="iijs_employee_directory.php?action=viewReg&regid=<?php echo $registration_id; ?>">Back</a></div>
		...
		<div style="float:right; padding-right:10px; font-size:12px;">
        <a href="iijs_export_combo_report.php">&nbsp;Download old COMBO</a>
        </div>
	   ...
	   <div class="clear">
		...
		<a href="iijs_employee_directory.php?action=old_to_part" onClick="return(window.confirm('Are you sure you want to Clear Old to Part Data'));" >Clear Old to Part Data</a>	
		<?php }	?>
		</div>
...
<td><strong>Visitor Status</strong></td>        
    <td>
		...
		<?php if($adminID!=199 || $adminID!=198 || $adminID!=197) { ?>
...
AND vd.isApplied='Y' AND vd.visitor_approval!='O' AND vo.payment_type='online'"; */
  ...
  <tr>
    ...
    <td align="center" valign="middle"><a href="iijs_employee_directory.php?actions=companyedit&regid=<?php echo $rows['id'];?>">
    ...
    <a href="iijs_employee_directory.php?action=viewReg&regid=<?php echo $rows['id'];?>">Employee Details</a> / <a href="iijs_employee_directory.php?action=orderDetails&regid=<?php echo $rows['id'];?>">Order Details</a>
    </td>
...
<?php echo pagination($limit,$page,'iijs_employee_directory.php?action=view&page=',$rCount); //call function to show pagination?>
</div>  
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="detail_txt" >
  ...
  <tr>
	...
	<td><a style="text-decoration:none; padding: 5px auto;height: auto;" class="content_head_button" href="iijs_employee_directory.php?action=uploadVaccineCertificate&visitor_id=<?php echo $rows['visitor_id'];?>&registration_id=<?php echo $registration_id;?>" > Upload  </a></td>
	<?php }else{?>
			<td></td>
	<?php }  ?>
...
<?php 
  ...
  if(($_REQUEST['regid']!='') || ($_REQUEST['actions']=='companyedit'))
  {
		...
		{			
			...
			$address_line1  = filter($row3['address_line1']);
			$address_line2  = filter($row3['address_line2']);
			$city  = filter($row3['city']);
			$pin_code  = filter($row3['pin_code']);
			$state  = filter(getState($row3['state'],$conn));
		}
		
		$lastModify = $conn ->query("SELECT * FROM gjepclivedatabase.visitor_approval_logs where registration_id='$registration_id' AND type='company_approval' order by post_date desc limit 1");
		if($row4 = $lastModify->fetch_assoc())
		{			
			$admin_name=filter($row4['admin_name']);
			$post_date=	date("d-m-Y",strtotime($row4['post_date']));
		}
	}
?>
<div class="content_details1">
...
<table width="100%" border="0" cellspacing="2" cellpadding="2" class="detail_txt" >
  ...
  <tr>
    ...
    <td><input type="text" name="address_line2" id="address_line2" class="input_txt" value="<?php echo $address_line2; ?>" /></td>
  </tr>
  <tr>
  ...
  <tr >
    <td><?php echo "Last modified by : ".$admin_name." on " . $post_date;?></td>
    <td>
    <input type="submit" value="Submit" class="input_submit"/>
	...
	if (confirm("Are you sure you want to update this address with Delta API")) {
		...
		success:function(data)
		{
			...
			if($.trim(data)==1){
				...
				window.location.href = "iijs_employee_directory.php?action=viewReg&regid="+registration_id;
			}else{
				alert("Sorry There is some problem with SAP response"); 
				window.location.href = "iijs_employee_directory.php?action=viewReg&regid="+registration_id;
	...
	if (confirm("Are you sure you want to update this address with Delta API")) {
		...
		success:function(data)
		{
			...
			if($.trim(data)==1){
				...
				window.location.href = "iijs_employee_directory.php?action=view";
			}else{
				alert("Sorry There is some problem with SAP response"); 
				window.location.href = "iijs_employee_directory.php?action=view";
