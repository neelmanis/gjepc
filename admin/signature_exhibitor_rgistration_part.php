...
if($_REQUEST['Reset']=="Reset")
{
  ...
  header("Location: signature_exhibitor_rgistration_part.php");
} else
{ 
...
{
		$get = "SELECT * FROM exh_reg_payment_details where `show`='IIJS SIGNATURE 2023' AND (sales_order_no='tem categor' || sales_order_no='rror in doc' || sales_order_no='old-to part' || sales_order_no='pecify eith' || sales_order_no='o customer')";
		$result_query = $conn ->query($get);
		$getCount = $result_query->num_rows;
		
		$get2 = "SELECT * FROM `utr_history` where `show` ='IIJS SIGNATURE 2023'  AND (first_sales_order_no='tem categor' || first_sales_order_no='rror in doc' || first_sales_order_no='old-to part' || first_sales_order_no='pecify eith' || first_sales_order_no='o customer')";
	...
	//	$sql="UPDATE `visitor_order_detail` SET `sap_sale_order_create_status` = '0', `sales_order_no` ='' WHERE `sales_order_no` ='old-to part' AND `sap_sale_order_create_status` = '1'";
		$sql="UPDATE `exh_reg_payment_details` SET `sap_sale_order_create_status` = '0', `sales_order_no` = '' WHERE `show`='IIJS SIGNATURE 2023' AND (sales_order_no='tem categor' || sales_order_no='rror in doc' || sales_order_no='old-to part' || sales_order_no='pecify eith' || sales_order_no='o customer') AND `sap_sale_order_create_status` = '1'";
		$result = $conn ->query($sql);  
		
		$sql2="UPDATE `utr_history` SET `sap_so_status` = '0', `first_sales_order_no` =  '' WHERE `show`='IIJS SIGNATURE 2023'  AND (first_sales_order_no='tem categor' || first_sales_order_no='rror in doc' || first_sales_order_no='old-to part' || first_sales_order_no='pecify eith' || first_sales_order_no='o customer') AND `sap_so_status` = '1'";
		$result2 = $conn ->query($sql2); 
		 
        if(!$result) die ($conn->error);
		echo "<meta http-equiv=refresh content=\"0;url=signature_exhibitor_rgistration_part.php\">";
		} else { 
...
<div id="main">
	<div class="content">
    	...
    	<div class="content_head"><a href="signature_exhibitor_rgistration_part.php">
		...
		<a href="export_utr_signature_exhibitor_rgistration_part.php">&nbsp;Download Payment Data</a> <a href="export_approve_signature_exhibitor_rgistration_part.php">&nbsp;Download Data</a></div>
		<?php
		if($_SESSION['curruser_login_id']=='28' || $_SESSION['curruser_login_id']=='1'){ ?>
		<a href="signature_exhibitor_rgistration_part.php?action=old_to_part" onClick="return(window.confirm('Are you sure you want to Clear Old to Part Data'));" >Clear SAP Error</a>	
		<?php }	?>
		</div>
...
where a.event_for='IIJS SIGNATURE 2023'";
}
else
{
...
b.section,b.selected_area,b.selected_premium_type,c.payment_status,c.document_status,c.application_status 
from exh_reg_general_info a inner join exh_registration b on a.id=b.gid inner join exh_reg_payment_details c on a.id=c.gid
where a.event_for='IIJS SIGNATURE 2023' and  a.region='".$_SESSION["curruser_region_id"]."' ";
}
}
...
b.section,b.selected_area,b.selected_premium_type,b.options,c.payment_status,c.sap_sale_order_create_status,c.document_status,c.application_status,c.sales_order_no,u.utr_number,u.show,u.utr_approved from exh_reg_general_info a inner join exh_registration b on a.id=b.gid inner join exh_reg_payment_details c on a.id=c.gid left join utr_history u on a.uid=u.registration_id where a.event_for='IIJS SIGNATURE 2023' and u.show='IIJS SIGNATURE 2023'"; */
...
else
{
	...
	{
	$sql="select a.id,a.uid,a.company_name,a.contact_person,a.region,a.billing_address_id,a.get_billing_bp_number,a.created_date,b.last_yr_participant,
b.section,b.selected_area,b.selected_premium_type,b.options,c.payment_status,c.sap_sale_order_create_status,c.document_status,c.application_status,c.sales_order_no,u.utr_number,u.advance_doc,u.show,u.utr_approved from exh_reg_general_info a inner join exh_registration b on a.id=b.gid inner join exh_reg_payment_details c on a.id=c.gid	left join utr_history u on a.uid=u.registration_id and a.event_for=u.`show` where a.event_for='IIJS SIGNATURE 2023'";
	} else {
  $sql="select a.id,a.uid,a.company_name,a.contact_person,a.region,a.billing_address_id,a.get_billing_bp_number,a.created_date,b.last_yr_participant,
...
where a.event_for='IIJS SIGNATURE 2023' and  a.region='".$_SESSION["curruser_region_id"]."' ";
  ...
  { 
	...
	$schk_payment = "SELECT payment_status FROM `utr_history` WHERE 1 AND `registration_id`='".$rows['uid']."' AND `show`='IIJS SIGNATURE 2023' AND payment_status='captured'";
  ...
  <tr >
	...
	if(preg_match('/O/',$_SESSION['curruser_admin_access']))
	{ } else { ?>
	<a href="signature_exh_registration_step1.php?id=<?php echo $rows['id'];?>&registration_id=<?php echo $rows['uid'];?>"><img src="images/edit1.png" border="0" /></a> <?php } ?>
    <a href="signature_exh_registration_step4.php?id=<?php echo $rows['id'];?>&registration_id=<?php echo $rows['uid'];?>">A/D </a>
    ...
    <?php } ?>
       ...
       <?php  $registration_id = $rows['uid'];
       
        $get_allotment_payment = $conn->query("SELECT * FROM `utr_history` WHERE registration_id='$registration_id' AND `show`='IIJS SIGNATURE 2023' AND payment_made_for='ALLOTMENT' and payment_status='captured' limit 1");

        if($get_allotment_payment->num_rows > 0 ){
             $allotment_payment_result  = $get_allotment_payment->fetch_assoc();

             $part_salesorder_status = $allotment_payment_result['part_salesorder_status'];
             $part_isDone = $allotment_payment_result['isDone'];
             $part_utr_payment_status = $allotment_payment_result['payment_status'];
             $part_utr_number = $allotment_payment_result['utr_number'];
             

        	if(($part_salesorder_status =='0' || $part_salesorder_status =='') && $part_isDone==0 && $rows['sales_order_no']!='' && $part_utr_payment_status=='captured') { ?>
					<td align="center" class="part" data-url="<?php echo $rows['sales_order_no'] ;?> <?php echo $registration_id;?> <?php echo $part_utr_number;?>">Click Partial </td>
					<?php } elseif($part_utr_payment_status!='captured') { ?>
...
<?php echo pagination($limit,$page,'signature_exhibitor_rgistration_part.php?page=',$rCount); //call function to show pagination?>
</div>        
...
$(".so").click(function() {
	...
	if (confirm("Are you sure you want to create sales order")) {
		...
		method:"POST",
		data:{bpno:bpno,registration_id:registration_id,csrf_sap_token:csrf_sap_token,show:"IIJS SIGNATURE 2023"},
		type: "POST",
		beforeSend: function() {
        	$("#overlay").show();
    	},
		...
		{ 	
			...
			if($.trim(data)==1){
				...
				window.location.href = "signature_exhibitor_rgistration_part.php";
			}else{
				alert("Sorry There is some problem with SAP response"); 
				window.location.href = "signature_exhibitor_rgistration_part.php";			
			}
