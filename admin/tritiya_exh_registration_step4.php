...
include('fetch_space_tritiya_order.php'); /* Space TRITIYA order status API */
...
if(($_REQUEST['action']=='del') && ($_REQUEST['id']!=''))
{
	...
	echo"<meta http-equiv=refresh content=\"0;url=tritiya_exh_registration_step4.php?id=$gid&registration_id=$registration_id\">";
}
?>
<?php
...
if($saveUTR=="saveinfo")
{
	...
	elseif(empty($amountPaid)) { $amountPaidError = "Plz Enter Amount Paid"; }
		...
		if($event=="iijstritiya23") { $event_for="IIJS TRITIYA 2023"; }
...
<p><strong>IIJS Web Team,</strong></p>
			...
			//	$cc="notification@gjepcindia.com";	
				$subject = "Exhibitor Registration For IIJS TRITIYA 2023";
				$headers  = 'MIME-Version: 1.0' . "\n";
				$headers .= 'Content-type: text/html; charset=iso-8859-1' . "\n";	
				$headers .='From: IIJS TRITIYA 2023 <admin@gjepc.org>';
...
<form name="form1" action="tritiya_exh_registration_step4.php?id=<?php echo $gid; ?>&registration_id=<?php echo $registration_id; ?>&action=update" method="post" onsubmit="return validation()"> 
<?php 
...
<tr class="orange1"><td colspan="11">Online Payment Details IIJS TRITIYA 2023</td></tr>
<tr><td colspan="2">
	<table border="1" cellspacing="0" cellpadding="5" style="margin-bottom:10px;" width="100%">
			...
			{
				$query_sel = "select cheque_tds_amount from exh_reg_payment_details where uid='$registration_id' AND `show`='IIJS TRITIYA 2023' AND event_selected='iijs' limit 0,1";
				$result_sel = $conn->query($query_sel);								
				$row = $result_sel->fetch_assoc();								
				return $row['cheque_tds_amount'];							
			}
			
			$utrExist = "SELECT * FROM `utr_history` WHERE registration_id='$registration_id'  AND `show`='IIJS TRITIYA 2023' AND payment_made_for='SPACE'";
			...
			<?php }	?>
				...
				<?php } ?>
				<td align="center"><a  href="tritiya_exh_registration_step4.php?action=del&utr_id=<?php echo $id?>&id=<?php echo $gid?>&registration_id=<?php echo $registration_id?>" onClick="return(window.confirm('Are you sure you want to delete?'));">Delete</a></td>
				</tr>
				<?php  //$CheckFirstEntry++; ?>
				<?php $i++;} */	?>
			</tbody>
