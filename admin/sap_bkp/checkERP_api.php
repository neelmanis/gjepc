    ...
    <soapenv:Body>
      <gjep:MT_RENEW_IN>
         ...
         <SOAD_Header>
            ...
            <sold_cust>'.$BPId.'</sold_cust>
            <ship_cust>'.$BPId.'</ship_cust>
...
$xml_post_string .= "
	...
	<BP_Table>
	 ...
	 while($row = mysql_fetch_array($result2)){
		$addressIdentity = $row['address_identity'];
		if($addressIdentity=="CTC"){ $pan_phone = $row['pan_no']; } else { $pan_phone = $row['landline_no1']; }
...
/* header ("Content-Type:text/xml");
	...
	$p_bp_number = trim($bpRows['c_bp_number']); /* Pass Parent BP Number From Information Form*/
