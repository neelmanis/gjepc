...
if($_REQUEST['action']=='old_to_part')
{
		...
		$result_query = mysql_query($get);
		$getCount = mysql_num_rows($result_query);
		if($getCount >0){
		...
		echo "<meta http-equiv=refresh content=\"0;url=onspot_badges_registration.php?action=view\">";
		} else { 
...
{	
	...
	$aadhar_no=addslashes($_REQUEST['aadhar_no']);
	$pan_no=addslashes($_REQUEST['pan_no']);
	$sendMobile=addslashes(trim($_REQUEST['mobile']));
	...
	//$sqlx = "UPDATE `visitor_directory` SET `mod_date`=NOW(), name='$name', lname='$lname', email='$email', aadhar_no='$aadhar_no', pan_no='$pan_no', `visitor_approval`='$approval', `disapprove_reason`='$disapprove_reason',adminID='$adminID' WHERE visitor_id='$id' and registration_id='$registration_id'";
	$sqlx = "UPDATE `visitor_directory` SET `mod_date`=NOW(), name='$name', lname='$lname', aadhar_no='$aadhar_no', `visitor_approval`='$approval', `disapprove_reason`='$disapprove_reason',adminID='$adminID' WHERE visitor_id='$id' and registration_id='$registration_id'";
...
if($approval=='Y')
{
	$message = "Dear ".$name.", your data for visitor badge has been approved, Kindly proceed for visitor registration and payment at www.iijs.org";
...
<tbody>
	<tr>
	   ...
	   <td align="left"><img src="https://iijs.org/images/IIJS2018Logo.png" /></td>
	</tr>
    <tr>
    	<td colspan="3" height="30"><hr></td>
    </tr>
	<tr><td align="center"><strong><u>PARTICIPANT PRE-REGISTRATION APPLICATION FOR THE IIJS PREMIERE 2019</u></strong></td></tr>
	<tr>
    ...
    <a href="https://iijs.org/visitor_registration.php" target="_blank">https://iijs.org/visitor_registration.php</a></p>
	</td>
	</tr>
    <tr>
	<td colspan="2"><br/>
    ...
    <tr>
        <td align="center" colspan="3">
            <p>
                ...
                Unit G2-A, Trade Center, Opp. BKC Telephone Exchange, Bandra Kurla Complex, Bandra (E) Mumbai 400 051 <br> <b>Toll Free Number :</b> 1800-103-4353, <b>Missed Call Number :</b> +91-7208048100<br> Website: <a href="http://www.gjepc.org/">http://www.gjepc.org/</a>
            </p>
        </td>
    </tr>       
</tbody>
	...
	$subject = "YOUR DATA FOR VISITOR BADGE FOR THE IIJS PREMIERE 2019 APPROVED"; 
...
{
	$message = "Dear ".$name.", your data for visitor badge has been disapproved, Due to $disapprove_reason kindly update your record at www.iijs.org";
	get_data($message,$sendMobile); 
...
<tbody>
    ...
    <a href="https://www.iijs.org/employee_directory.php" target="_blank">https://www.iijs.org/employee_directory.php</a></p>
	</td>
	</tr>
    <tr>
	<td colspan="2"><br/>
	...
	$to =$email_id.',pvr@gjepcindia.com';
	$subject = "YOUR DATA FOR VISITOR BADGE FOR THE IIJS PREMIERE 2019 DISAPPROVED"; 
	$headers  = 'MIME-Version: 1.0' . "\n"; 
	$headers .= 'Content-type: text/html; charset=iso-8859-1' . "\n"; 
	$headers .= 'From:admin@gjepc.org';			
	mail($to, $subject, $message, $headers);
...
/***  Emailer End **/
echo"<meta http-equiv=refresh content=\"0;url=onspot_badges_registration.php?action=viewReg&regid=$registration_id\">";
}
}
?>
...
{	
	...
	$company_pan_no=addslashes($_REQUEST['company_pan_no']);
	$company_gstn=addslashes($_REQUEST['company_gstn']);
	$mobile_no=addslashes($_REQUEST['mobile_no']);
	$company_type=addslashes($_REQUEST['comp_type']);
	...
	$query=mysql_query("select * from registration_master where company_pan_no='$company_pan_no' AND id!=$registration_id");
    $num=mysql_num_rows($query);
	if($num>0)
	{
		echo '<script language="javascript">';
		echo 'alert("PAN No Not Updated because already exist")';
		echo '</script>';
		
	echo"<meta http-equiv=refresh content=\"0;url=onspot_badges_registration.php?action=view\">";exit;
	}
	
	 $sqlreg = "UPDATE `registration_master` SET company_pan_no='$company_pan_no', company_gstn='$company_gstn',company_type='$company_type',`approval_status`='$approval_status', `disapprove`='$disapprove' WHERE id='$registration_id'";
...
<tbody>
	...
	$to =$email_id.',pvr@gjepcindia.com';
	$subject = "YOUR GST and PAN No FOR THE IIJS PREMIERE 2019 Approved"; 
	$headers  = 'MIME-Version: 1.0' . "\n"; 
	$headers .= 'Content-type: text/html; charset=iso-8859-1' . "\n"; 
	$headers .= 'From:admin@gjepc.org';			
	mail($to, $subject, $message, $headers);
...
<tbody>
	...
	$to =$email_id.',pvr@gjepcindia.com';
	$subject = "YOUR GST and PAN No FOR THE IIJS PREMIERE 2019 Disapproved"; 
	$headers  = 'MIME-Version: 1.0' . "\n"; 
	$headers .= 'Content-type: text/html; charset=iso-8859-1' . "\n"; 
	$headers .= 'From:admin@gjepc.org';			
	mail($to, $subject, $message, $headers);
...
if($_REQUEST['Reset']=="Reset")
{
  ...
  header("Location: onspot_badges_registration.php?action=view");
} else
{
...
<div id="main">
	<div class="content">
		...
		<?php echo strtoupper(getNameCompany($_REQUEST['regid'])); echo "&nbsp;"; echo getCompanyType($_REQUEST['regid']);?>&nbsp;
		<?php } ?>
		
		<!--<a href="combo_visitor.php?action=view" target="_blank">COMBO Visitors</a>-->
		
        <?php if($_REQUEST['action']=='orderDetails') { ?>
        Order Details <div class="content_head_button" style="float:right; padding-right:10px; font-size:12px;"><a href="onspot_badges_registration.php?action=view">Back</a></div>
        ...
        <a href="onspot_badges_registration.php?action=orderDetails&regid=<?php echo $registration_id; ?>">Back</a></div>
        Payment Details
        <?php } elseif($_REQUEST['action']=='viewReg'){ ?>
        <div class="content_head_button" style="float:right; padding-right:10px; font-size:12px;"><a href="onspot_badges_registration.php?action=view">Back</a></div> 
		
		<?php } elseif($_REQUEST['action']=='view'){ ?> Onspot Visitor 
        <div class="content_head_button" style="float:right; padding-right:10px; font-size:12px;"><a href="iijs_export_visitor_onspot.php">Export Report</a></div>
        ...
        <a href="onspot_badges_registration.php?action=viewReg&regid=<?php echo $registration_id; ?>">Back</a></div>
        ...
        <a href="onspot_badges_registration.php?action=view">Back</a></div> 
        <?php } ?>			
		<?php
		if($_SESSION['curruser_login_id']=='44' || $_SESSION['curruser_login_id']=='1'){ ?>
		<a href="onspot_badges_registration.php?action=old_to_part" onClick="return(window.confirm('Are you sure you want to Clear Old to Part Data'));" >Clear Old to Part Data</a>	
		<?php }	?>
        </div>
<?php if($_REQUEST['action']=='view') { ?>
<div class="content_details1">
<?php 
	$sql5="select * from visitor_directory where isApplied='Y' AND registration_id!='0' AND visitor_approval!='O' and paymentThrough='onSpot'";
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="detail_txt" >
  ...
  $sql="SELECT DISTINCT rm.id, rm.company_name, rm.company_pan_no, rm.mobile_no, rm.company_gstn, rm.email_id,rm.approval_status FROM registration_master rm inner join visitor_directory vd on rm.id=vd.registration_id AND vd.isApplied='Y' AND vd.visitor_approval!='O' and vd.paymentThrough='onSpot'";
...
AND vd.isApplied='Y' AND vd.visitor_approval!='O' AND vo.payment_type='online'"; */
  ...
  <tr>
    ...
    <td align="center" valign="middle"><a href="onspot_badges_registration.php?actions=companyedit&regid=<?php echo $rows['id'];?>">
    ...
    <a href="onspot_badges_registration.php?action=viewReg&regid=<?php echo $rows['id'];?>">Employee Details</a> / <a href="onspot_badges_registration.php?action=orderDetails&regid=<?php echo $rows['id'];?>">Order Details</a></td>
...
<?php echo pagination($limit,$page,'onspot_badges_registration.php?action=view&page=',$rCount); //call function to show pagination?>
</div>  
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="detail_txt" >
  ...
  <tr>
    ...
    <td><?php echo strtoupper($rows['name'].' '.$rows['lname']);?></td>
	<td><?php echo strtoupper($rows['bp_number']);?></td>
    <td><?php echo $rows['degn_type'];?></td> 
    <td><?php echo getVisitorDesignation($rows['designation']);?></td> 
    ...
    <td align="left" valign="middle"><a href="onspot_badges_registration.php?action=edit&id=<?php echo $rows['visitor_id'];?>&regid=<?php echo $rows['registration_id'];?>">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="detail_txt" >
  ...
  ?>
  <tr >
    <td><?php echo getNameCompany($rows['regId']);?></td>
    <td><?php echo $rows['payment_type'];?></td> 
    <td><?php echo $rows['txn_msg'];?></td> 
    <td><?php echo $rows['orderId'];?></td> 
	<td align="left" valign="middle"><a href="onspot_badges_registration.php?action=orderHistory&orderId=<?php echo $rows['orderId'];?>&regid=<?php echo $rows['regId'];?>" style="color:#000000">VIEW</a></td>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="detail_txt" >
  ...
  ?>
  <tr >
    <td><?php echo VisitorFLName($rows['visitor_id']);?></td> 
...
<table width="100%" border="0" cellspacing="2" cellpadding="2" class="detail_txt" >
  ...
  <tr >
    ...
    <td>  
    <a data-fancybox="gallery" href="https://iijs.org/images/employee_directory/<?php echo $registration_id; ?>/photo/<?php echo $photo; ?>"> 
    <img class="blah" src="https://iijs.org/images/employee_directory/<?php echo $registration_id; ?>/photo/<?php echo $photo; ?>" alt="your image" /></a> 
    </td>
  </tr>
  <tr >
    <td ><strong>Pan Card Copy</strong></td>
    <td>
	<a data-fancybox="gallery" href="https://iijs.org/images/employee_directory/<?php echo $registration_id; ?>/pan_copy/<?php echo $pan_copy; ?>">
    <img class="blah" src="https://iijs.org/images/employee_directory/<?php echo $registration_id; ?>/pan_copy/<?php echo $pan_copy; ?>" alt="your image"/>
    </a> 
    </td>
  </tr>
  ...
  <tr >
    ...
    <td>
    <a data-fancybox="gallery" href="https://iijs.org/images/employee_directory/<?php echo $registration_id; ?>/salary/<?php echo $salary_slip_copy; ?>">
    <img class="blah" src="https://iijs.org/images/employee_directory/<?php echo $registration_id; ?>/salary/<?php echo $salary_slip_copy; ?>" alt="your image"  /></a> 
    </td>
  </tr>
  <?php } else { ?>
  <tr >
    ...
    <td >
    <a data-fancybox="gallery" href="https://iijs.org/images/employee_directory/<?php echo $registration_id; ?>/partner/<?php echo $partner; ?>">
    <img class="blah" src="https://iijs.org/images/employee_directory/<?php echo $registration_id; ?>/partner/<?php echo $partner; ?>" alt="your image" />
    </a> 
    </td>
  </tr>
  <?php } ?>
...
<?php 
  ...
  if(($_REQUEST['regid']!='') || ($_REQUEST['actions']=='companyedit'))
  {
		...
		{			
			...
			$company_type=stripslashes($row3['company_type']);
...
<table width="100%" border="0" cellspacing="2" cellpadding="2" class="detail_txt" >
  ...
  <tr>
    ...
    <!--<td><input type="text" name="comp_type" id="comp_type" class="input_txt" value="<?php echo $company_type; ?>" /></td>-->
	<td><input type="radio" name="comp_type" id="comp_type" value="propritory" <?php if($comp_type=='propritory'){ echo "checked"; }?>>Proprietary
      <input type="radio" name="comp_type" id="comp_type" value="partnership" <?php if($comp_type=='partnership'){ echo "checked"; }?>>Partnership
      <input type="radio" name="comp_type" id="comp_type" value="pvt" <?php if($comp_type=='pvt'){ echo "checked"; }?>>Private Ltd.
      <input type="radio" name="comp_type" id="comp_type" value="public" <?php if($comp_type=='public'){ echo "checked"; }?>>Public Ltd.
      <input type="radio" name="comp_type" id="comp_type" value="llp" <?php if($comp_type=='llp'){ echo "checked"; }?>>LLP
      <input type="radio" name="comp_type" id="comp_type" value="huf" <?php if($comp_type=='huf'){ echo "checked"; }?>>HUF
      </td>
  </tr>
  <tr>
  ...
  <tr >
    ...
    <td>  
    <a data-fancybox="gallery" href="https://iijs.org/images/gst_copy/<?php echo $gst_copy; ?>"> 
    <img class="blah" src="https://iijs.org/images/gst_copy/<?php echo $gst_copy; ?>" alt="your image" /></a> 
    </td>
  </tr>
  <tr >
    <td ><strong>Pan Card Copy</strong></td>
    <td>
	<a data-fancybox="gallery" href="https://iijs.org/images/pan_no_copy/<?php echo $pan_no_copy; ?>">
    <img class="blah" src="https://iijs.org/images/pan_no_copy/<?php echo $pan_no_copy; ?>" alt="your image"/>
    </a> 
    </td>
  </tr>
  <tr>
