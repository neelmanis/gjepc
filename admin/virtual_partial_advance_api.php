...
//if(true)
{
	...
	$query="select * from virtual_event_registration where event_selected='$event' AND payment_status='approved' AND application_status='approved' AND registration_id='".$registration_id."' ";
	$getChallanResult = $conn ->query($query);
	$eventCountx = $getChallanResult->num_rows;
	$challanResult = $getChallanResult->fetch_assoc();
		
	/* UTR Data */
	$getUTR = "SELECT * FROM `utr_history` where registration_id='$registration_id' AND utr_approved='Y' AND utr_number='$utr_no' AND `show` ='$show' AND event_selected='$event' limit 1";
	$utrResult = $conn ->query($getUTR);
	$utrResultrows = $utrResult->fetch_assoc();
	$utr_number = trim($utrResultrows['utr_number']);
	    ...
	    <soapenv:Body>
			<gjep:MT_PADV_IN>
			 <SO_Advance>
				...
				<wbs_element>DE-051</wbs_element>
           ...
           // curl_setopt($ch1, CURLOPT_TIMEOUT, 10);
			...
			foreach($xml->xpath('//soapenv:Body') as $header)
			{
					...
					{	$flag=1;
						...
						$sqlx = "UPDATE `utr_history` SET `sap_push_date`=NOW(),`sap_push_admin`='$adminID',`part_sales_order_no`='$sales_order_no',`part_salesorder_status`='1' WHERE `utr_number`='$utr_no' AND `registration_id`='$registration_id' AND `show` ='$show' AND event_selected='$event'";
						$resulty = $conn ->query($sqlx);
					}
			}
			echo $flag;	
	}
