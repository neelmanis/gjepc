...
$sql="select * from exh_reg_general_info where uid='$registration_id' and event_for='$show_value'";
...
if($action=='UPDATE')
{
	...
	header("Location: exhibitor_space_registration_step4.php?gid=$gid&registration_id=$uid&shortcode=$shortcode");
	}
}
?>
...
<title><?php echo $show_name; ?> &gt; Exhibitor Registration &gt; Step-3</title>
...
<div id="main">
	<div class="content">
        ...
        <div style="float:right; padding-right:10px; font-size:12px;"><a href="exhibitor_space_registration.php">Back to Search</a></div>
        </div>
...
//echo "select * from exh_registration where uid='$registration_id' and `show`='IIJS Signature 2018' and last_yr_section_flag='1' order by exh_id desc limit 0,1";
	...
	$query1=$conn->query("select * from exh_registration where uid='$registration_id' and `show`='$show_value' order by exh_id desc limit 0,1");
	$result1=$query1->fetch_assoc();
	$exh_id=$result1['exh_id'];
	$num1=$query1->num_rows;
	
	$query=$conn->query("select * from exh_registration where uid='$registration_id' and gid='$gid' and `show`='$show_value'");
