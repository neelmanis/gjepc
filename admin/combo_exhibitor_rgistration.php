...
if($_REQUEST['Reset']=="Reset")
{
  ...
  header("Location: combo_exhibitor_rgistration.php");
...
{ 
	...
	if($search_type=='SEARCH')
	{
		if($_SESSION['first_name']=="" && $_SESSION['company_name']=="" && $_SESSION['from_date']=="Form" && $_SESSION['to_date']=="To" && $_SESSION['country']=="" && $_SESSION['status']=="" && $_SESSION['last_yr_participant']=="" && $_SESSION['section']=="" && $_SESSION['selected_area']=="" && $_SESSION['selected_premium_type']=="" && $_SESSION['payment_status']=="" && $_SESSION['document_status']=="" && $_SESSION['region']=="")
		{
...
{
		$get = "SELECT * FROM exh_reg_payment_details where `show`='IIJS Tritiya 2023' or `show`='IIJS Signature 2023' and AND (sales_order_no='tem categor' || sales_order_no='rror in doc' || sales_order_no='old-to part')";
		$result_query = $conn ->query($get);
		$getCount = $result_query->num_rows;
		
		$get2 = "SELECT * FROM `utr_history` where `show` ='IIJS Tritiya 2023' or `show`='IIJS Signature 2023' AND event_selected='iijstritiya23' or event_selected='iijstritiya23' AND (first_sales_order_no='tem categor' || first_sales_order_no='rror in doc' || first_sales_order_no='old-to part')";
		...
		$sql="UPDATE `exh_reg_payment_details` SET `sap_sale_order_create_status` = '0', `sales_order_no` =  '' WHERE `show`='IIJS Tritiya 2023' or `show`='IIJS Signature 2023' AND (sales_order_no='tem categor' || sales_order_no='rror in doc' || sales_order_no='old-to part') AND `sap_sale_order_create_status` = '1'";
		$result = $conn ->query($sql);  
		
		$sql2="UPDATE `utr_history` SET `sap_so_status` = '0', `first_sales_order_no` =  '' WHERE `show`='IIJS Tritiya 2023' or `show`='IIJS Signature 2023' AND event_selected='iijstritiya23' or event_selected='iijstritiya23' AND (first_sales_order_no='tem categor' || first_sales_order_no='rror in doc' || first_sales_order_no='old-to part') AND `sap_so_status` = '1'";
		$result2 = $conn ->query($sql2); 
		 
        if(!$result) die ($conn->error);
		echo "<meta http-equiv=refresh content=\"0;url=combo_exhibitor_rgistration.php\">";
		} else { 
...
<title>IIJS Combo Exhibitor List</title>
...
<div class="breadcome"><a href="admin.php">Home</a> >IIJS-Combo</div>
</div>

<div id="main">
	<div class="content">
    
    	<div class="content_head"><a href="combo_exhibitor_rgistration.php">
    	<div class="content_head_button">IIJS-Combo Exhibitor List</div></a>
		...
		<!--<a href="export_utr_combo_exhibitor_rgistration.php">&nbsp;Download Payment Data</a> <a href="export_approve_combo_exhibitor_rgistration.php">&nbsp;Download Data</a> -->
		</div>
		<?php
		if($_SESSION['curruser_login_id']=='28' || $_SESSION['curruser_login_id']=='1'){ ?>
		<a href="combo_exhibitor_rgistration.php?action=old_to_part" onClick="return(window.confirm('Are you sure you want to Clear Old to Part Data'));" >Clear SAP Error</a>	
		<?php }	?>
		</div>
...
{
	 $sql5="select a.id,a.company_name,a.contact_person,a.region,a.created_date,b.last_yr_participant,b.section,b.selected_area,b.selected_premium_type,c.payment_status,c.document_status,c.application_status from exh_reg_general_info a inner join exh_registration b on a.id=b.gid inner join exh_reg_payment_details c on a.id=c.gid where a.event_for='IIJS Signature 2023' OR a.event_for='IIJS TRITIYA 2023'";
}
else
{
/*	if(preg_match('/O/',$_SESSION['curruser_admin_access']))
	...
	where a.event_for='$show'";
	} else { */
	if($_SESSION['curruser_login_id']==160)
	{
	$sql5="select a.id,a.company_name,a.contact_person,a.region,a.created_date,b.last_yr_participant,b.section,b.selected_area,b.selected_premium_type,c.payment_status,c.document_status,c.application_status from exh_reg_general_info a inner join exh_registration b on a.id=b.gid inner join exh_reg_payment_details c on a.id=c.gid wherea.event_for='IIJS TRITIYA 2023' OR a.event_for='IIJS TRITIYA 2023'";	
	} else {
	$sql5="select a.id,a.company_name,a.contact_person,a.region,a.created_date,b.last_yr_participant,
	b.section,b.selected_area,b.selected_premium_type,c.payment_status,c.document_status,c.application_status 
	from exh_reg_general_info a inner join exh_registration b on a.id=b.gid inner join exh_reg_payment_details c on a.id=c.gid
	where a.event_for='IIJS Signature 2023' OR a.event_for='IIJS TRITIYA 2023' and  a.region='".$_SESSION["curruser_region_id"]."' ";
	}
}
...
<table width="100%" border="0" cellspacing="2" cellpadding="2" class="detail_txt" >
  ...
  <!--<tr >
   ...
   <tr>
    ...
    <tr><td><strong>Check Sales order Status</strong></td><td>
  ...
  <tr>
    ...
    <?php 
	$section_query = "select * from signature_section_master";
  ...
  <tr>
    ...
    <?php 
	$section_query = "select * from signature_area_master";
	$execute_section = $conn->query($section_query);
	while($show_section = $execute_section->fetch_assoc())
	{			
	?>
  ...
  <tr>
    <td>Selected Premium Type</td>
     <td>
    <select name="selected_premium_type" >
        <option value="">---Select Premium Type---</option>
    	 <?php $section_query = "select * from signature_premium_master";
			$execute_section = $conn->query($section_query);			
			while($show_section =$execute_section->fetch_assoc())
			{			
			?>
...
if($_SESSION['curruser_role']=="Super Admin")
{
	$sql="select a.id,a.uid,a.company_name,a.contact_person,a.region,a.billing_address_id,a.get_billing_bp_number,a.created_date,
b.last_yr_participant,b.section,b.selected_area,b.selected_premium_type,b.options,c.payment_status,c.sap_sale_order_create_status,c.document_status,c.application_status,c.sales_order_no,u.utr_number,u.advance_doc,u.show,u.utr_approved,u.part_sales_order_no,u.part_salesorder_status,u.tds_salesorder_status 
from exh_reg_general_info a inner join exh_registration b on a.id=b.gid inner join exh_reg_payment_details c on a.id=c.gid left join utr_history u on a.uid=u.registration_id and a.event_for=u.`show` where a.event_for='IIJS Signature 2023' OR a.event_for='IIJS TRITIYA 2023'";
}
else
{
/*	if(preg_match('/O/',$_SESSION['curruser_admin_access']))
	{
	$sql ="select a.id,a.uid,a.company_name,a.contact_person,a.region,a.country,a.billing_address_id,a.created_date,b.last_yr_participant,
	b.section,b.selected_area,b.selected_premium_type,c.payment_status,c.document_status,c.application_status 
	from exh_reg_general_info a inner join exh_registration b on a.id=b.gid inner join exh_reg_payment_details c on a.id=c.gid left join utr_history u on a.uid=u.registration_id where a.event_for='$show' AND u.event_selected='signature'";
	} else { */
	if($_SESSION['curruser_login_id']==160)
	{
	$sql="select a.id,a.uid,a.company_name,a.contact_person,a.region,a.billing_address_id,a.get_billing_bp_number,a.created_date,
...
from exh_reg_general_info a inner join exh_registration b on a.id=b.gid inner join exh_reg_payment_details c on a.id=c.gid left join utr_history u on a.uid=u.registration_id and a.event_for=u.`show` where a.event_for='IIJS Signature 2023' OR a.event_for='IIJS TRITIYA 2023'";	
	...
	$sql="select a.id,a.uid,a.company_name,a.contact_person,a.region,a.country,a.billing_address_id,c.created_date,b.last_yr_participant,b.options,
	b.section,b.selected_area,b.selected_premium_type,c.sap_sale_order_create_status,c.payment_status,c.document_status,c.application_status ,c.sales_order_no
	from exh_reg_general_info a inner join exh_registration b on a.id=b.gid inner join exh_reg_payment_details c on a.id=c.gid where a.event_for='IIJS Signature 2023' OR a.event_for='IIJS TRITIYA 2023' and  a.region='".$_SESSION["curruser_region_id"]."'";
	}
}
  ...
  {	
  		if($_SESSION["sales_order_status"]=='Y')
			...
			$sql.=" and c.sales_order_no IS NULL";		
  }
...
//	$schk_membership="SELECT * FROM `approval_master`  WHERE 1 and `registration_id`='".$rows['uid']."' and issue_membership_certificate_expire_status='Y'";
  ...
  <tr >
	...
	<td><?php echo $rows['advance_doc'];?></td>
    <?php /*?><td><?php echo strtoupper($rows['contact_person']);?></td><?php */?>
    ...
    <td><?php echo $rows['last_yr_participant']." | ".getSection_desc($rows['section'],$conn)." | ".$rows['selected_area']." | ".$rows['selected_premium_type']; ?></td>
    <td> 
	<?php if($rows['options']=="Same stall position size as of previous year")
			echo "Retain";
		 else if($rows['options']=="Same area but different location as of previous year Signature")
		 	echo "Different Location";
		else if($rows['options']=="More area than previous year Signature")
			echo "More Area";
	...
	if(preg_match('/O/',$_SESSION['curruser_admin_access']))
	{ } else { ?>
	<!-- <a href="tritiya_exh_registration_step1.php?id=<?php //echo $rows['id'];?>&registration_id=<?php //echo $rows['uid'];?>"><img src="images/edit1.png" border="0" /></a>  -->
	<?php } ?>
	<!-- <a href="tritiya_exh_registration_step4.php?gid=<?php //echo $rows['id'];?>&registration_id=<?php //echo $rows['uid'];?>&bp_number=<?php //echo $billingBP;?>">A/D</a>  -->
...
<?php echo pagination($limit,$page,'combo_exhibitor_rgistration.php?page=',$rCount); //call function to show pagination?>
</div>        
</div>
</div>
...
$(".so").click(function() {
	...
	//alert(csrf_sap_token);
	
	if (confirm("Are you sure you want to create sales order")) {
		$.ajax({
		url: "api_export_exhibition_ERP.php",
		method:"POST",
	//	data:{bpno:bpno,registration_id:registration_id,csrf_sap_token:csrf_sap_token},
		...
		{ 	//console.log(data); exit;
			if($.trim(data)==1){
				...
				window.location.href = "combo_exhibitor_rgistration.php";
			} else {
				alert("Sorry There is some problem with SAP response");
				window.location.href = "combo_exhibitor_rgistration.php";		
			}
