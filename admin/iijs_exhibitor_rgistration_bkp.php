...
{
		$get = "SELECT * FROM exh_reg_payment_details where `show`='IIJS PREMIERE 2022' AND (sales_order_no='tem categor' || sales_order_no='rror in doc' || sales_order_no='old-to part' || sales_order_no='pecify eith' || sales_order_no='o customer')";
		$result_query = $conn ->query($get);
		$getCount = $result_query->num_rows;
		
		$get2 = "SELECT * FROM `utr_history` where `show` ='IIJS PREMIERE 2022' AND event_selected='iijs' AND (first_sales_order_no='tem categor' || first_sales_order_no='rror in doc' || first_sales_order_no='old-to part' || first_sales_order_no='pecify eith' || first_sales_order_no='o customer')";
	...
	//	$sql="UPDATE `visitor_order_detail` SET `sap_sale_order_create_status` = '0', `sales_order_no` ='' WHERE `sales_order_no` ='old-to part' AND `sap_sale_order_create_status` = '1'";
		$sql="UPDATE `exh_reg_payment_details` SET `sap_sale_order_create_status` = '0', `sales_order_no` = '' WHERE `show`='IIJS PREMIERE 2022' AND (sales_order_no='tem categor' || sales_order_no='rror in doc' || sales_order_no='old-to part' || sales_order_no='pecify eith' || sales_order_no='o customer') AND `sap_sale_order_create_status` = '1'";
		$result = $conn ->query($sql);  
		
		$sql2="UPDATE `utr_history` SET `sap_so_status` = '0', `first_sales_order_no` =  '' WHERE `show`='IIJS PREMIERE 2022' AND event_selected='iijs' AND (first_sales_order_no='tem categor' || first_sales_order_no='rror in doc' || first_sales_order_no='old-to part' || first_sales_order_no='pecify eith' || first_sales_order_no='o customer') AND `sap_so_status` = '1'";
		$result2 = $conn ->query($sql2); 
		 
        if(!$result) die ($conn->error);
		echo "<meta http-equiv=refresh content=\"0;url=iijs_exhibitor_rgistration.php\">";
		} else { 
...
<div id="main">
	<div class="content">
		...
		<a href="export_utr_NM_iijs_exh_rgistration.php">&nbsp;Download Payment Including NM Data</a> | <a href="export_utr_iijs_exhibitor_rgistration.php">&nbsp;Download Payment Data</a> | <a href="export_approve_iijs_exhibitor_registration.php">&nbsp;Download All Data</a></div>
		<?php
...
b.section,b.selected_area,b.selected_premium_type,c.payment_status,c.document_status,c.application_status 
from exh_reg_general_info a inner join exh_registration b on a.id=b.gid inner join exh_reg_payment_details c on a.id=c.gid
where a.event_for='IIJS PREMIERE 2022' and  a.region='".$_SESSION["curruser_region_id"]."' ";
}
}
...
b.section,b.selected_area,b.selected_premium_type,b.options,c.payment_status,c.sap_sale_order_create_status,c.document_status,c.application_status,c.sales_order_no,u.utr_number,u.show,u.utr_approved from exh_reg_general_info a inner join exh_registration b on a.id=b.gid inner join exh_reg_payment_details c on a.id=c.gid left join utr_history u on a.uid=u.registration_id where a.event_for='IIJS PREMIERE 2022' and u.show='IIJS PREMIERE 2022'"; */
...
b.section,b.selected_area,b.selected_premium_type,b.options,c.payment_status,c.sap_sale_order_create_status,c.document_status,c.application_status,c.sales_order_no,u.utr_number,u.advance_doc,u.show,u.utr_approved,u.part_sales_order_no,u.part_salesorder_status,u.tds_salesorder_status from exh_reg_general_info a inner join exh_registration b on a.id=b.gid inner join exh_reg_payment_details c on a.id=c.gid left join utr_history u on a.uid=u.registration_id where a.event_for='IIJS PREMIERE 2022' AND (u.event_selected='iijs' || u.event_selected='signature')";*/

$sql="select a.id,a.uid,a.company_name,a.contact_person,a.region,a.billing_address_id,a.get_billing_bp_number,a.created_date,b.last_yr_participant,
b.section,b.selected_area,b.selected_premium_type,b.options,c.payment_status,c.sap_sale_order_create_status,c.document_status,c.application_status,c.sales_order_no,u.utr_number,u.advance_doc,u.show,u.utr_approved,u.part_sales_order_no,u.part_salesorder_status,u.tds_salesorder_status,u.payment_status as 'paymentCheck' from exh_reg_general_info a inner join exh_registration b on a.id=b.gid inner join exh_reg_payment_details c on a.id=c.gid left join utr_history u on a.uid=u.registration_id and a.event_for=u.`show` where a.event_for='IIJS PREMIERE 2022'";
}
else
{
	if(preg_match('/O/',$_SESSION['curruser_admin_access']))
	{
	$sql="select a.id,a.uid,a.company_name,a.contact_person,a.region,a.billing_address_id,a.get_billing_bp_number,a.created_date,b.last_yr_participant,
b.section,b.selected_area,b.selected_premium_type,b.options,c.payment_status,c.sap_sale_order_create_status,c.document_status,c.application_status,c.sales_order_no,u.utr_number,u.advance_doc,u.show,u.utr_approved from exh_reg_general_info a inner join exh_registration b on a.id=b.gid inner join exh_reg_payment_details c on a.id=c.gid	left join utr_history u on a.uid=u.registration_id and a.event_for=u.`show` where a.event_for='IIJS PREMIERE 2022'";
	} else {
  $sql="select a.id,a.uid,a.company_name,a.contact_person,a.region,a.billing_address_id,a.get_billing_bp_number,a.created_date,b.last_yr_participant,
...
where a.event_for='IIJS PREMIERE 2022' and  a.region='".$_SESSION["curruser_region_id"]."' ";
  ...
  { 
	...
	$schk_payment = "SELECT payment_status FROM `utr_history` WHERE 1 AND `registration_id`='".$rows['uid']."' AND `show`='IIJS PREMIERE 2022' AND payment_status='captured'";
  ...
  <tr >
	...
	if(preg_match('/O/',$_SESSION['curruser_admin_access']))
	{ } else { ?>
	<a href="iijs_exh_registration_step1.php?id=<?php echo $rows['id'];?>&registration_id=<?php echo $rows['uid'];?>"><img src="images/edit1.png" border="0" /></a> <?php } ?>
	...
	if (confirm("Are you sure you want to create sales order")) {
		...
		{ 	
			...
			if($.trim(data)==1){
				...
				window.location.href = "iijs_exhibitor_rgistration.php";
			}else{
				alert("Sorry There is some problem with SAP response"); 
				window.location.href = "iijs_exhibitor_rgistration.php";			
			}
