...
{
		$get = "SELECT * FROM  `visitor_order_detail` WHERE  (`sales_order_no` = 'old-to part' || `sales_order_no` = 'o customer' || `sales_order_no` = 'pecify eith') AND `sap_sale_order_create_status` =  '1'";
		$result_query = $conn ->query($get);
		$getCount = $result_query->num_rows;
		if($getCount >0){
		$sql="UPDATE `visitor_order_detail` SET `sap_sale_order_create_status` = '0', `sales_order_no` ='' WHERE (`sales_order_no` = 'old-to part' || `sales_order_no` = 'o customer' || `sales_order_no` = 'pecify eith') AND `sap_sale_order_create_status` = '1'";
...
{	
	...
	$sqlx = "UPDATE `visitor_directory` SET `mod_date`=NOW(), name='$name', lname='$lname', aadhar_no='$aadhar_no', `visitor_approval`='$approval', `disapprove_reason`='$disapprove_reason',category='$category',adminID='$adminID' WHERE visitor_id='$id' and registration_id='$registration_id'";
...
else if($approval=='D') //Send mail For DisApproved
{
	...
	/* WhastApp Msg */
	/* $mob = '919619662253';
	$msgurl = single_visitor_disapproval($mob,$name,$disapprove_reason,$url);
	$getResults = json_decode($msgurl,true);
	if($getResults['response']['status']=="success")
	{
		echo $getResults['response']['status'];
...
<table width="100%" border="0" cellspacing="2" cellpadding="2" class="detail_txt" >
  ...
  <tr >
    ...
    <img class="blah" src="https://registration.gjepc.org/images/employee_directory/<?php echo $registration_id; ?>/photo/<?php echo $photo; ?>" title="<?php echo $name;?>" /></a> 
    <?php } ?>   
    </td>
  </tr>
  <tr>
