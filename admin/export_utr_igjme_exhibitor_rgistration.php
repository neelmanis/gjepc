...
$category = 'IGJME EXHIBITOR';
$report_name = 'DOWNLOAD IGJME PAYMENT DATA';
...
{     
    if($payment_made_for =="ALLOTMENT"){
        $query_sel = "select tdsAmount from utr_history where registration_id='$registration_id' AND `show`='IGJME' AND event_selected='igjme23' and utr_number='$utr_number'";
        $result_sel = $conn->query($query_sel);               
        $row = $result_sel->fetch_assoc();                
...
$sql="SELECT u.registration_id,a.id,a.company_name, u.show,u.utr_number,u.amountPaid,u.tdsAmount,u.utr_approved,u.payment_status,u.payment_date,c.c_bp_number,u.payment_made_for FROM utr_history u, registration_master a,communication_address_master c where u.registration_id=a.id and u.registration_id=c.registration_id and c.type_of_address=2 and u.show='IGJME'";
