...
if(!empty($_POST)) 
{
	...
	/* UTR Data */
	$getUTR = "SELECT * FROM `utr_history` where registration_id='$registration_id' AND `show`='$show' AND payment_made_for='PART' AND utr_number='$utr_no' AND payment_status='captured' limit 1";
	...
	echo $xml_exhibition_string; exit; */
           ...
           // curl_setopt($ch1, CURLOPT_TIMEOUT, 10);
			...
			foreach($xml->xpath('//soapenv:Body') as $header)
			{
					...
					$sqlx = "UPDATE `utr_history` SET `sap_push_date`=NOW(),`sap_push_admin`='$adminID',`part_sales_order_no`='$sales_order_no',`part_salesorder_status`='1' WHERE `utr_number`='$utr_no' AND `registration_id`='$registration_id' AND `show` ='$show' AND payment_made_for='PART'"; 
