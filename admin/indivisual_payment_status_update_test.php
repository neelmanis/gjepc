...
$querys =  $conn ->query("select * from challan_master where challan_financial_year='2022' and  registration_id='600907393'");
//$querys =  $conn ->query("select * from challan_master where challan_financial_year='2022' and (Response_Code is null || Response_Code!='E000') limit 10");

//$querys =  $conn ->query("select * from challan_master where challan_financial_year='2022' and registration_id='$registration_id' ");
while($result = $querys->fetch_assoc())
{
	$post_date=date('Y-m-d');
	$ReferenceNo = trim($result['ReferenceNo']);
	echo $registration_id = intval($result['registration_id']);
