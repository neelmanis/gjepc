...
if($_REQUEST['Reset']=="Reset")
{
  ...
  header("Location: manage_visitor_order_test.php?action=view");
} else {
...
<td>Products Dealing in</td>
<td>Objective of Visiting</td>
<td>How you know about the show</td>
</tr>';
	...
	{ 
		...
		$getObmp ="SELECT pd_jewellery,obj_of_visit,how_you_learn_abt_iijs FROM visitor_obmp_details where uid='$registration_id' limit 1";
		$getObmpResult = $conn ->query($getObmp);
		$getObmpRow= $getObmpResult->fetch_assoc();
		$pd_jewellery = $getObmpRow['pd_jewellery']; 
		$obj_of_visit = $getObmpRow['obj_of_visit']; 
		$how_you_learn_abt_iijs = $getObmpRow['how_you_learn_abt_iijs']; 
...
<div id="main">
    ...
    <div style="float:right; padding-right:10px; font-size:12px;"><a href="manage_visitor_order_test.php?action=view">Back to Visitor</a></div>
    <?php } ?>
    </div>-->
...
<?php echo pagination($limit,$page,'manage_visitor_order_test.php?action=view&page=',$rCount); //call function to show pagination?>
</div>
</div>
</div>
