...
if($_REQUEST['Reset']=="Reset")
{
  ...
  header("Location: manage_lost_badges.php?action=view");
} else
{
...
<div id="main">
	<div class="content">
        ...
        Order Details <div class="content_head_button" style="float:right; padding-right:10px; font-size:12px;"><a href="manage_lost_badges.php?action=view">Back</a></div>
        ...
        <a href="manage_lost_badges.php?action=orderDetails&regid=<?php echo $registration_id; ?>">Back</a></div>
        Payment Details
        <?php } elseif($_REQUEST['action']=='viewReg'){ ?>Employee Directory 
        <div class="content_head_button" style="float:right; padding-right:10px; font-size:12px;"><a href="manage_lost_badges.php?action=view">Employee Directory</a></div>
        <div class="content_head_button" style="float:right; padding-right:10px; font-size:12px;"><a href="manage_lost_badges.php?action=view">Back</a></div> 
        ...
        <a href="manage_lost_badges.php?action=viewReg&regid=<?php echo $registration_id; ?>">Back</a></div>
        ...
        <a href="manage_lost_badges.php?action=view">Back</a></div> 
        <?php } else { ?>	Onspot Lost Badges
        <div style="float:right; padding-right:10px; font-size:12px;">
	    <div style="float:right; padding-right:10px; font-size:12px;">
        <a href="export_visitor_lost_badges.php">Download Order History</a></div>
		</div>
...
<?php 
$sql5="SELECT a.*, b.* FROM registration_master a INNER JOIN visitor_lost_badges b ON a.id = b.regId where b.payment_status='Y' group by b.regId";
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="detail_txt" >
  <tr class="orange1">
	...
	$start=($page-1)*$limit;
 	...
 	$sql="SELECT DISTINCT rm.id, rm.company_name, rm.company_pan_no, rm.mobile_no, rm.company_gstn, rm.email_id,rm.approval_status FROM registration_master rm
	inner join visitor_lost_badges vd on rm.id=vd.regId where vd.payment_status='Y'";
  ...
  <tr>
    ...
    <td align="center" valign="middle"><a href="manage_lost_badges.php?actions=companyedit&regid=<?php echo $rows['id'];?>">
    <img class="icons" src="images/view.png" title="Company Details" border="0" /></a></td>
    <td align="left" valign="middle"><a href="manage_lost_badges.php?action=orderDetails&regid=<?php echo $rows['id'];?>">Order Details</a> /  
    <a href="manage_lost_badges.php?action=viewReg&regid=<?php echo $rows['id'];?>">Employee Details</a></td>
  </tr>
  <?php
   $i++;
   } 
...
?>	
<div class="pages_1" id="page_ids">Total number of Company: <?php echo $total_application;?>
<?php echo pagination($limit,$page,'manage_lost_badges.php?action=view&page=',$total_application); //call function to show pagination?>
</div>  
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="detail_txt" >
  ...
  <tr>
    ...
    <td align="left" valign="middle"><a href="manage_lost_badges.php?action=edit&id=<?php echo $rows['visitor_id'];?>&regid=<?php echo $_REQUEST['regid'];?>">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="detail_txt" >
  <tr class="orange1">
    ...
    $sql="SELECT * FROM visitor_lost_badges where regId='$registration_id' AND payment_status='Y'"; 	
	$result=mysql_query($sql);
  ...
  <tr >
	...
	<td align="left" valign="middle"><a href="manage_lost_badges.php?action=orderHistory&orderId=<?php echo $rows['orderId'];?>&regid=<?php echo $_REQUEST['regid'];?>" style="color:#000000">VIEW</a></td>
	<td><?php echo $rows['sales_order_no'];?></td>
	<!--.....................Sales Order Create API------------>
    <?php if($rows['sap_sale_order_create_status'] == 0) { ?>
	<?php if($memberBP!=''){?>
	<td class="so" data-url="<?php echo $memberBP;?> <?php echo $_REQUEST['regid'];?> <?php echo $rows['orderId'];?>">CREATE SO</td>
	<?php } ?>
    <?php } else { ?>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="detail_txt" >
  <tr class="orange1">
    ...
    $sql="SELECT * FROM visitor_lost_badges where orderId = '$orderId' AND regId = '$registration_id'";  	 
	$result=mysql_query($sql);
	$rCount=mysql_num_rows($result);
	$sql1= $sql."  limit $start, $limit";
	$result1=mysql_query($sql1);	
...
if(($_REQUEST['action']=='add') || ($_REQUEST['action']=='edit'))
{
  ...
  if(($_REQUEST['id']!='') || ($_REQUEST['action']=='edit'))
  {
		...
		$result2 = mysql_query("SELECT * FROM visitor_directory where visitor_id='$id' and registration_id = '$registration_id'");
...
$(".so").click(function() {
	...
	if (confirm("Are you sure you want to create sales order")) {
		$.ajax({
		url: "create_lost_badges_visitor_so.php",
		method:"POST",
