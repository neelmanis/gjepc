...
$show = "IGJME 2022";
$sql1="select * from igjme_exh_reg_general_info where  uid='$registration_id' and event_for='$show' and id='$gid'";
...
if($saveUTR=="saveinfo")
{
	...
	elseif(empty($amountPaid)) { $amountPaidError = "Plz Enter Amount Paid"; }
		...
		if($event=="igjme22"){ $event_for="IGJME 2022"; } 
		$mmx = "SELECT * FROM `igjme_utr_history` WHERE registration_id='$registration_id' AND utr_number='$utr_number' AND `show`='$event_for' AND `event_selected`='$event'";
		...
		$updateUTR = "UPDATE `igjme_utr_history` SET `modified_date`=NOW(),`utr_number`='$utr_number',`amountPaid`='$amountPaid', `tdsAmount`='$tdsAmount' WHERE `registration_id`='$registration_id' AND id='$id' AND `show`='$event_for' AND `event_selected`='$event'";
			$resultUTR = $conn->query($updateUTR);
			if($resultUTR) { $utrNameSuccess = "Saved Successfully"; }
		} else {
		$insertUTR = "INSERT INTO `igjme_utr_history`(`post_date`, `registration_id`, `utr_number`,`amountPaid`, `tdsAmount`, `show`,`event_selected`, `status`) VALUES (NOW(),'$registration_id','$utr_number','$amountPaid','$tdsAmount','$event_for','$event','1')";
...
<?php
$sqlm="select * from igjme_exh_registration where uid='$registration_id' and gid='$gid' and `show`='$show'";
$querym=$conn->query($sqlm);		
$resultm=$querym->fetch_assoc();
$grandTotals = $resultm["grand_total"];
?>
<?php
$sql="select  * from igjme_exh_reg_payment_details where gid='$gid' and uid='$registration_id' AND  `show` =  '$show'";
...
if(isset($_REQUEST["action"]))
{
	...
	$sqll = "UPDATE `igjme_utr_history` SET utr_date='$utr_date[$i]',utr_approved='$utr_approve' WHERE id='$utr_id[$i]' AND `registration_id`='$registration_id'";
	$mmx= $conn->query($sqll);
	}
	...
	if($action=='UPDATE')
	{
		...
		{
			$app_query = "select payment_status,document_status from igjme_exh_reg_payment_details where payment_id='$payment_id' and gid='$gid' and uid='$registration_id' and `show`='$show'";
			...
			if($result_app)
			{
				...
				$update_app_query = "update igjme_exh_reg_payment_details set application_status='$app_status' where payment_id='$payment_id' and gid='$gid' and uid='$registration_id' and `show`='$show'";
...
<tr class="orange1">
    <td colspan="11" >Payment Details</td>
		...
		<?php 
		 $sql="select * from  igjme_exh_registration where uid='$registration_id' and gid='$gid' and `show`='$show'";
...
<tr><td>
	<table border="1" cellspacing="0" cellpadding="5" style="margin-bottom:10px;" width="100%">
			...
			<tbody>
				<?php
				$utrExist = "SELECT * FROM `igjme_utr_history` WHERE registration_id='$registration_id' AND `show`='IGJME 2022' order by `utr_date` asc";
