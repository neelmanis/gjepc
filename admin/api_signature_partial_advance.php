	    ...
	    <soapenv:Body>
			<gjep:MT_PADV_IN>
			 <SO_Advance>
				...
				<wbs_element>DE-056</wbs_element>
           ...
           // curl_setopt($ch1, CURLOPT_TIMEOUT, 10);
			...
			foreach($xml->xpath('//soapenv:Body') as $header)
			{
					...
					{	$flag=1;
						...
						$sqlx = "UPDATE `utr_history` SET `sap_push_date`=NOW(),`sap_push_admin`='$adminID',`part_sales_order_no`='$sales_order_no',`part_salesorder_status`='1' WHERE `utr_number`='$utr_no' AND `registration_id`='$registration_id' AND `show` ='$show'";
