...
{
	$sql="delete from circulars_master where id=?";
	$stmt = $conn -> prepare($sql);
	$stmt->bind_param("i", $_REQUEST['id']);
	if($stmt->execute()){
	echo "<meta http-equiv=refresh content=\"0;url=circulars.php?action=view\">";
	}
}
...
if(($_REQUEST['action']=='active') && ($_REQUEST['id']!=''))
{
	...
	$id=$_REQUEST['id'];
	$sql="update circulars_master set status='$status' where id='$id'";
	$stmt = $conn -> prepare($sql);
	if($stmt->execute()){
		echo "<meta http-equiv=refresh content=\"0;url=circulars.php?action=view\">";
	}
...
if($_REQUEST['action']=='save')
{
	...
	//---------------------------------------- uplaod  Circulars pdf  -----------------------------------------------
		...
		if(preg_match("/.php/i", $file_name)) {
    		echo "<script langauge=\"javascript\">alert(\"Sorry you have select Invalid file.\");location.href='circulars.php?action=add';</script>";
			exit;
		}
		else if($file_name!='')
		{
			if(($_FILES["upload_circulars"]["type"] == "application/PDF") || ($_FILES["upload_circulars"]["type"] == "application/pdf"))
			{
				...
				$sql="INSERT INTO circulars_master (cat_id,name,upload_circulars,set_archive,status,post_date) VALUES (?,?,?,?,?,?)";
				$stmt = $conn -> prepare($sql);
				$stmt->bind_param("isssis", $cat_id,$name,$upload_circulars,$set_archive,$status,$post_date);
				if($stmt->execute()){
				echo "<meta http-equiv=refresh content=\"0;url=circulars.php?action=view\">";
				...
				{
				echo "<script langauge=\"javascript\">alert(\"Sorry you can not upload this  file.\");location.href='circulars.php?action=add';</script>";
				return;
				}
			 }else
			 {
		...
		echo "<script langauge=\"javascript\">alert(\"Sorry Invalid Token Error.\");location.href='circulars.php?action=add';</script>";
	}
}
	...
	//------------------------------------  Update Circulars PDF ----------------------------------------------------
		...
		else if($file_name != '')
		{
		  //Unlink the previuos image
		   $smx = "select upload_circulars from circulars_master where id=?";
		   ...
		   unlink($filename);
			...
			if(($_FILES["upload_circulars"]["type"] == "application/PDF") || ($_FILES["upload_circulars"]["type"] == "application/pdf"))
			{
				...
				{
					$upload_circulars = $temp_code.'_'.$file_name;
					$sql="update circulars_master set upload_circulars='$upload_circulars' where id='$id'";
					$stmt = $conn -> prepare($sql);
					$stmt->execute();
				}
				else
				{
					echo "<script langauge=\"javascript\">alert(\"Sorry you can not upload this file.\");location.href='circulars.php?action=edit&id=$id';</script>";
					return;
				}
			}else
			 {
			 echo "<script langauge=\"javascript\">alert(\"Sorry you have select Invalid file.\");location.href='circulars.php?action=edit&id=$id';</script>";
			 }	
		}	
		
	$sql="update circulars_master set cat_id=?, name=? ,order_no=?,set_archive=?,post_date=? where id=?";
	$stmt = $conn -> prepare($sql);
	$stmt->bind_param("issssi", $cat_id,$name,$order_no,$set_archive,$post_date,$id);
...
<div class="breadcome_wrap">
	<div class="breadcome"><a href="admin.php">Home</a> > Manage GJEPC Circulars</div>
</div>

<div id="main">
	<div class="content">
    	<div class="content_head"><a href="circulars.php?action=add"><div class="content_head_button">Add GJEPC Circulars</div></a> <a href="circulars.php?action=view"><div class="content_head_button">View GJEPC Circulars</div></a> </div>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="detail_txt" >
    ...
    $i=1;
	$sqlx1 = "SELECT * FROM circulars_master where 1".$attach." ";
    ...
    ?>  
 	<tr <?php if($i%2==0){echo "bgcolor='#CCCCCC'";}?>>
        ...
        <td><?php if($row['status'] == 1) { ?> <a style="text-decoration:none;" href="circulars.php?id=<?php echo $row['id']; ?>&status=0&action=active" onClick="return(window.confirm('Are you sure you want to Deactivate.'));"><img src="images/inactive.png" border="0" title="Inactive"/></a><?php } else { ?><a style="text-decoration:none;" href="circulars.php?id=<?php echo $row['id']; ?>&status=1&action=active" onClick="return(window.confirm('Are you sure you want to Activate.'));"><img src="images/active.png" border="0" title="Active"/></a><?php } ?></td>
        
        <td ><a href="circulars.php?action=edit&id=<?php echo $row['id']?>"><img src="images/edit.png" title="Edit" border="0" /></a></td>
        <td ><a href="circulars.php?action=del&id=<?php echo $row['id']?>" onClick="return(window.confirm('Are you sure you want to delete?'));"><img src="images/delete.png" title="Delete" border="0" /></td>
 	</tr>
...
if(($_REQUEST['action']=='add') || ($_REQUEST['action']=='edit'))
{
  ...
  {
		$action='update';
		$sql3 = "SELECT * FROM circulars_master where id=?";
...
<table width="100%" border="0" cellspacing="0" cellpadding="0"  >
  	<tr class="orange1">
    <td colspan="2"> &nbsp;Add New Circulars</td>
    </tr>
  	<tr>
