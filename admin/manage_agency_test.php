...
if(($_REQUEST['action']=='vis_del') && ($_REQUEST['id']!='') && ($_REQUEST['agency_id']!=''))
{
   ...
   $deleteFromLab="delete from visitor_lab_info where visitor_id='".$_REQUEST[id]."' AND registration_id='".$_REQUEST[agency_id]."' AND 	category_for='CONTR'";	
	...
	echo"<meta http-equiv=refresh content=\"0;url=manage_agency_test.php?action=viewVisitors&agencyId=".$_REQUEST['agency_id']."\">";
}
...
if(($_REQUEST['action']=='active') && ($_REQUEST['id']!=''))
{
	...
	if($conn->query($sql)) {
	echo"<meta http-equiv=refresh content=\"0;url=manage_agency_test.php?action=view\">";
	} else {
		...
		if($visitors !=""){
			foreach ($visitors as $key => $visitor) {
                   ...
                   $countUniqueIdentifier = $checkUniqueIdentifier->num_rows;
					   ...
					   $checkRecord = $conn->query("SELECT * FROM globalExhibition WHERE  `registration_id`='$agency_id' AND `visitor_id`='$visitor'  AND `mobile`='$mobile' ");
                  $checkRecordNum = $checkRecord->num_rows;
	                        if($checkRecordNum>0){
	                        	$checkRecordRow = $checkRecord->fetch_assoc();
	                        	$global_dose1_status = $checkRecordRow['dose1_status'] ; 
	                        	$global_dose2_status = $checkRecordRow['dose2_status'] ;
	                        	if($global_dose1_status =="Y" || $global_dose2_status =="Y"){
                              	$global_status = "Y";
	                        	}else{
                              	$global_status = "P";
	                        	}

                              $updateGlobal = "UPDATE  globalExhibition  SET `modified_date`='$modified_at',`status`='$global_status' WHERE  `registration_id`='$agency_id' AND `visitor_id`='$visitor'  AND `pan_no`='$pan_no' AND `participant_type`= '$participant_type' ";
								...
								if($updateGlobalResult){
									...
									echo "<script langauge=\"javascript\">alert(\" Updating Global table failed. Error:".$conn->error."\");location.href='manage_agency_test.php?action=viewVisitors&agencyId=".$agency_id."';</script>";
								}
	                        }else{
                                 $insertGlobal = "INSERT INTO globalExhibition  SET `post_date`='$created_at',`uniqueIdentifier`='$uniqueIdentifier',`registration_id`='$agency_id',`visitor_id`='$visitor',`fname`='$fname',`lname`='$lname',`mobile`='$mobile',`pan_no`='$pan_no',`designation`='$designation',`company`='$company',`photo_url`='$photo_url',`participant_type`='$participant_type',`covid_report_status`='$covid_report_status',`days_allow`='$days_allow',`agency_category`='$category',`committee`='$committee',`status`='P'";
						        $insertGlobalResult = $conn->query($insertGlobal);
								if($insertGlobalResult){
									$sqlUpdate = "UPDATE visitor_agency_registration SET `person_status`='$status',`adminId`='$adminID' WHERE `agency_id`='$agency_id' AND `id`='$visitor'";
									$result = $conn->query($sqlUpdate);
								}else{
									echo "<script langauge=\"javascript\">alert(\" Inserting Global table failed. Error:".$conn->error."\");location.href='manage_agency_test.php?action=viewVisitors&agencyId=".$agency_id."';</script>";
			...
			echo "<script langauge=\"javascript\">alert(\" Submitted successfully\");location.href='manage_agency_test.php?action=viewVisitors&agencyId=".$agency_id."';</script>";

		}else{
			echo "<script langauge=\"javascript\">alert(\" Select Visitors\");location.href='manage_agency_test.php?action=viewVisitors&agencyId=".$agency_id."';</script>";
		}
	}else{
		echo "<script langauge=\"javascript\">alert(\" Select approval status\");</script>";
	}
...
if ($_REQUEST['action']=='save')
{
	...
	if($cnt > 0)
	{
		echo "<script langauge=\"javascript\">alert(\"Email ID already in use\");location.href='manage_agency_test.php?action=view';</script>";
	}
	else
	{
...
<div id="main">
	<div class="content">
    	<div class="content_head"><a href="manage_agency_test.php?action=add"><div class="content_head_button">Add Agency</div></a> <a href="manage_agency_test.php?action=view"><div class="content_head_button">View Agency</div></a> <a href="visitor-vendor-category.php?action=view" target="_blank"><div class="content_head_button">Add Category</div></a></div>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="detail_txt" >
    ...
    ?>  
 	...
 	<tr <?php if($i%2==0){echo "bgcolor='#CCCCCC'";}?>>
        ...
        <td><a href="manage_agency_test.php?action=viewVisitors&agencyId=<?php echo $row['id']?>">View</a></td>
        <td>
		<?php if($row['status'] == '1') 
        { 
        ...
        <td><?php if($row['status'] == 1) { ?> <a style="text-decoration:none;" href="manage_agency_test.php?id=<?php echo $row['id']; ?>&status=0&action=active" onClick="return(window.confirm('Are you sure you want to Deactivate.'));"><img src="images/inactive.png" border="0" title="Inactive"/></a><?php } else { ?><a style="text-decoration:none;" href="manage_agency_test.php?id=<?php echo $row['id']; ?>&status=1&action=active" onClick="return(window.confirm('Are you sure you want to Activate.'));"><img src="images/active.png" border="0" title="Active"/></a><?php } ?></td>
        
        <td ><a href="manage_agency_test.php?action=edit&id=<?php echo $row['id']?>"><img src="images/edit.png" title="Edit" border="0" /></a></td>
        <!-- <td ><a href="manage_agency_test.php?action=del&id=<?php echo $row['id']?>" onClick="return(window.confirm('Are you sure you want to delete?'));"><img src="images/delete.png" title="Delete" border="0" /></td> -->
 	</tr>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="detail_txt" >
    ...
    ?>  
 	<tr <?php if($i%2==0){echo "bgcolor='#CCCCCC'";}?>>
        ...
        <td><a href="manage_agency_test.php?action=viewDetails&agencyId=<?php echo $agency_id;?>&visitorId=<?php echo $row['id']; ?>"> View Details</a></td>
        <td>
         	<?php
       ...
       <td >
       	<?php if($row['person_status'] !=="Y"){?>
            <a href="manage_agency_test.php?action=vis_del&id=<?php echo $row['id']?>&agency_id=<?php echo $row['agency_id'];?>" onClick="return(window.confirm('Are you sure you want to delete?'));"><img src="images/delete.png" title="Delete" border="0" />
       <?php 	}?>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
    ...
    <td><label><input type="password" name="password" id="password" rows="5" class="input_txt" value="<?php echo $password_text; ?>"></label></td>
    </tr>
...
?>
 <div class="content_details1">
   <table width="100%" border="0" cellspacing="5" cellpadding="5"  >
     <tr class="orange1">
       <td colspan="2">&nbsp;View Details &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <a href="manage_agency_test.php?action=viewVisitors&agencyId=<?php echo $agencyId?>" align="right">Back</a></td>
     </tr>  
     <tr>
