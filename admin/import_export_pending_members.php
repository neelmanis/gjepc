...
<td colspan="7" style="color:blue;font-weight:600; text-align:center;font-size:24px">Pending Members Data Gem And Jewellery '.$financial_year.'-'.$quarter_year.'</td>          
            </tr>
<tr>
<td>Company Name</td>
<td>Company BP</td>
...
<td>Region</td>
    ...
    $sql="SELECT a.registration_id FROM `approval_master` a JOIN challan_master b JOIN information_master c WHERE a.eligible_for_renewal ='Y' AND b.challan_financial_year='2020' AND a.registration_id=b.registration_id AND a.registration_id=c.registration_id AND c.region_id = '$region' AND a.registration_id  NOT IN ( SELECT registration_id FROM statistics WHERE financial_year='".$financial_year."' AND quarter_year='".$quarter_year."' group by registration_id order by region )";
