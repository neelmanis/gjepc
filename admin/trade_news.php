...
{
	$sql="delete from trade_news_master where id='$_REQUEST[id]'";
	
	if (!mysql_query($sql,$dbconn))
	{
		die('Error: ' . mysql_error());
	}
	echo"<meta http-equiv=refresh content=\"0;url=trade_news.php?action=view\">";
}
...
if (($_REQUEST['action']=='active') && ($_REQUEST['id']!=''))
{
		...
		$id=$_REQUEST['id'];
		$sql="update trade_news_master set status='$status',adminId='$adminID',modified_date=NOW() where id='$id'";
		if (!mysql_query($sql,$dbconn))
		{
			die('Error: ' . mysql_error());
		}
...
if ($_REQUEST['action']=='save')
{
	...
	$order_no=mysql_real_escape_string($_REQUEST['order_no']);
		...
		$sql="INSERT INTO trade_news_master (name,short_desc,long_desc,order_no,status,post_date,adminId) VALUES ('$name','$short_desc','$long_desc','$order_no','1','$post_date','$adminID')";
...
if (($_REQUEST['action']=='update')&&($_REQUEST['id']!=''))
{
	...
	$sql="update trade_news_master set name='$name',short_desc='$short_desc',long_desc='$long_desc',order_no='$order_no',modified_date=NOW(),adminId='$adminID' where id='$id'";
...
<div class="breadcome_wrap">
	<div class="breadcome"><a href="admin.php">Home</a> > Manage Trade News</div>
</div>

<div id="main">
	<div class="content">
    	<div class="content_head"><a href="trade_news.php?action=add">
    	<div class="content_head_button">Add Trade News</div>
    	</a> <a href="trade_news.php?action=view">
    	<div class="content_head_button">View Trade News</div>
    	</a> </div>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="detail_txt" >
    ...
    $i=1;
	$result = mysql_query("SELECT * FROM trade_news_master where 1".$attach." ");
    $rCount=0;
    ...
    ?>  
 	<tr <?php if($i%2==0){echo "bgcolor='#CCCCCC'";}?>>
        ...
        <td><?php if($row['status'] == 1) { ?> <a style="text-decoration:none;" href="trade_news.php?id=<?php echo $row['id']; ?>&status=0&action=active" onClick="return(window.confirm('Are you sure you want to Deactivate.'));"><img src="images/inactive.png" border="0" title="Inactive"/></a><?php } else { ?><a style="text-decoration:none;" href="trade_news.php?id=<?php echo $row['id']; ?>&status=1&action=active" onClick="return(window.confirm('Are you sure you want to Activate.'));"><img src="images/active.png" border="0" title="Active"/></a><?php } ?></td>
        
        <td ><a href="trade_news.php?action=edit&id=<?php echo $row['id']?>"><img src="images/edit.png" title="Edit" border="0" /></a></td>
        <td ><a href="trade_news.php?action=del&id=<?php echo $row['id']?>" onClick="return(window.confirm('Are you sure you want to delete?'));"><img src="images/delete.png" title="Delete" border="0" /></td>
 	</tr>
...
if(($_REQUEST['action']=='add') || ($_REQUEST['action']=='edit'))
{
  ...
  {
		$action='update';
		$result2 = mysql_query("SELECT *  FROM trade_news_master  where id='$_REQUEST[id]'");
...
<table width="100%" border="0" cellspacing="0" cellpadding="0"  >
  	<tr class="orange1">
    <td colspan="2"> &nbsp;Add New Trade News</td>
    </tr>
    <tr>
