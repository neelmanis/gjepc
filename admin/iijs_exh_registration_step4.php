...
//include('fetch_allotment_order.php'); /* Space order status API */
...
if(($_REQUEST['action']=='del') && ($_REQUEST['id']!=''))
{
	$ssx = "INSERT INTO utr_history_log
		SELECT * FROM  utr_history WHERE id='$_REQUEST[utr_id]' AND `registration_id`='$_REQUEST[registration_id]' AND `show`='IIJS PREMIERE 2022' AND payment_made_for='ALLOTMENT' AND source='admin'";
	$queryData = $conn->query($ssx);
	if($queryData){
	$sql = "delete from utr_history where id='$_REQUEST[utr_id]' AND registration_id='$_REQUEST[registration_id]' AND `show`='IIJS PREMIERE 2022' AND payment_made_for='ALLOTMENT' AND source='admin'";
	$result = $conn->query($sql);
	}
	if(!$result){die($conn->error);}
	echo"<meta http-equiv=refresh content=\"0;url=iijs_exh_registration_step4.php?id=$gid&registration_id=$registration_id\">";
}
?>
<?php
...
if($saveUTR=="saveinfo")
{
	...
	elseif(empty($amountPaid)) { $amountPaidError = "Plz Enter Amount Paid"; }
		...
		if($event=="iijs") { $event_for="IIJS PREMIERE 2022"; }
...
<form name="form1" action="iijs_exh_registration_step4.php?id=<?php echo $gid; ?>&registration_id=<?php echo $registration_id; ?>&action=update" method="post" onsubmit="return validation()"> 
<?php 
...
<tr class="orange1"><td colspan="11">Online Payment Details IIJS PREMIERE 2022</td></tr>
<tr><td colspan="2">
	<table border="1" cellspacing="0" cellpadding="5" style="margin-bottom:10px;" width="100%">
			...
			$utrExist = "SELECT * FROM `utr_history` WHERE registration_id='$registration_id' AND gcode='$gid' AND `show`='IIJS PREMIERE 2022' AND payment_made_for='SPACE'";
			...
			<?php }	?>
				...
				<?php } ?>
				<td align="center"><a  href="iijs_exh_registration_step4.php?action=del&utr_id=<?php echo $id?>&id=<?php echo $gid?>&registration_id=<?php echo $registration_id?>" onClick="return(window.confirm('Are you sure you want to delete?'));">Delete</a></td>
				</tr>
				<?php  //$CheckFirstEntry++; ?>
				<?php $i++;} */	?>
			</tbody>
	...
	<tr class="orange1"><td colspan="11">Online Allotment Payment Details IIJS PREMIERE 2022</td></tr>
<tr><td colspan="2">
	<table border="1" cellspacing="0" cellpadding="5" style="margin-bottom:10px;" width="100%">
			<tr>
				<th align="center">Date</th>
			...
			<!-------------------------- Start Partial --------------------------------------------->
			<?php
			$utrExistSuccess = "SELECT * FROM `utr_history` WHERE registration_id='$registration_id' AND gcode='$gid' AND `show`='IIJS PREMIERE 2022' AND payment_made_for='SPACE' AND payment_status='captured'";
		...
		//	$numSuccess = $existResultSuccess->num_rows;
			...
			$utrExistAll = "SELECT * FROM `utr_history` WHERE registration_id='$registration_id' AND `show`='IIJS PREMIERE 2022' AND payment_made_for='ALLOTMENT' AND comment='ALLOTMENT' ";
...
$utrExist1 = "SELECT sum(amountPaid) as `amountPaid`,sum(tdsAmount) as `tdsAmount` FROM `utr_history` WHERE registration_id='$registration_id' AND `show`='IIJS PREMIERE 2022' AND payment_status='captured'";
...
{
	$query_sel1 = "select cheque_tds_amount from exh_reg_payment_details where uid='$registration_id' AND `show`='IIJS PREMIERE 2022' AND event_selected='iijs' limit 0,1";
