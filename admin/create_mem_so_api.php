...
{ //echo "<pre>"; print_r($_POST);exit;
	...
	/* Micro SO Start */
	if($member=='MICRO'  && ($renew_check=='N' || $renew_check=='Y')) // New Micro Member 
	{ //echo 'Micro';  exit;
	//$soapSalesUrl = "https://webdisp.gjepcindia.com:44303/XISOAPAdapter/MessageServlet?channel=:BC_SOAD:CC_SOAD_Sender"; // DEV
	...
	$result1 = $conn ->query("SELECT export_fob_value,import_cif_value,ReferenceNo,sap_push_date,challan_region_name,total_payable,admission_fees,gjepc_account_no,membership_fees,Transaction_Date FROM `challan_master` WHERE `challan_financial_year`='$financial_year' and registration_id='$registration_id'");
	...
	//$transaction_date = date("Ymd", strtotime("2020-03-11"));
           ...
           // curl_setopt($ch1, CURLOPT_TIMEOUT, 10);
			...
			foreach($xml->xpath('//soapenv:Body') as $header)
			{
					...
					{	$flag=1;
						...
						$result1 = $conn ->query("UPDATE `challan_master` SET `sales_order_no`='$sales_order_no' WHERE registration_id='$registration_id' and challan_financial_year='$financial_year'");
						$result2 = $conn ->query("UPDATE `approval_master` SET `sap_push_date`=NOW(),`sap_push_admin`='$adminID',`sap_sale_order_create_status`='1' WHERE registration_id='$registration_id'");
					}
			}
			echo $flag;
	}
	...
	if($renew_check=='N' && $member=='NORMAL') // New Member 
	{ //echo '--- New'; exit;
	$soapSalesUrl = "https://webdisp.gjepcindia.com:44306/XISOAPAdapter/MessageServlet?channel=:BC_SOAD_PRD:CC_SOAD_Sender"; // Live
	$soapUser = "pi_admin";  //  username
	$soapPassword = "Deloitte@123"; // password
	...
	if($renew_check=='Y' && $member=='NORMAL') // Renewal
	{ //echo '++++ Renewal'; exit;
	...
	$bpInfoResult = $conn ->query($bpinfo);
	$bpRows = $bpInfoResult->fetch_assoc();
	$BP_roles = getroles($bpRows['type_of_address'],$conn); /* Pass Parent BP Number From communication_address_master Table*/
	$p_bp_number = trim($bpRows['c_bp_number']); /* Pass Parent BP Number From communication_address_master Table*/
	$city = trim($bpRows['city']);
	$Date = date('Ymd');
   ...
   <soapenv:Body>
	...
	$result = $conn ->query("SELECT `export_sales_to_foreign_tourists`, `export_synthetic_stones`, `export_costume_jewellery`, `export_other_precious_metal_jewellery`, `export_pearls`, `export_coloured_gemstones`, `export_gold_jewellery`, `export_studded_gold_jewellery`, `export_silver_jewellery`, `export_studded_silver_jewellery`, `export_rough_diamonds`, `export_cut_polished_diamonds`, `export_rough_lgd`,`export_cut_polished_lgd`,`export_other_items`,`import_findings_mountings`, `import_false_pearls`, `import_rough_imitation_stones`, `import_silver`, `import_raw_pearls`, `import_cut_polished_gemstones`, `import_rough_gemstones`, `import_gold`, `import_cut_polished_diamonds`, `import_rough_diamonds`, `import_synthetic_stones`, `import_gold_jewellery`, `import_silver_jewellery`,`import_rough_lgd`,`import_cut_polished_lgd`, `import_other_items` FROM `challan_master` WHERE `challan_financial_year`='$financial_year' and registration_id='$registration_id'");
