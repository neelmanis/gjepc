...
if(($_REQUEST['action']=='del') && ($_REQUEST['id']!=''))
{
	...
	if($stmtd->execute()){	echo"<meta http-equiv=refresh content=\"0;url=news_new.php?action=view\">"; }
}
...
if(($_REQUEST['action']=='active') && ($_REQUEST['id']!=''))
{
	...
	if($stmt->execute()){	echo"<meta http-equiv=refresh content=\"0;url=news_new.php?action=view\">"; }
}
...
if($_REQUEST['action']=='save')
{
	...
	if(preg_match("/.php/i", $name)) {
    		echo "<script langauge=\"javascript\">alert(\"Sorry you have select Invalid file.\");location.href='news_new.php?action=add';</script>";
			exit;
	}
	else if($name !='')
	{
	...
	if($stmt->execute()){ echo"<meta http-equiv=refresh content=\"0;url=news_new.php?action=view\">";  } else {  die ($conn->error); }
...
if(isset($_SESSION['token']) && $_POST['token'] === $_SESSION['token']) {
	...
	else if($name !='')
	{
			if(($_FILES["p_image"]["type"] == "image/jpeg") || ($_FILES["p_image"]["type"] == "image/jpg") ||($_FILES["p_image"]["type"] == "image/png"))
			{
				...
				echo "<script langauge=\"javascript\">alert(\"Sorry you can not upload this file.\");location.href='news_new.php?action=edit&id=$id';</script>";
				return;
				}
			} else	{
...
//	echo $sql; exit;
	$stmt = $conn -> prepare($sql);
	$stmt->execute();
	echo"<meta http-equiv=refresh content=\"0;url=news_new.php?action=view\">";
    } else {
		echo "<script langauge=\"javascript\">alert(\"Invalid Token Error.\");</script>";
	}
}
...
<div id="main">
	<div class="content">
    	<div class="content_head"><a href="news_new.php?action=add">
    	<div class="content_head_button">Add News</div>
    	</a> <a href="news_new.php?action=view">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="detail_txt" >
    ...
    ?>  
 	<tr <?php if($i%2==0){echo "bgcolor='#CCCCCC'";}?>>
        ...
        <td><?php if($row['status'] == 1) { ?> <a style="text-decoration:none;" href="news_new.php?id=<?php echo $row['id']; ?>&status=0&action=active" onClick="return(window.confirm('Are you sure you want to Deactivate.'));"><img src="images/inactive.png" border="0" title="Inactive"/></a><?php } else { ?><a style="text-decoration:none;" href="news_new.php?id=<?php echo $row['id']; ?>&status=1&action=active" onClick="return(window.confirm('Are you sure you want to Activate.'));"><img src="images/active.png" border="0" title="Active"/></a><?php } ?></td>
        
        <td ><a href="news_new.php?action=edit&id=<?php echo $row['id']?>"><img src="images/edit.png" title="Edit" border="0" /></a></td>
        <td ><a href="news_new.php?action=del&id=<?php echo $row['id']?>" onClick="return(window.confirm('Are you sure you want to delete?'));"><img src="images/delete.png" title="Delete" border="0" /></td>
 	</tr>
