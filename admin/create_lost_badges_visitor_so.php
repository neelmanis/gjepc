...
{	//echo "><pre>"; print_r($_POST);exit;
	...
	$query="SELECT * FROM `visitor_lost_badges` WHERE regId='".$registration_id."' AND orderId='".$order_id."'";
	$getChallanResult = mysql_query($query);
	$challanResult = mysql_fetch_array($getChallanResult);
	
	$amount = $challanResult['total_payable'];
	$payment_mode = $challanResult['payment_mode'];
	$visitorBP_id = trim($challanResult['bp_number']);
	$event_material = getSap_material_no(str_replace(' ', '', $challanResult['show']));
           ...
           // curl_setopt($ch1, CURLOPT_TIMEOUT, 10);
			...
			foreach($xml->xpath('//soapenv:Body') as $header)
			{
					...
					{	$flag=1;
						...
						$sqlx = "UPDATE `visitor_lost_badges` SET `sap_push_date`=NOW(),`sap_push_admin`='$adminID',`sales_order_no`='$sales_order_no',`sap_sale_order_create_status`='1' WHERE orderId='".$order_id."' AND regId='".$registration_id."'";
