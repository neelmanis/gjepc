...
{
		$get = "SELECT * FROM exh_reg_payment_details where `show`='$show_value' AND (sales_order_no='tem categor' || sales_order_no='rror in doc' || sales_order_no='old-to part')";
		$result_query = $conn ->query($get);
		$getCount = $result_query->num_rows;
		
		$get2 = "SELECT * FROM `utr_history` where `show` ='$show_value' AND event_selected='$shortcode' AND (first_sales_order_no='tem categor' || first_sales_order_no='rror in doc' || first_sales_order_no='old-to part')";
	...
	//	$sql="UPDATE `visitor_order_detail` SET `sap_sale_order_create_status` = '0', `sales_order_no` ='' WHERE `sales_order_no` ='old-to part' AND `sap_sale_order_create_status` = '1'";
		$sql="UPDATE `exh_reg_payment_details` SET `sap_sale_order_create_status` = '0', `sales_order_no` =  '' WHERE `show`='$shortcode' AND (sales_order_no='tem categor' || sales_order_no='rror in doc' || sales_order_no='old-to part') AND `sap_sale_order_create_status` = '1'";
		$result = $conn ->query($sql);  
		
		$sql2="UPDATE `utr_history` SET `sap_so_status` = '0', `first_sales_order_no` =  '' WHERE `show`='$show_value' AND event_selected='$show_value' AND (first_sales_order_no='tem categor' || first_sales_order_no='rror in doc' || first_sales_order_no='old-to part') AND `sap_so_status` = '1'";
		$result2 = $conn ->query($sql2); 
		 
        if(!$result) die ($conn->error);
		echo "<meta http-equiv=refresh content=\"0;url=exhibitor_space_registration.php?shortcode=$shortcode\">";
		} else { 
...
<title><?php echo $show_name; ?> Exhibitor List</title>
...
<div class="breadcome"><a href="admin.php">Home</a> ><?php echo $show_name;?></div>
</div>

<div id="main">
	<div class="content">
    
    	<div class="content_head"><a href="exhibitor_space_registration.php?shortcode=<?php echo $shortcode; ?>">
    	<div class="content_head_button"><?php echo $show_name;?> Exhibitor List</div></a>
		...
		<a href="export_utr_exhibitor_space_registration.php?shortcode=<?php echo $shortcode; ?>">&nbsp;Download Payment Data</a> <a href="export_approve_exhibitor_space_registration.php?shortcode=<?php echo $shortcode; ?>">&nbsp;Download Data</a></div>
		<?php
		if($_SESSION['curruser_login_id']=='28' || $_SESSION['curruser_login_id']=='1'){ ?>
		<a href="exhibitor_space_registration.php?action=old_to_part" onClick="return(window.confirm('Are you sure you want to Clear Old to Part Data'));" >Clear SAP Error</a>	
		<?php }	?>
		</div>
...
{
	 $sql5="select a.id,a.company_name,a.contact_person,a.region,a.created_date,b.last_yr_participant,b.section,b.selected_area,b.selected_premium_type,c.payment_status,c.document_status,c.application_status from exh_reg_general_info a inner join exh_registration b on a.id=b.gid inner join exh_reg_payment_details c on a.id=c.gid where a.event_for='$show_value'";
}
else
{
/*	if(preg_match('/O/',$_SESSION['curruser_admin_access']))
	...
	b.section,b.selected_area,b.selected_premium_type,c.payment_status,c.document_status,c.application_status 
	from exh_reg_general_info a inner join exh_registration b on a.id=b.gid inner join exh_reg_payment_details c on a.id=c.gid
	where a.event_for='$show_value' and  a.region='".$_SESSION["curruser_region_id"]."' ";
	}
}
...
from exh_reg_general_info a inner join exh_registration b on a.id=b.gid inner join exh_reg_payment_details c on a.id=c.gid left join utr_history u on a.uid=u.registration_id and a.event_for=u.`show` where a.event_for='$show_value'";
}
else
{
/*	if(preg_match('/O/',$_SESSION['curruser_admin_access']))
	...
	b.section,b.selected_area,b.selected_premium_type,c.payment_status,c.document_status,c.application_status 
	from exh_reg_general_info a inner join exh_registration b on a.id=b.gid inner join exh_reg_payment_details c on a.id=c.gid left join utr_history u on a.uid=u.registration_id where a.event_for='$show_value' AND u.event_selected='signature'";
	} else { */
	if($_SESSION['curruser_login_id']==160)
	{
	$sql="select a.id,a.uid,a.company_name,a.contact_person,a.region,a.billing_address_id,a.get_billing_bp_number,a.created_date,
...
from exh_reg_general_info a inner join exh_registration b on a.id=b.gid inner join exh_reg_payment_details c on a.id=c.gid left join utr_history u on a.uid=u.registration_id and a.event_for=u.`show` where a.event_for='$show_value'";	
	...
	from exh_reg_general_info a inner join exh_registration b on a.id=b.gid inner join exh_reg_payment_details c on a.id=c.gid where a.event_for='$show_value' and  a.region='".$_SESSION["curruser_region_id"]."'";
	}
}
...
//	$schk_membership="SELECT * FROM `approval_master`  WHERE 1 and `registration_id`='".$rows['uid']."' and issue_membership_certificate_expire_status='Y'";
  ...
  <tr >
	...
	if(preg_match('/O/',$_SESSION['curruser_admin_access']))
	{ } else { ?>
	<a href="exhibitor_space_registration_step1.php?id=<?php echo $rows['id'];?>&registration_id=<?php echo $rows['uid'];?>&shortcode=<?php echo $shortcode; ?>"><img src="images/edit1.png" border="0" /></a> 
	<?php } ?>
	<a href="exhibitor_space_registration_step4.php?gid=<?php echo $rows['id'];?>&registration_id=<?php echo $rows['uid'];?>&bp_number=<?php echo $billingBP;?>&shortcode=<?php echo $shortcode; ?>">A/D</a> 
	<?php if($_SESSION['curruser_role']=="Super Admin") { ?>
    <a href="delete_exh_registration.php?id=<?php echo $rows['id'];?>&registration_id=<?php echo $rows['uid'];?>&shortcode=<?php echo $shortcode; ?>"> Delete</a>
	<?php } ?> </td>
...
<?php echo pagination($limit,$page,'exhibitor_space_registration.php?shortcode='.$shortcode.'&page=',$rCount); //call function to show pagination?>
</div>        
</div>
</div>
