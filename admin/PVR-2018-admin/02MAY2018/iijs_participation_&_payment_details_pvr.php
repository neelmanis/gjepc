...
$(document).ready(function(){
	$('input[name="payment_mode"]').click(function(){
		...
		if($(this).val()=="Cheque or DD") 
			$('#bank_details').show("slow");
		else 
			$('#bank_details').hide('slow');
	});
	if($('#DD').attr("checked"))
		$('#bank_details').show("fast");
	else
		$('#bank_details').hide('fast');
...
<tr>
    ...
    <div>
       ...
       <select name="payment_made_for" id="payment_made_for" onchange="fees_calculate(this.value)" class="selectField" >
		...
		<option value="COMBO" <?php if($payment_made_for=="COMBO") echo "selected"; ?>>Combo (IIJS 2017 + Signature IIJS 2018)</option>
		<option value="iijs" <?php if($payment_made_for=="iijs") echo "selected"; ?>>IIJS 2017 Only</option>
        <option value="igjme_signature" <?php if($payment_made_for=="igjme_signature") echo "selected"; ?>>Signature IIJS 2018 + IGJME 2018</option>
		<option value="igjme" <?php if($payment_made_for=="igjme") echo "selected"; ?>>Only IGJME 2018 (Complementary)</option>
        </select>
    </div>
      <div class="clear" style="margin-bottom:8px;"></div>
    <div>
...
<!--			 <span>Cash</span>
            ...
            <input name="payment_mode" type="radio" value="2" class="radio" <?php if(preg_match('/2/',$payment_mode)) echo "checked"; ?> />
...
<p class="bottomSpace">Cheque / Demand Draft should be drawn in the favour of "The Gem & Jewellery Export Promotion Council"</p>
