...
header("Location: bulk_sms_panel.php?action=panel");
}
if (isset($_POST["action"]) && $_POST["action"]=="send_messsage") {
  ...
  if($_REQUEST['main_category'] ==""){
    ...
    if(preg_match("/.php/i", $name)) {
     ...
     /*SAVE MESSAGE RECORD*/
    $save_message = "INSERT INTO whatsapp_messages_history  SET `department`='".$_SESSION["msg_dept"]."',`category`='".$_SESSION["main_category"]."',`media_type`='$msg_type',`title`='$msg_title',`description`='$msg_description',`attatchment`='$file'"; 
    ...
    $insertCust = "INSERT INTO whatsapp_temp_data SET `person`= '',`company`='',`mobile`='$cust->value',`variable_name`='',`type`='$msg_type',`title`='$msg_title',`description`='$msg_description',`attatchment`='$file'";
          ...
          while (($getData = fgetcsv($file_csv)))
           {
            ...
            $sql_import = "INSERT INTO whatsapp_temp_data SET `person`= '$person_name',`company`='$company_name',`mobile`='$mobile',`variable_name`='$msg_variable',`type`='$msg_type',`title`='$msg_title',`description`='$msg_description',`attatchment`='$file'";
...
$counter_csv ++;
    ...
    $successMsg = "Message has been sent successfully.";
      ...
      if($msg_variable !=="none"){
        $name_tr =str_replace(" ","%20",$test_variable);
        $msg_description = str_replace_variable("VARIABLE",trim($name_tr),trim($msg_description));
      }else{
        $msg_description = trim($msg_description);
      }
      if($file !=""){
        $attatchment = "https://gjepc.org/admin/whatsappAttatchments/".$file;

      }else{
         $attatchment = "";
      }
      
      $optIN =  sendOPTIN($test_number);
      $isSend = sendMediaMsg($msg_type,$test_number,$attatchment,$msg_description,$msg_title);
      // print_r($isSend);exit;
      if($isSend['response']['status']=="success")
      {
        $successMsg = "Template is ok. Message has been sent successfully.";
      } else { 
        $errorMsg =  $isSend['response']['details'];
      }
    ...
    $sql.="  b.region_id='".$memRegionVal."' ";
    }else{
    $sql.=" or b.region_id='".$memRegionVal."' ";
    }
    ...
    if($rCount>0){
      ...
      $insert = "INSERT INTO whatsapp_temp_data SET `person`= '$name',`company`='$company',`mobile`='$mobile',`variable_name`='$msg_variable',`type`='$msg_type',`title`='$msg_title',`description`='$msg_description',`attatchment`='$file',`platform`='G'";
...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
    ...
    function checkdata()
    {
       if (confirm("Confirm before sending  title(if required) and description must be encoded format otherwise message will not send")) {
  ...
  <body>
    ...
    <div id="main">
      ...
      <td> <select class="input_txt" name="msg_variable">
  ...
  <tr>
    ...
    <td> <input type='text' class="input_txt" name="msg_title" id="msg_title" value="<?php echo $_SESSION['msg_title'];?>" style="width:90%">
      ...
      <a href='javascript:void(0);' id="encode_title" class="input_submit content_head_button" style="display: inline-block; color: #fff;text-decoration: none;">Encode Title</a>
&nbsp;&nbsp;&nbsp;&nbsp;<a href='javascript:void(0);' id="decode_title" class="input_submit" style="display: inline-block; color: #fff;text-decoration: none;">Decode Title</a>
    </td>
  </tr>
</tr>
<td valign="top" class="content_txt">Description</td>
<td> <textarea  rows="14" class="input_txt" name="msg_description" id="msg_description" style="width: 90%"><?php echo $_SESSION['msg_description'];?></textarea><br>
<a href='javascript:void(0);' id="encode_string" class="input_submit content_head_button" style="display: inline-block; color: #fff;text-decoration: none;">Encode Description</a>
&nbsp;&nbsp;&nbsp;&nbsp;<a href='javascript:void(0);' id="decode_string" class="input_submit" style="display: inline-block; color: #fff;text-decoration: none;">Decode Description</a>
<br>
  <i>Replace message variable with  VARIABLE </i>
</td>
</tr>
<tr>
<td valign="top" class="content_txt">Attatchment</td>
<td> <input type='file' class="input_txt" name="msg_attatchment" ><br>
...
<td colspan="2" style="padding: 20px">
  ...
  <input type="hidden" name="action" id="action" value="send_messsage" />
   <a href="bulk_sms_panel.php?action=Reset"><div class="content_head_button input_submit">Reset</div> </a>
