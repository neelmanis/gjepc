...
/*	$registration_id = "214750";
	$order_id 		 = "visOnSpot3000";
	$ho_bp_number 	 = "7000013143";*/
	...
	$tpsl_txn_time=str_replace('/','-',$challanResult['tpsl_txn_time']);
	$getTxnDate = date("Ymd", strtotime($tpsl_txn_time));
	//$getTxnDate = 20190211;
...
/*.................................. Visitor Loop ........................................*/
	$visitor_query = "SELECT * FROM `visitor_order_history` WHERE `registration_id`= '$registration_id' AND `orderId`='".$order_id."' AND payment_status='Y'";
	...
	$event_material = getSap_material_no($visitor_row['payment_made_for']);
	if($visitor_id!=''){
           ...
           // curl_setopt($ch1, CURLOPT_TIMEOUT, 10);
			...
			foreach($xml->xpath('//soapenv:Body') as $header)
			{
					...
					{	$flag=1;
						...
						$sqlx = "UPDATE `visitor_order_detail` SET `sap_push_date`=NOW(),`sap_push_admin`='$adminID',`sales_order_no`='$sales_order_no',`sap_sale_order_create_status`='1' WHERE `regId`='$registration_id' AND orderId='".$order_id."'";
