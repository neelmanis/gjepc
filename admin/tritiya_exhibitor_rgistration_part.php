...
if($_REQUEST['Reset']=="Reset")
{
  ...
  header("Location: tritiya_exhibitor_rgistration_part.php");
} else
{ 
...
if($_REQUEST['action']=='old_to_part')
{
	...
	//	$sql="UPDATE `visitor_order_detail` SET `sap_sale_order_create_status` = '0', `sales_order_no` ='' WHERE `sales_order_no` ='old-to part' AND `sap_sale_order_create_status` = '1'";
		...
		echo "<meta http-equiv=refresh content=\"0;url=tritiya_exhibitor_rgistration_part.php\">";
		} else { 
...
<div id="main">
	<div class="content">
    		...
    		<div class="content_head"><a href="tritiya_exhibitor_rgistration_part.php">
		...
		<a href="export_utr_tritiya_exhibitor_rgistration_part.php">&nbsp;Download Payment Data</a> <a href="export_approve_tritiya_exhibitor_rgistration_part.php">&nbsp;Download Data</a></div>
		<?php
		if($_SESSION['curruser_login_id']=='28' || $_SESSION['curruser_login_id']=='1'){ ?>
		<a href="tritiya_exhibitor_rgistration_part.php?action=old_to_part" onClick="return(window.confirm('Are you sure you want to Clear Old to Part Data'));" >Clear SAP Error</a>	
		<?php }	?>
		</div>
  ...
  <tr >
    ...
    <?php } ?>
     ...
     <?php  $registration_id = $rows['uid'];
        ...
        $get_allotment_payment = $conn->query("SELECT * FROM `utr_history` WHERE registration_id='$registration_id' AND `show`='IIJS TRITIYA 2023' AND payment_made_for='PART' and payment_status='captured' limit 1");
...
<?php echo pagination($limit,$page,'tritiya_exhibitor_rgistration_part.php?page=',$rCount); //call function to show pagination?>
</div>        
	...
	if (confirm("Are you sure you want to create sales order")) {
		...
		{ 	
			...
			if($.trim(data)==1){
				...
				window.location.href = "tritiya_exhibitor_rgistration_part.php";
			}else{
				alert("Sorry There is some problem with SAP response"); 
				window.location.href = "tritiya_exhibitor_rgistration_part.php";			
			}
