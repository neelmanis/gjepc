...
if(!empty($_POST))
{
	...
	$addflag = "SELECT * FROM `webinar_registration_details` WHERE `id`='$registration_id' limit 1";
	...
	$company_name = strtoupper(trim(htmlspecialchars($deliveryResult['company_name'])));
	$company_name = str_replace('&', '&amp;', $deliveryResult['company_name']);
	$address1=strtoupper($deliveryResult['address1']); 
	$address2=strtoupper($deliveryResult['address2']); 
	$address3=""; 
	$city=strtoupper($deliveryResult['address3']);		 
	$region=strtoupper(getRegionName($deliveryResult['state'],$conn));
	$state = $deliveryResult['state']; 
	$country = "IN";
	$pincode=$deliveryResult['pincode'];	
	$id	=	$deliveryResult['id'];
	$email_id	=	filter($result['email_id']);
	$pan_no		=	strtoupper(filter($deliveryResult['pan_no']));	
	$gst_no		=	strtoupper(filter($deliveryResult['gst_no']));			 
	$legal_Entity=11;
	$mobile_no = filter($deliveryResult['mobile_no']);
	$tel1	   = "";
           ...
           $headers = array(
			...
			foreach($xml->xpath('//soapenv:Body') as $header)
			{
					...
					foreach ($tmp as $value) {
						$tmp = explode(",", $value);
						$flag=1;
						$sqlx = "UPDATE `webinar_registration_details` SET `bp_number`='$tmp[3]' WHERE id='$tmp[1]'";
