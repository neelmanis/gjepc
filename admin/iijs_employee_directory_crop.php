...
if($_REQUEST['company_action']=='save_document')
{
		...
		{
    		echo "<script langauge=\"javascript\">alert(\"Sorry you have select Invalid file.\");location.href='iijs_employee_directory_crop.php?action=view';</script>";
			exit;
		}
		elseif($file_name != '')
		{
			if($_FILES["upload_company_document"]["type"] == "application/pdf" || $_FILES["upload_company_document"]["type"] == "application/PDF")
			{
				...
				$stmt->execute();
				echo"<meta http-equiv=refresh content=\"0;url=iijs_employee_directory_crop.php?action=view\">";
				}
				else
				{
				echo "<script langauge=\"javascript\">alert(\"Sorry you can not upload this  file.\");location.href='iijs_employee_directory_crop.php?actions=companyedit&regid=$registration_id';</script>";
				return;
				}
			} else
			{
			 echo "<script langauge=\"javascript\">alert(\"Sorry you have select Invalid file.\");location.href='iijs_employee_directory_crop.php?actions=companyedit&regid=$registration_id';</script>";
			}		
		}
}
?>
			 ...
			 if ($vaccine_certificate =="fail") {
			 	echo "<script> alert('Sorry, report uploading has been failed on server. Please contact administrator');</script>";
			  		 	...
			  		 	echo "<script> alert('Please Select valid file type');</script>";
			  		 	 echo "<meta http-equiv=refresh content=\"0;url=iijs_employee_directory_crop.php?action=uploadVaccineCertificate&visitor_id=$visitor_id&registration_id=$registration_id\">"; exit;
			 }
		}else{
...
{ 
	...
	echo "<meta http-equiv=refresh content=\"0;url=iijs_employee_directory_crop.php?action=viewReg&regid=$registration_id\">"; exit;
	} else {
...
if($approval=='Y')
{
	//$message = "Dear ".$name.", your data for visitor badge has been approved, Kindly proceed for visitor registration of IIJS SIGNATURE 2022 SHOW.Please visit https://gjepc.org/iijs-signature/";
...
//	send_sms($message,$sendMobile);
	...
	//$mob = "9619662253,9987753842,9987753835,9004277744,9930922260,7498949490,8657584746";  
	/*$apiurl = sendOPTIN($sendMobile);
	...
	if($getResult['response']['status']=="success")
	{
		foreach($getResult['data'] as $value)
		{
			...
			$msgurl = visitor_Individual_Directory_Approval($sendMobile,$name);
    ...
    <tr>
        <td align="center" colspan="3">
            <p>
                ...
                Unit G2-A, Trade Center, Opp. BKC Telephone Exchange, Bandra Kurla Complex, Bandra (E) Mumbai 400 051 <br> <b>Toll Free Number :</b> 1800-103-4353, <b>Missed Call Number :</b> +91-7208048100<br> Website: <a href="https://gjepc.org/iijs-signature/">https://gjepc.org/iijs-signature/</a>
            </p>
        </td>
    </tr>       
</tbody>
...
{
	//$message = "Dear ".$name.", your data for visitor badge has been disapproved, Due to $disapprove_reason kindly update your record at IIJS SIGNATURE 2022 SHOW";
	...
	if($getResult['response']['status']=="success")
	{
		foreach($getResult['data'] as $value)
		{
			...
			$msgurl = visitor_Individual_Directory_Disapproval($sendMobile,$name,$disapprove_reason);
			$getResults = json_decode($msgurl,true);
			if($getResults['response']['status']=="success")
			{
				echo $getResults['response']['status'];
...
/***  Emailer End **/
echo"<meta http-equiv=refresh content=\"0;url=iijs_employee_directory_crop.php?action=viewReg&regid=$registration_id\">";
}
}
?>
...
{	
	...
	if($num>0)
	{
		...
		echo '</script>';		
	echo "<meta http-equiv=refresh content=\"0;url=iijs_employee_directory_crop.php?action=view\">"; exit;
	}
...
if($_REQUEST['Reset']=="Reset")
{
  ...
  header("Location: iijs_employee_directory_crop.php?action=view");
} else
{
...
<div id="main">
	<div class="content">
        ...
        Order Details <div class="content_head_button" style="float:right; padding-right:10px; font-size:12px;"><a href="iijs_employee_directory_crop.php?action=view">Back</a></div>
        ...
        <a href="iijs_employee_directory_crop.php?action=orderDetails&regid=<?php echo $registration_id; ?>">Back</a></div>
        Payment Details
        <?php } elseif($_REQUEST['action']=='viewReg'){ ?>Employee Directory 
        <div class="content_head_button" style="float:right; padding-right:10px; font-size:12px;"><a href="iijs_employee_directory_crop.php?action=view">Employee Directory</a></div>
        <div class="content_head_button" style="float:right; padding-right:10px; font-size:12px;"><a href="iijs_employee_directory_crop.php?action=view">Back</a></div> 
        ...
        <a href="iijs_employee_directory_crop.php?action=viewReg&regid=<?php echo $registration_id; ?>">Back</a></div>
        ...
        <a href="iijs_employee_directory_crop.php?action=view">Back</a></div> 
        <?php } else { ?>	Employee Directory
        <div style="float:right; padding-right:10px; font-size:12px;">
        <a href="iijs_export_approve_emp_dir.php">&nbsp;Download Approved Data</a></div>
        <div style="float:right; padding-right:10px; font-size:12px;">
        ...
        <div style="float:right; padding-right:10px; font-size:12px;">
       <a href="export_domestic_pending_vaccination_report.php">&nbsp;Download Pending Vaccination Certificate report</a>
        </div>
       </div><div class="clear">
        <?php } ?>		
		<?php
		if($_SESSION['curruser_login_id']=='44' || $_SESSION['curruser_login_id']=='1'){ ?>
		<a href="iijs_employee_directory_crop.php?action=old_to_part" onClick="return(window.confirm('Are you sure you want to Clear Old to Part Data'));" >Clear Old to Part Data</a>	
		<?php }	?>
        </div>
<?php if($_REQUEST['action']=='view') { ?>
...
<?php 
	...
	//$sql5 = "select * from visitor_directory where visitor_approval='Y' and status = '1' AND registration_id!='0' AND visitor_approval!='O'";
...
AND vd.isApplied='Y' AND vd.visitor_approval!='O' AND vo.payment_type='online'"; */
  ...
  <tr>
    ...
    <td align="center" valign="middle"><a href="iijs_employee_directory_crop.php?actions=companyedit&regid=<?php echo $rows['id'];?>">
    ...
    <a href="iijs_employee_directory_crop.php?action=viewReg&regid=<?php echo $rows['id'];?>">Employee Details</a> / <a href="iijs_employee_directory_crop.php?action=orderDetails&regid=<?php echo $rows['id'];?>">Order Details</a></td>
...
<?php echo pagination($limit,$page,'iijs_employee_directory_crop.php?action=view&page=',$rCount); //call function to show pagination?>
</div>  
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="detail_txt" >
  <tr class="orange1">
    ...
    <td>Status</td>
    <td>View Details</td>
	<?php if($adminID==1 || $adminID==44 || $adminID==131 || $adminID==123 || $adminID==91 || $adminID==109 || $adminID==97 || $adminID==136 || $adminID==110 || $adminID==34 || $adminID==31){?><td>Delete</td><?php } ?>
  </tr>
    <?php  
	$page=1;//Default page
  ...
  <tr>
    ...
    <td align="left" valign="middle"><a href="iijs_employee_directory_crop.php?action=edit&id=<?php echo $rows['visitor_id'];?>&regid=<?php echo $rows['registration_id'];?>">
    <img src="images/edit.gif" title="Edit" border="0"/></a></td>
	
	<?php if($adminID==1 || $adminID==44 || $adminID==131 || $adminID==123 || $adminID==91 || $adminID==109 || $adminID==97 || $adminID==136 || $adminID==110 || $adminID==34 || $adminID==31){ ?>
	<td><a style="text-decoration:none;" href="iijs_employee_directory_crop.php?action=delVisitor&visitor_id=<?php echo $rows['visitor_id'];?>&registration_id=<?php echo $registration_id;?>" onClick="return(window.confirm('Are you sure you want to Delete.'));"><img src="images/no.gif" border="0" title="Delete"/></a></td>
	<?php } ?>
  </tr>
  <?php
   $i++;
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="detail_txt" >
  ...
  <tr >
	...
	<td align="left" valign="middle"><a href="iijs_employee_directory_crop.php?action=orderHistory&orderId=<?php echo $rows['orderId'];?>&regid=<?php echo $rows['regId'];?>" style="color:#000000">VIEW</a></td>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="detail_txt" >
  ...
  <tr>
	...
	<?php if($adminID==1){ if($rows['show'] == "signature22"){ ?>
	<td><a style="text-decoration:none;" href="iijs_employee_directory_crop.php?action=global&visitor_id=<?php echo $rows['visitor_id'];?>&registration_id=<?php echo $registration_id;?>" onClick="return(window.confirm('Are you sure you want to Send.'));"><img src="images/active.png" border="0" title="send"/></a></td>
	  
	<?php } } ?>
	<?php  if($rows['show'] == "signature22"){ ?>
	<td><a style="text-decoration:none; padding: 5px auto;height: auto;" class="content_head_button" href="iijs_employee_directory_crop.php?action=uploadVaccineCertificate&visitor_id=<?php echo $rows['visitor_id'];?>&registration_id=<?php echo $registration_id;?>" > Upload  </a></td>
	<?php }else{?>
			<td></td>
	<?php }  ?>
...
<table width="100%" border="0" cellspacing="2" cellpadding="2" class="detail_txt" >
   ...
   <!--  <a data-fancybox="gallery" href="https://registration.gjepc.org/images/employee_directory/<?php echo $registration_id; ?>/photo/<?php echo $photo; ?>"> 
    ...
    <div class="row">
    	<div class="col-6">
	      ...
	      <img id="blah_photo" class="img-fluid " src="https://registration.gjepc.org/images/employee_directory/<?php echo $registration_id; ?>/photo/<?php echo $photo; ?>" alt="your image" style="display: block;"/>
  ...
  <tr>
   ...
   <?php } else { ?>
    ...
    <img class="img_e" src="https://registration.gjepc.org/images/employee_directory/<?php echo $registration_id; ?>/pan_copy/<?php echo $pan_copy; ?>" title="<?php echo $pan_no; ?>"/>
    </a> 
	<?php } ?>
    </td>
  </tr>
  ...
  <tr>
    ...
    <img class="img_e" src="https://registration.gjepc.org/images/employee_directory/<?php echo $registration_id; ?>/salary/<?php echo $salary_slip_copy; ?>" alt="your image"/></a> 
	<?php } ?>
    </td>
  </tr>
  <?php } else { ?>
  <tr>
    ...
    <img class="img_e" src="https://registration.gjepc.org/images/employee_directory/<?php echo $registration_id; ?>/partner/<?php echo $partner; ?>" alt="your image" /></a><?php } ?>
    </td>
  </tr>
  <?php } ?>
  <tr>
...
<table width="100%" border="0" cellspacing="2" cellpadding="2" class="detail_txt" >
  ...
  <tr>
    ...
    <td><input type="text" name="company_gstn" id="company_gstn" class="input_txt" value="<?php echo $company_gstn; ?>" maxlength="15"/></td>
  </tr>
   <tr>
  ...
  <tr >
    ...
    <img class="img_e" src="https://registration.gjepc.org/images/gst_copy/<?php echo $gst_copy; ?>" title="<?php echo $company_gstn; ?>"/></a>
	<?php } ?>
    </td>
  </tr>
  <tr>
    ...
    <img class="img_e" src="https://registration.gjepc.org/images/pan_no_copy/<?php echo $pan_no_copy; ?>" title="<?php echo $company_pan_no; ?>"/></a>
    <?php } ?>
    </td>
  </tr>
  <tr>
	...
	if (confirm("Are you sure you want to update this address with Delta API")) {
		...
		success:function(data)
		{
			...
			if($.trim(data)==1){
				...
				window.location.href = "iijs_employee_directory_crop.php?action=viewReg&regid="+registration_id;
			}else{
				alert("Sorry There is some problem with SAP response"); 
				window.location.href = "iijs_employee_directory_crop.php?action=viewReg&regid="+registration_id;
	...
	if (confirm("Are you sure you want to update this address with Delta API")) {
		...
		success:function(data)
		{
			...
			if($.trim(data)==1){
				...
				window.location.href = "iijs_employee_directory_crop.php?action=view";
			}else{
				alert("Sorry There is some problem with SAP response"); 
				window.location.href = "iijs_employee_directory_crop.php?action=view";
