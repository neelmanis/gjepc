...
/**********************************************************  International Domestic BP Creation  ***************************************/
if(!empty($_POST))
{
	...
	$pan_no	=	filter(strtoupper($result['company_pan_no']));	
	$gst_no	=	filter(strtoupper($result['company_gstn']));
	...
	$company_name = filter(strtoupper($result['company_name']));
	...
	if($country=="IN"){
			$state=strtoupper(addslashes($result['state']));
			$currency="INR";
	} else {
			$state="";
			$currency="USD";
	...
	if($result['pin_code']=="" || $result['pin_code']==0)
		$pincode=12345;
	else 
  ...
  <soapenv:Body>
	...
	$xml_post_string .= "
		...
		<BP_item1>
			...
			<curr>$currency</curr>
			<price_grp>02</price_grp>
			<cust_price_pro>02</cust_price_pro>
			<deliver_plant>$region</deliver_plant>
			<acc_ass_grp>02</acc_ass_grp>
			<CGST>0</CGST>
			<SGST>0</SGST>
			<IGST>0</IGST>
		</BP_item1>";
           ...
           $headers = array(
			...
			foreach($xml->xpath('//soapenv:Body') as $header)
			{
					...
					foreach ($tmp as $value) {
						$tmp = explode(",", $value);
						$flag=1;
						$sqlx = "UPDATE `registration_master` SET `NM_bp_number`='$tmp[3]' WHERE id='$tmp[1]'";
