<div class="form-panel">
        <h4 class="mb">Edit Contact Details</h4>
		
			<?php
			if(is_array($editcontact)){
            foreach($editcontact as $row){
			 //echo '<pre>';print_r($row);
			?>
			<form method="post" class="form-horizontal style-form categoryform" action="<?php echo base_url("showinfo/updateContact");?>" enctype="multipart/form-data">
			...
			<?php if(!empty($row->html_files)) { ?>
				 <a href="<?php echo base_url();?>uploads/contact/<?php echo $row->html_files;?>" target="_blank">Your File</a>
					<?php } ?>
			</div>
			</div>
