<div class="form-panel">
        <h4 class="mb">Edit How to Reach Details</h4>
		
			<?php
			if(is_array($editreach)){
            foreach($editreach as $row){
			 //echo '<pre>';print_r($row);
			?>
			<form method="post" class="form-horizontal style-form categoryform" action="<?php echo base_url("showinfo/updateReach");?>" enctype="multipart/form-data">
			...
			<?php if(!empty($row->html_files)) { ?>
				 <a href="<?php echo base_url();?>uploads/reach/<?php echo $row->html_files;?>" target="_blank">Your File</a>
					<?php } ?>
			</div>
			</div>
