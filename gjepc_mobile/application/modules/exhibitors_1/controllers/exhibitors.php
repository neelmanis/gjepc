...
/**************************************************************Shital*******************************************************************/	
	...
	function editExhibitor($id)
	{
		if(!empty($id))
		{
		  ...
		  $data['display']=$this->mdl_exhibitors->getUser($id);
		  $data['menu'] = 'abc';
		  echo Modules::run('template/'.$template, $data);
		}
		else
	...
	function updateUser()
	{
        ...
        $this->form_validation->set_rules('email', 'Email','required|trim|max_length[30]|xss_clean|valid_email');
		$this->form_validation->set_rules('userSpace', 'User Space','required');
