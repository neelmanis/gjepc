...
/***************************************************** Start Show Details *****************************************************/	
	...
	function venueList($status = 'active')
	{
		$template = 'admin';
		$data['viewFile'] = "venue";
		$data['page'] = 'details';
		$data['menu'] = 'infos';
		$data['details'] = $this->mdl_venue->getVenueList();
		echo Modules::run('template/'.$template, $data);
	}	
	...
	{
		  $template = 'admin';
		  $data['viewFile'] = "addvenuedetails";
		  $data['page'] = 'addshowdetails';
		  $data['menu'] = 'abc';
		  $data['events']= $this->mdl_venue->getEvents();
		  $data['years']= $this->mdl_venue->getYear();
		  echo Modules::run('template/'.$template, $data);
	}
	...
	{
	   $getEvents = $this->mdl_venue->getEventName($event);
	   echo  $getEvents;
    }
	...
	{
	   $getYear = $this->mdl_venue->getYearName($year);
	   echo  $getYear;
    }
		...
		$userdata = $this->input->post();
				...
				if(!empty($_FILES['my_file']['name']))
				  { 
				    ...
				    $img = $this->uploadFile($imgname,"my_file","venue");	
				  }
			 ...
			 $data = array(
					...
					$updateData = $this->mdl_venue->addDetail($data);					
	...
	function editinfo($id)
	{
		...
		{
		  $template = 'admin';
		  $data['viewFile'] = "editvenue";
		  $data['page'] = 'editinfo';
		  $data['editinfo']=$this->mdl_venue->getUser($id);
		  $data['events']= $this->mdl_venue->getEvents();
		  $data['years']= $this->mdl_venue->getYear();
		  $data['menu'] = 'abc';
		  echo Modules::run('template/'.$template, $data);
	...
	function updateDetails()
	{
		...
		$id =$userdata['id'];
			 ...
			 $data = array(
					...
					$updateData = $this->mdl_venue->userUpdate($data,$id);
					redirect('venue/venueList');
	  }
	    ...
	    $template = 'admin';
		$data = array();
		$data['viewFile'] = 'viewvenuedetails';
		$data['page'] = 'viewshowdetails';
		$data['menu'] = 'abc';
		//$data['title'] = 'Signin';
	    $data['view_details'] = $this->mdl_venue->getUser($id);
        $data['image'] = $this->mdl_venue->getUser($id);
		$data['events']= $this->mdl_venue->getEvents();
		$data['years']= $this->mdl_venue->getYear();
		echo Modules::run('template/'.$template, $data);
		}
		...
		function delete_details($id)
		{
			if(!empty($id))
			{
				$deleteData = $this->mdl_venue->del_Detail($id);
				redirect('venue/venueList');	
			}
		}
