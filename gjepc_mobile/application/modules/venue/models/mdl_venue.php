...
class Mdl_venue extends CI_Model {
...
/* Start ShowInfo Details */	
	...
	{
            $this->db->select('*');
            $this->db->from('cms_services_venue');
            $query = $this->db->get(); 
	 ...
	 {
		$this->db->where("id", $id);
		$ans=$this->db->update("cms_services_venue",$data);
	...
	{
		$this->db->insert("cms_services_venue",$data);
		//echo $ans=$this->db->last_query(); exit;
	}
   
   function getUser($id)
	{
		$ans=$this->db->get_where("cms_services_venue",array("id"=>$id));
		if($ans->num_rows()>0)
		{
			return $ans->result();
		}
	...
	{
	    $this ->db-> where('id', $id);
        $result = $this ->db-> delete('cms_services_venue');
