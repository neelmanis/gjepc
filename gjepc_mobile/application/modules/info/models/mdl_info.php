...
/* Start ShowInfo Details */	
	public function infoList()
	{
	 ...
	 {
		$this->db->where("id", $id);
		$ans=$this->db->update("cms_show_info",$data);
	...
	{
		$this->db->insert("cms_show_info",$data);
		//echo $ans=$this->db->last_query(); exit;
	}
   
   function getUser($id)
	{
		$ans=$this->db->get_where("cms_show_info",array("id"=>$id));
		if($ans->num_rows()>0)
		{
			return $ans->result();
		}
	...
	{
	    $this ->db-> where('id', $id);
        $result = $this ->db-> delete('cms_show_info');
