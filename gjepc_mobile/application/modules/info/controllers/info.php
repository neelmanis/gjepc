...
/***************************************************** Start Show Details *****************************************************/	
	...
	function infoList($status = 'active')
	{
		$template = 'admin';
		$data['viewFile'] = "info";
		$data['page'] = 'details';
		$data['menu'] = 'infos';
		$data['details'] = $this->mdl_info->infoList();
		echo Modules::run('template/'.$template, $data);
	}	
	...
	{
		  $template = 'admin';
		  $data['viewFile'] = "addinfodetails";
		  $data['page'] = 'addshowdetails';
		  $data['menu'] = 'abc';
		  $data['events']= $this->mdl_info->getEvents();
		  $data['years']= $this->mdl_info->getYear();
		   echo Modules::run('template/'.$template, $data);
	}
	...
	{
	   $getEvents = $this->mdl_info->getEventName($event);
	   echo  $getEvents;
    }
	...
	{
	   $getYear = $this->mdl_info->getYearName($year);
	   echo  $getYear;
    }
		...
		$userdata = $this->input->post();
				...
				if(!empty($_FILES['my_file']['name']))
				  { 
				    ...
				    $img = $this->uploadFile($imgname,"my_file","showinfo");	
				  }
			 ...
			 $data = array(
					...
					$updateData = $this->mdl_info->addDetail($data);
	...
	function editinfo($id)
	{
		...
		{
		  $template = 'admin';
		  $data['viewFile'] = "editinfo";
		  $data['page'] = 'editinfo';
		  $data['editinfo']=$this->mdl_info->getUser($id);
		  $data['events']= $this->mdl_info->getEvents();
		  $data['years']= $this->mdl_info->getYear();
		  $data['menu'] = 'abc';
		  echo Modules::run('template/'.$template, $data);
	...
	function updateDetails()
	{
		...
		$id =$userdata['id'];
			 ...
			 $data = array(
					...
					$updateData = $this->mdl_info->userUpdate($data,$id);
					redirect('info/infoList');	
	  }
	    ...
	    $template = 'admin';
		$data = array();
		$data['viewFile'] = 'viewinfodetails';
		$data['page'] = 'viewshowdetails';
		$data['menu'] = 'abc';
		//$data['title'] = 'Signin';
	    $data['view_details'] = $this->mdl_info->getUser($id);
        $data['image'] = $this->mdl_info->getUser($id);
		$data['events']= $this->mdl_info->getEvents();
		$data['years']= $this->mdl_info->getYear();
		echo Modules::run('template/'.$template, $data);
		}
		...
		function delete_details($id)
		{
			if(!empty($id))
			{
				$deleteData = $this->mdl_info->del_Detail($id);
				redirect('info/infoList');	
			}
		}
