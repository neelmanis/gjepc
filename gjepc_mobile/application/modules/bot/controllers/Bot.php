...
function checkAuth($obj){
  ...
  if($method !== 'POST'){
    ...
    if($obj['username']=="gjepc@yellow.ai" && $obj['password']=="gjepc@2022"){
      return true;
    }else{
      return false;
    }
...
function checkVisitorPan(){
 ...
 // $event = trim($obj['event']);
  $datetime = date("Y-m-d H:i:s");

    if($this->checkAuth($obj)){
    if($pan_no !=""){
    
        /*
        ** CHECK PAN NUMBER IS ALREADY EXIST IN TABLE OR NOT
        */
		$sql = "SELECT * FROM gjepclivedatabase.globalExhibition where 1 AND pan_no='$pan_no' AND (participant_Type='VIS' || participant_Type='IGJME')";
        $panExist = $this->Mdl_badge->isExistCustom($sql);
        if($panExist ===TRUE){
          $digits = 4;  
        ...
        $face_status =  $getGlobal[0]->face_status;
        if($status  =="Y")
		{
          if($face_status =="Y")
		  {
          ...
          $isVerified = $getGlobal[0]->isVerified;
		   ...
		   } else if($face_status =="D"){
			  $response=array(               
                "Result"=>array(),
                "Message"=>"Your Face Recognition DisApproved",
                "status"=>"false"
              );  
		   } else {
			  $response=array(               
                "Result"=>array(),
                "Message"=>"Your Face Recognition status is not Active",
                "status"=>"false"
              );  
		   }
        } else {
        ...
        "Result"=>array(),
        "Message"=>"You are not registered for selected show",
        "status"=>"false"
        );  
        }
  }else{
    ...
    $response=array(
        "Result"=>array(),
        "Message"=>"check method and credentials. ",
        "status"=>"false"
    );  
}
           ...
           if($getOtpDetails !="NA"){            
            ...
            $result=$this->Mdl_badge->update("globalExhibition",array("pan_no"=>$pan_no,"id"=>$id,),$data);
              $uniqueId = trim($getOtpDetails[0]->uniqueIdentifier);
                //$link  = "https://registration.gjepc.org/generate_badge.php?unique_id=".$uniqueId;
                $link  = "https://registration.gjepc.org/visitor-badge.php?action=generateBadge&uniqueIdentifier=".$uniqueId;
                $response=array(
                    "Result"=>array("badge_link"=>$link),
