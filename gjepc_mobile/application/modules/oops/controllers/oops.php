...
class Oops extends MX_Controller {
	
	function __construct()
	{
		parent::__construct();
		$this->load->model('mdl_oops');
...
/***************************************************** Start Show Details *****************************************************/	
	...
	{
		$template = 'admin';
		$data['viewFile'] = "oopslist";
		$data['page'] = 'details';
		$data['menu'] = 'infos';
		$data['details'] = $this->mdl_oops->getOopsList();
		echo Modules::run('template/'.$template, $data);
	}	
	...
	{
		  $template = 'admin';
		  $data['viewFile'] = "addoopsdetails";
		  $data['page'] = 'addoopsdetails';
		  $data['menu'] = 'abc';
		  $data['events']= $this->mdl_oops->getEvents();
		  $data['years']= $this->mdl_oops->getYear();
		  echo Modules::run('template/'.$template, $data);
	}
	...
	{
	   $getEvents = $this->mdl_oops->getEventName($event);
	   echo  $getEvents;
    }
	...
	{
	   $getYear = $this->mdl_oops->getYearName($year);
	   echo  $getYear;
    }
		...
		$userdata = $this->input->post();
				...
				if(!empty($_FILES['my_file']['name']))
				  { 
				    ...
				    $img = $this->uploadFile($imgname,"my_file","oops");	
				  }
			 ...
			 $data = array(
					...
					$updateData = $this->mdl_oops->addDetail($data);					
					redirect('oops/oopsList');		
	}
	...
	function editinfo($id)
	{
		...
		{
		  $template = 'admin';
		  $data['viewFile'] = "editoops";
		  $data['page'] = 'editoops';
		  $data['editinfo']=$this->mdl_oops->getUser($id);
		  $data['events']= $this->mdl_oops->getEvents();
		  $data['years']= $this->mdl_oops->getYear();
		  $data['menu'] = 'abc';
		  echo Modules::run('template/'.$template, $data);
	...
	function updateDetails()
	{
		...
		$id =$userdata['id'];
			 ...
			 $data = array(
					...
					$updateData = $this->mdl_oops->userUpdate($data,$id);
					redirect('oops/oopsList');
	  }
	    ...
	    $template = 'admin';
		$data = array();
		$data['viewFile'] = 'viewoopsdetails';
		$data['page'] = 'viewoopsdetails';
		$data['menu'] = 'abc';
		//$data['title'] = 'Signin';
	    $data['view_details'] = $this->mdl_oops->getUser($id);
        $data['image'] = $this->mdl_oops->getUser($id);
		$data['events']= $this->mdl_oops->getEvents();
		$data['years']= $this->mdl_oops->getYear();
		echo Modules::run('template/'.$template, $data);
		}
		...
		function delete_details($id)
		{
			if(!empty($id))
			{
				$deleteData = $this->mdl_oops->del_Detail($id);
				redirect('oops/oopsList');	
			}
		}
