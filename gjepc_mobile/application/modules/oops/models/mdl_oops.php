...
class Mdl_oops extends CI_Model {
	...
	function get_table() {
		$table = "cms_operational_manual";
		return $table;
	}
	
/* Start ShowInfo Details */	
	public function getOopsList()
	{
            $this->db->select('*');
            $this->db->from('cms_operational_manual');
            $query = $this->db->get(); 
	 ...
	 {
		$this->db->where("id", $id);
		$ans=$this->db->update("cms_operational_manual",$data);
	...
	{
		$this->db->insert("cms_operational_manual",$data);
		//echo $ans=$this->db->last_query(); exit;
	}
   
   function getUser($id)
	{
		$ans=$this->db->get_where("cms_operational_manual",array("id"=>$id));
		if($ans->num_rows()>0)
		{
			return $ans->result();
		}
	...
	{
	    $this ->db-> where('id', $id);
        $result = $this ->db-> delete('cms_operational_manual');
