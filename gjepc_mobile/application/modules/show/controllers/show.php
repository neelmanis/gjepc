...
/*********************************************** Start Show Updates Details ***************************************************/	
	...
	function showList()
	{
		...
		$data['viewFile'] = "show";
		$data['page'] = 'details';
		$data['menu'] = 'show_update';
		$data['details'] = $this->mdl_show->getShowList();
		echo Modules::run('template/'.$template, $data);
	}	
	...
	{
		  $template = 'admin';
		  $data['viewFile'] = "addupdatedetails.php";
		  $data['page'] = 'addshowdetails';
		  $data['menu'] = 'abc';
		  $data['events']= $this->mdl_show->getEvents();
		  $data['years']= $this->mdl_show->getYear();
		   echo Modules::run('template/'.$template, $data);
	}
	...
	{
	   $getEvents = $this->mdl_show->getEventName($event);
	   echo  $getEvents;
    }
	...
	{
	   $getYear = $this->mdl_show->getYearName($year);
	   echo  $getYear;
    }
	...
	{ 
		$userdata = $this->input->post();				  
			...
			$updateData = $this->mdl_show->addDetail($data);					
	...
	function editinfo($id)
	{
		...
		{
		  $template = 'admin';
		  $data['viewFile'] = "editupdate";
		  $data['page'] = 'editupdate';
		  $data['editinfo']=$this->mdl_show->getUser($id);
		  $data['events']= $this->mdl_show->getEvents();
		  $data['years']= $this->mdl_show->getYear();
		  $data['menu'] = 'abc';
		  echo Modules::run('template/'.$template, $data);
	...
	function updateDetails()
	{
		...
		$id =$userdata['id'];						  
			...
			$updateData = $this->mdl_show->userUpdate($data,$id);
			redirect('show/showList');	
	  }
		...
		$data['menu'] = 'abc';
		//$data['title'] = 'Signin';
	    $data['view_details'] = $this->mdl_show->getUser($id);
        $data['image'] = $this->mdl_show->getUser($id);
		$data['events']= $this->mdl_show->getEvents();
		$data['years']= $this->mdl_show->getYear();
		echo Modules::run('template/'.$template, $data);
		}
		...
		function delete_details($id)
		{
			if(!empty($id))
			{
				$deleteData = $this->mdl_show->del_Detail($id);
				redirect('show/showList');	
			}
		}
