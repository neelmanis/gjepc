<?php 
$pageTitle = "Gems And Jewellery Industry In India | Region-wise Import - GJEPC India";
...
include 'functions.php';
		...
		if($_REQUEST['Reset']=="Reset")
		{
		  ...
		  header("Location: region-wise-import.php");
		} else {
    ...
    <div class="row justify-content-center">
			...
			<input type="hidden" name="search_type" id="search_type" value="SEARCH"/>
                ...
                <div class="form-group col-12 mb-4">
                    <p class="blue text-center">IMPORTS - REGION-WISE</p>
                </div>
		...
		?>
            <div class="col-12">
                        <div class="row py-3" style="border-top: 1px solid #ccc">
                            <div class="col-12"><h2 class="title text-center mb-1"><span class="d-table mx-auto mb-3" style="width:35px;"><img src="assets/images/black_star.png" class="img-fluid d-block"></span>Imports - Region-wise</h2></div>
                            ...
                            <div class="col-12">
                                ...
                                <button onclick="exportToExcel('tblexportData', 'region-wise-import')" class="cta gold_btn" title="Export To Excel"><i class="fa fa-file-excel-o" style="font-size: 40px;"></i></button>
                                </div> 
                            </div>
					...
					/* Same */		if(getFinancialYear($conn)==$financial_year){ $sameYear = "YES";	} else { $sameYear = "NO"; }		
									while($row = $resultx->fetch_assoc())
									{
										if($currency!="")
										{
											...
											$getPreviousYearData = getPreviousFinancialYearRegionINR_i($last_finyr,$row['region_code'],$sameYear,$currentMonth,$conn);
										...
										//	$newTotal_Paisa  = round($row['total_usd']/$_POST['currency_usd'],2);
											...
											$getPreviousYearData = getPreviousFinancialYearRegionUSD_i($last_finyr,$row['region_code'],$sameYear,$currentMonth,$conn);
				...
				/* Same */		if(getFinancialYear($conn)==$financial_year){ $sameYear = "YES";	} else { $sameYear = "NO"; }			
									while($row = $resultx->fetch_assoc())
									{
										...
										if($currency!="")
										{
											...
											$getPreviousYearData  = getLastYearSingleRegionCountryINR_i($last_finyr,$row['country_id'],$sameYear,$currentMonth,$conn);
											$getPreviousData   	  = round($getPreviousYearData/$_POST['currency_inr'],2);
											$sub_lastYear_Paisa   = round($sub_lastYear_Paisa + $getPreviousYearData/$_POST['currency_inr'],2);
										...
										//	$newTotal_Paisa  = round($row['total_usd']/$_POST['currency_usd'],2);
											...
											$getPreviousYearData = getLastYearSingleRegionCountryUSD_i($last_finyr,$row['country_id'],$sameYear,$currentMonth,$conn);
										//	$getPreviousData   	 = round($getPreviousYearData/$_POST['currency_usd'],2);
											$number  = $getPreviousYearData/$_POST['currency_usd'];
											$getPreviousDataNOround  = number_format($number, 12, '.', '');
											$zeroLevel = strspn($getPreviousDataNOround, "0", strpos($getPreviousDataNOround, ".")+1);
