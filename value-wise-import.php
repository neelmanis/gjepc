<?php 
$pageTitle = "Gems And Jewellery Industry In India | Value-wise Import - GJEPC India";
...
<?php
		...
		if($_REQUEST['Reset']=="Reset")
		{
		  ...
		  header("Location: value-wise-import.php");
		} else {
	...
	//	echo '<pre>'; print_r($arr_string); 
		...
		{	
			...
			$sql2=$conn ->query("SELECT `year`, `financial_year`, `post_date`, `inr_value`, SUM(REPLACE(inr_value, ',', '')) as total_inr, `dollar_value`, SUM(REPLACE(dollar_value, ',', '')) as total_usd, DATE_FORMAT(post_date,'%Y-%m') as entry_month FROM `statistics_integration_data` WHERE 1 AND post_date BETWEEN '$lastCompairYear-01-01' AND '$lastCompairYear-12-31' AND trade_type='Import' group by entry_month order by entry_month asc");
    ...
    <div class="row justify-content-center">
			...
			<input type="hidden" name="search_type" id="search_type" value="SEARCH"/>
                ...
                <div class="form-group col-12 mb-4">
                    <p class="blue text-center">IMPORTS - VALUE-WISE</p>
                </div>   
		...
		<div class="col-12">
                        <div class="row py-3" style="border-top: 1px solid #ccc">
                            <div class="col-12"><h2 class="title text-center mb-1"><span class="d-table mx-auto mb-3" style="width:35px;"><img src="assets/images/black_star.png" class="img-fluid d-block"></span>Import - Value-wise</h2></div>
                                ...
                                <table class="table table-bordered tableInfo" id="tblexportData">
									...
									{ 	
										...
										if($currency!="")
										{
											...
											$getPreviousFinancialYearData = round(getPreviousFinancialYearImportValueINR_i($lastCompairYear,$sameYear,$currentMonth,$conn)/$_POST['currency_inr'],2);
											...
											$getPreviousFinancialYearData = round(getPreviousFinancialYearImportValueUSD_i($lastCompairYear,$sameYear,$currentMonth,$conn)/$_POST['currency_usd'],2);
											$lastYearTotal_Paisa   = str_replace(',','',$getPreviousFinancialYearData);
											$sub_lastYear_Paisa = round($sub_lastYear_Paisa + $lastYearTotal_Paisa);
				...
				/* Same */			if($cur_calenderyr==$_POST['year_calendar_year']){ $sameYear = "YES";	} else { $sameYear = "NO"; }
									...
									//	$lastCompairDate = $row['post_date'];
										if($currency!="")
										{
											...
											$getPreviousYearData = getLastMonthYearImportValueINR_i($lastCompairYear,$sameYear,$currentMonth,$conn);
											...
											$getPreviousYearData = getLastMonthYearImportValueUSD_i($lastCompairYear,$sameYear,$currentMonth,$conn);
                                ...
                                <table class="table table-bordered tableInfo" id="tblexportData">
									...
									//	$lastCompairYear = getLastMonths($year_calendar_year,$conn);
										if($currency!="")
										{
											...
											$getPreviousYearData = getLastMonthImportValueINR_i($lastCompairYear,$lastCompairMonths,$conn);
											$sub_lastYear_Paisa  = round($sub_lastYear_Paisa + $getPreviousYearData/$_POST['currency_inr'],2);
											$getPreviousData	 = round($getPreviousYearData/$_POST['currency_inr'],2);
											...
											$getPreviousYearData = getLastMonthImportValueUSD_i($last_finyr,$lastCompairMonths,$conn);
											$sub_lastYear_Paisa  = round($sub_lastYear_Paisa + $getPreviousYearData/$_POST['currency_usd'],2);
											$getPreviousData	 = round($getPreviousYearData/$_POST['currency_usd'],2);					
