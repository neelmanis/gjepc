...
$FORM_TYPE="E";
	...
	if(!empty($_REQUEST['export_app_id1']))
	{	//echo '<pre>'; print_r($_POST); exit;
	$strNo = rand(1,10000000);
	$sqlx="insert into kp_payment_log set ReferenceNo='$strNo',APPLICANT_ID='$APPLICANT_ID',FORM_TYPE='$FORM_TYPE',PAYMENT_TYPE='$PAYMENT_TYPE',PAYMENT_AMOUNT='$PAYMENT_AMOUNT',APPLICANT_NAME='$APPLICANT_NAME'";
	$getSqlxResults = mysqli_query($conn,$sqlx);
    ...
    {
		$query = mysqli_query($conn,"SELECT TOTAL_AMOUNT,LOC_PICKUP_ID from kp_export_application_master where EXPORT_APP_ID='$val'");
	...
	$PAYMENT_MST_ID = mysqli_insert_id($conn);
		...
		$query1 = mysqli_query($conn,"select MEMBER_TYPE_ID,MEMBER_ID,NON_MEMBER_ID from kp_export_application_master where EXPORT_APP_ID='$val'");
		...
		$detail = "insert into kp_payment_details set EXPORT_APP_ID='$val',PAYMENT_MST_ID='$PAYMENT_MST_ID',RECIEPT_NO='$RECIEPT_NO',APPLICANT_ID ='$APPLICANT_ID',MEMBER_TYPE_ID='$MEMBER_TYPE_ID',MEMBER_ID='$MEMBER_ID',FORM_TYPE='E',TRAN_ID='$TRAN_ID'";
		$detailResult = mysqli_query($conn,$detail);
		if($detailResult){		 
		mysqli_query($conn,"update kp_export_application_master set payment_made_status='Y',KP_PAYMENT_ID='$PAYMENT_MST_ID' where EXPORT_APP_ID='$val'");
	...
	*/
	...
	$getPResults = mysqli_query($conn,"insert into kp_payment_details set EXPORT_APP_ID='$val',PAYMENT_MST_ID='$PAYMENT_MST_ID',RECIEPT_NO='$RECIEPT_NO',APPLICANT_ID ='$APPLICANT_ID',MEMBER_TYPE_ID='$MEMBER_TYPE_ID',MEMBER_ID='$MEMBER_ID',FORM_TYPE='E',TRAN_ID='$TRAN_ID'");	
	if(!$getPResults) die ($conn->error);
	
	$getEResults = mysqli_query($conn,"update kp_export_application_master set payment_made_status='Y',KP_PAYMENT_ID='$PAYMENT_MST_ID',ReferenceNo='$ReferenceNo' where EXPORT_APP_ID='$val'"); 
...
<section class="py-5">
	...
	<div class="container inner_container">
			...
			<div class="col-12 box-shadow">
				...
				<form action="" method="POST" onsubmit="return checkdata()" name="form2" id="form2">
					...
					<div class="search_app_div">
						<table class="table">
							<td><!--<input name="check_all" id="check_all" class="check_all" type="checkbox" value="" onClick="toggle(this)"/>--></td>
					...
					<td><a class="btn btn-dark p-1" href="export_application.php?EXPORT_APP_ID=<?php echo $result_import_pending['EXPORT_APP_ID'];?>&action=update">Edit</a></td>
				</tr>
		...
		<div class="search_app_div"><p class="error"> <b>Important information : </b> Dear User, Kindly Use the mozilla firefox browser,for the hassle free transaction for KP applications & avoid using other browsers.</p>		</div>
		<div class="row">
			<div class="col-12">
