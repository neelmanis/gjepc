...
if($action=="ADD")
{
	...
	$USER_NAME="KP".rand(0,999999);
	$AGENT_NAME=filter(strtoupper($_REQUEST['AGENT_NAME']));
	...
	$STATE=strtoupper(getOrginStateName($STATE_ID));
	$CITY=filter(strtoupper($_REQUEST['CITY']));
	...
	$CONTACT_PERSON1=filter(strtoupper($_REQUEST['CONTACT_PERSON1']));
	$CONTACT_PERSON2=filter(strtoupper($_REQUEST['CONTACT_PERSON2']));
	...
	$FILE_UPLOAD=filter($_REQUEST['FILE_UPLOAD']);
		...
		{
			if (($_FILES["FILE_UPLOAD"]["type"] == "application/msword") || ($_FILES["FILE_UPLOAD"]["type"] == "application/pdf"))
			{
				$target_path = $target_folder.$temp_code.'_'.$_FILES['FILE_UPLOAD']['name'];
				if(@move_uploaded_file($_FILES['FILE_UPLOAD']['tmp_name'], $target_path))
				{
				$FILE_UPLOAD = $temp_code.'_'.$_FILES['FILE_UPLOAD']['name'];
				}
				else
				{
				echo "<script langauge=\"javascript\">alert(\"Sorry you can not upload this  file.\");location.href='agent_registration.php?action=add';</script>";
				return;
				}
			 }else
			 {
			 echo "<script langauge=\"javascript\">alert(\"Sorry you have select Invalid file.\");location.href='agent_registration.php?action=add';</script>";
			 }		
		}
		
		$query=mysqli_query($conn,"select * from kp_agent_master where EMAIL='$EMAIL' or company_pan_no='$company_pan_no' or MOBILE='$MOBILE'");
	    ...
	    {
		$sql="INSERT INTO `kp_agent_master` (`AGENT_NO`, `AGENT_NAME`,company_pan_no,company_gstn,company_type, `SHORT_NAME`, `MFCODE`, `USER_NAME`, `PASSWORD`, `ADDRESS1`, `ADDRESS2`, `ADDRESS3`, `ADDRESS4`, `STATE_ID`, `STATE`, `CITY_ID`, `CITY`, `PINCODE`, `COUNTRY_ID`, `COUNTRY`, `TELEPHONE1`, `TELEPHONE2`, `FAX`, `MOBILE`, `EMAIL`, `WEBSITE`, `CONTACT_PERSON1`, `DESIGNATION1`, `CONTACT_PERSON2`, `CHA_NO`, `DESIGNATION2`, `IEC_NO`, `REMARKS`, `STATUS`, `SPACE_SOFT_ID`, `SPACE_SOFT_NUMBER`, `SR_NO`, `SEZ`, `LOCATION`, `FILE`, `ENTERED_BY`, `ENTERED_ON`, `MODIFIED_ON`, `MODIFIED_BY`, `REASON`) VALUES ('$USER_NAME', '$AGENT_NAME', '$company_pan_no','$company_gstn','$company_type',NULL, NULL, '$USER_NAME', '$USER_NAME', '$ADDRESS1', '$ADDRESS2', '$ADDRESS3', '$ADDRESS4', '$STATE_ID', '$STATE', '$CITY_ID', '$CITY', '$PINCODE', '$COUNTRY_ID', '$COUNTRY', '$TELEPHONE1', '$TELEPHONE2', '$FAX', '$MOBILE', '$EMAIL', '$WEBSITE', '$CONTACT_PERSON1','$DESIGNATION1', '$CONTACT_PERSON2', '$CHA_NO', '$DESIGNATION2', '$IEC_NO', '$REMARKS', '1', '0', '0', '0', '$SEEPZ', '$LOCATION', '0', 'ADMIN', NOW(), NOW(), 'Admin', '$REASON')";
		...
		}else
		{
		  ...
		  $message = $message .= "AGENT Name : $AGENT_NAME<br>"; 
		  ...
		  $subject = "AGENT Registration For Kimberley Process"; 
...
<section class="py-5">
	<!-- <div class="banner_wrap mb">
		<ul class="d-flex breadcrumb">
    		...
    		<li class="active">Search Applications</li>
  		</ul>
	</div> -->
		
	<div class="container inner_container">
    
    <div class="bold_font text-center"> <div class="d-block"><img src="assets/images/gold_star.png"></div>Agent Registration </div>
		
		
	
			<div class="row justify-content-center">
            <div class="col-lg-10 ">
				<form class="box-shadow mb-5" method="post" name="from1" id="form1" enctype="multipart/form-data" onSubmit="return validate()">
...
<input type="hidden" name="action" id="action" value="ADD" />
 ...
 <div class="row">
 	<div class="col-md-6">
 		...
 		<label class="star">Agent Name:</label>
 		<input name="AGENT_NAME" id="AGENT_NAME" class="form-control" type="text" maxlength="40" />
 		</div>
 	...
 	<div class="col-md-6">
 		<div class="form-group">
            ...
            <input type="radio" id="company_type" name="company_type" value="14" class="mr-2"> Propritory 
            </label>
            <label for="Partnership" class="mr-3">
            <input type="radio" id="company_type" name="company_type" value="11" class="mr-2"> Partnership
            </label>
            <label for="Private" class="mr-3">
            <input type="radio" id="company_type" name="company_type" value="13" class="mr-2"> Private Ltd.
            </label>                          
            <label for="Public">
            <input type="radio" id="company_type" name="company_type" value="12" class="mr-2"> Public Ltd.
            </label>
 	...
 	<div class="col-md-6">
 		<div class="form-group">
 			<label class="star  d-block">GSTIN Holder Status</label>
 				...
 				<label for="gst_holder">
 					<input name="gst_holder1" id="gst_holder" value="Y" type="radio" class="gstholder_status_yes mr-3"/>Yes
 				</label>
 				<label for="gst_holder2">
 					<input name="gst_holder" id="gst_holder2" value="N" type="radio" class="gstholder_status_no mr-3"/>No
 				</label>
 	...
 	<div class="col-md-6">
 		<div class="form-group">
 			...
 			<input id="company_pan_no" name="company_pan_no" autocomplete="off" maxlength="10" class="search_app_bg_text form-control" type="text" />
 		</div>
 	</div>
 	<div class="col-md-6">
 		<div class="form-group">
 			...
 			<div id="gst_status">
 				<input id="company_gstn" name="company_gstn" autocomplete="off" maxlength="15" class="search_app_bg_text form-control" type="text" />
 			</div>
 		</div>
 	</div>
 	<div class="col-md-6">
 		<div class="form-group">
 			...
 			<input name="ADDRESS1" id="ADDRESS1" class="search_app_bg_text form-control" type="text" />
 		</div>
 	</div>
 	<div class="col-md-6">
 		<div class="form-group">
 			<label class="star">Address2</label>
 			<input name="ADDRESS2" id="ADDRESS2" class="search_app_bg_text form-control" type="text" />
 		</div>
 	</div>
 	<div class="col-md-6">
 		<div class="form-group">
 			<label>Address3</label>
 			<input name="ADDRESS3" id="ADDRESS3" class="search_app_bg_text form-control" type="text" />
 		</div>
 	</div>
    <div class="col-md-6">
    	<div class="form-group">
    		<label>Address4</label>
    		<input name="ADDRESS4" id="ADDRESS4" class="search_app_bg_text form-control" type="text" />
    	</div>
    </div>
    <div class="col-md-6">
    	<div class="form-group">
    ...
    <div class="col-md-6">
    	<div class="form-group">
    		...
    		<input name="PINCODE" id="PINCODE" class="form-control" type="text" maxlength="6" />
    	</div>
    </div>
    <div class="col-md-6">
    	<div class="form-group">
    ...
    <div class="col-md-6">
    	<div class="form-group">
    		...
    		<input name="MOBILE" id="MOBILE" class="form-control" type="text" maxlength="10" />
    	</div>
    </div>
    <div class="col-md-6">
    	<div class="form-group">
    ...
    <div class="col-md-6">
    	<div class="form-group">
    		<label class="d-block">Do You Belong To Seepz?</label>
   ...
   <div class="d-flex align-items-center mt-4">
    ...
    <a href="javascript: refreshCaptcha();"><i class="fa fa-refresh" aria-hidden="true"></i>
</a>
   </div>
