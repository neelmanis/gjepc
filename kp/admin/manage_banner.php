...
{
	$sql="delete from kp_banner_master where id='$_REQUEST[id]'";
...
if (($_REQUEST['action']=='active') && ($_REQUEST['id']!=''))
{
		...
		$id=$_REQUEST['id'];
		$sql="update kp_banner_master set status='$status' where id='$id'";
		if (!mysql_query($sql,$dbconn))
		{
			die('Error: ' . mysql_error());
		}
...
if ($_REQUEST['action']=='save')
{
	...
	//---------------------------------------- uplaod  newsletter pdf  -----------------------------------------------
		...
		$sql="INSERT INTO kp_banner_master (banner_position,banner_name,banner_image,url,target,post_date,status) VALUES ('$banner_position','$banner_name','$banner_image','$url','$target','$post_date','1')";
...
if (($_REQUEST['action']=='update')&&($_REQUEST['id']!=''))
{
	...
	//------------------------------------  Update Newsletter PDF ----------------------------------------------------------
		...
		if($_FILES['banner_image']['name'] != '')
		{
		  //Unlink the previuos image
		   ...
		   $qpreviousimg=mysql_query("select banner_image from kp_banner_master where id='$id'");
		   ...
		   unlink($filename);
			...
			if (($_FILES["banner_image"]["type"] == "image/gif") || ($_FILES["banner_image"]["type"] == "image/jpeg") || ($_FILES["banner_image"]["type"] == "image/pjpeg") ||($_FILES["banner_image"]["type"] == "image/png"))
			{
				...
				{
					$banner_image = $temp_code.'_'.$_FILES['banner_image']['name'];
					$sql="update kp_banner_master set banner_image='$banner_image' where id='$id'";
	...
	$sql="update kp_banner_master set banner_position='$banner_position',banner_name='$banner_name',url='$url',target='$target',order_no='$order_no' where id='$id'";
...
<div id="main">
	<div class="content">
    	<div class="content_head"><a href="manage_banner.php?action=add"><div class="content_head_button">Add Banner</div></a> <a href="manage_banner.php?action=view"><div class="content_head_button">View Banner</div></a> </div>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="detail_txt" >
    ...
    <?php 
    $order_by = isset($_REQUEST['order_by'])?$_REQUEST['order_by']:'banner_position,order_no';
    $asc_desc = isset($_REQUEST['asc_desc'])?$_REQUEST['asc_desc']:'desc';
    $attach = " order by ".$order_by." ".$asc_desc." ";
    
    $i=1;
	$result = mysql_query("SELECT * FROM kp_banner_master where 1 ".$attach." ");
    $rCount=0;
    ...
    ?>  
 	<tr <?php if($i%2==0){echo "bgcolor='#CCCCCC'";}?>>
        <td><?php echo $i;?></td>
        <td><?php echo $row['banner_position']; ?></td>
        <td><?php echo $row['banner_name']; ?></td>
        <td><?php if($row['banner_image']!=""){?><img src="Banner/<?php echo $row['banner_image'];?>" width="100px" height="50px" /><?php } ?></td>
        ...
        <td><?php if($row['status'] == 1) { ?> <a style="text-decoration:none;" href="manage_banner.php?id=<?php echo $row['id']; ?>&status=0&action=active" onClick="return(window.confirm('Are you sure you want to Deactivate.'));"><img src="images/inactive.png" border="0" title="Inactive"/></a><?php } else { ?><a style="text-decoration:none;" href="manage_banner.php?id=<?php echo $row['id']; ?>&status=1&action=active" onClick="return(window.confirm('Are you sure you want to Activate.'));"><img src="images/active.png" border="0" title="Active"/></a><?php } ?></td>
...
if(($_REQUEST['action']=='add') || ($_REQUEST['action']=='edit'))
{
  ...
  {
		$action='update';
		$result2 = mysql_query("SELECT *  FROM kp_banner_master  where id='$_REQUEST[id]'");
