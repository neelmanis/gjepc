...
{
	$sql="delete from category_master where id='$_REQUEST[id]'";
...
if (($_REQUEST['action']=='active') && ($_REQUEST['id']!=''))
{
		...
		$id=$_REQUEST['id'];
		$sql="update category_master set status='$status' where id='$id'";
		if (!mysql_query($sql,$dbconn))
		{
			die('Error: ' . mysql_error());
		}
...
if ($_REQUEST['action']=='save')
{
	//$post_date = date('Y-m-d');
	 ...
	 $category=addslashes($_REQUEST['cat_name']);
	 //$dept=addslashes($_REQUEST['dept']);
				
						$sql="INSERT INTO category_master (name,status,post_date) VALUES ('$category','1','$post_date')";	
						if (!mysql_query($sql,$dbconn))
						{
							die('Error: ' . mysql_error());
						}
...
<div id="main">
	<div class="content">
    	<div class="content_head"><a href="manage_category.php?action=add"><div class="content_head_button">Add Category</div></a> <a href="manage_category.php?action=view"><div class="content_head_button">View Category</div></a> </div>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="detail_txt" >
	...
	$result = mysql_query("SELECT * FROM category_master where 1".$attach." ");
    $rCount=0;
    ...
    ?>  
 	<tr <?php if($i%2==0){echo "bgcolor='#CCCCCC'";}?>>
        ...
        <td><?php if($row['status'] == 1) { ?> <a style="text-decoration:none;" href="manage_category.php?id=<?php echo $row['id']; ?>&status=0&action=active" onClick="return(window.confirm('Are you sure you want to Deactivate.'));"><img src="images/inactive.png" border="0" title="Inactive"/></a><?php } else { ?><a style="text-decoration:none;" href="manage_category.php?id=<?php echo $row['id']; ?>&status=1&action=active" onClick="return(window.confirm('Are you sure you want to Activate.'));"><img src="images/active.png" border="0" title="Active"/></a><?php } ?></td>
        
        <!--<td><a href="manage_admin.php?action=edit&id=<?php echo $row['id']; ?>"><img src="images/edit.png" title="Edit" border="0" /></a></td>-->
        
        <td><a href="manage_category.php?action=del&id=<?php echo $row['id']; ?>" onClick="return(window.confirm('Are you sure you want to delete?'));"><img src="images/delete.png" title="Delete" border="0" /></a></td>
 	</tr>
