...
{
	$sql="delete from  mis_master where id='$_REQUEST[id]'";
	
	if (!mysql_query($sql,$dbconn))
	{
		die('Error: ' . mysql_error());
	}
	echo"<meta http-equiv=refresh content=\"0;url=manage_mis.php?action=view\">";
}
...
if (($_REQUEST['action']=='active') && ($_REQUEST['id']!=''))
{
		...
		$id=$_REQUEST['id'];
		$sql="update mis_master set status='$status' where id='$id'";
		if (!mysql_query($sql,$dbconn))
		{
			die('Error: ' . mysql_error());
		}
...
if ($_REQUEST['action']=='save')
{
	//$post_date = date('Y-m-d');
	 ...
	 $report_name=addslashes($_REQUEST['report_name']);
	 $dept=addslashes($_REQUEST['dept']);
		$export_attachment = '';
		$target_folder = '../images/mis/';
		$path_parts = "";
		$ext="";
		$target_path = "";
		$filetoupload="";
			...
			{	
				...
				{
					$target_path = $target_folder.$_FILES['file_name']['name'];
					if(@move_uploaded_file($_FILES['file_name']['tmp_name'], $target_path))
					{
						$file_name = $_FILES['file_name']['name'];
						
						$sql="INSERT INTO mis_master (report_name,file_name,dept,status,post_date) VALUES ('$report_name','$file_name','$dept','1','$post_date')";	
					...
					{
						echo "<script langauge=\"javascript\">alert(\"Sorry you can not upload this  file.\");location.href='manage_mis.php?action=add';</script>";
					return;
					}
			 }else
			 {
			 echo "<script langauge=\"javascript\">alert(\"Sorry you have select Invalid file.\");location.href='manage_mis.php?action=add';</script>";
			 }	
			}
...
function checkdata()
{
	...
	{
		alert("Please Enter Reports Name");
...
<div id="main">
	<div class="content">
    	<div class="content_head"><a href="manage_mis.php?action=add"><div class="content_head_button">Add Reports</div></a> <a href="manage_mis.php?action=view"><div class="content_head_button">View Reports</div></a> </div>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="detail_txt" >
	...
	$result = mysql_query("SELECT * FROM mis_master where 1".$attach." ");
    $rCount=0;
    ...
    ?>  
 	<tr <?php if($i%2==0){echo "bgcolor='#CCCCCC'";}?>>
        ...
        <td><?php if($row['status'] == 1) { ?> <a style="text-decoration:none;" href="manage_mis.php?id=<?php echo $row['id']; ?>&status=0&action=active" onClick="return(window.confirm('Are you sure you want to Deactivate.'));"><img src="images/inactive.png" border="0" title="Inactive"/></a><?php } else { ?><a style="text-decoration:none;" href="manage_mis.php?id=<?php echo $row['id']; ?>&status=1&action=active" onClick="return(window.confirm('Are you sure you want to Activate.'));"><img src="images/active.png" border="0" title="Active"/></a><?php } ?></td>
        
        <!--<td><a href="manage_admin.php?action=edit&id=<?php echo $row['id']; ?>"><img src="images/edit.png" title="Edit" border="0" /></a></td>-->
        
        <td><a href="manage_mis.php?action=del&id=<?php echo $row['id']; ?>" onClick="return(window.confirm('Are you sure you want to delete?'));"><img src="images/delete.png" title="Delete" border="0" /></a></td>
 	</tr>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0"  >
    ...
    <tr>
    <td class="content_txt">Reports Name <span class="star">*</span></td>
    <td><input type="text" name="report_name" id="report_name" title="Please enter your name" class="show-tooltip input_txt" />    </td>
    </tr>
  
   <tr>
    <td class="content_txt">Department. <span class="star">*</span></td>
    <td><input type="text" name="dept" id="dept" class="input_txt" />
           </td>
    </tr>
    <tr>
    <td class="content_txt">Browse. <span class="star">*</span></td>
    <td><input type="file" name="file_name" id="file_name" class="input_txt" />
           </td>
    </tr>
