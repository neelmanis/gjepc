...
{
	$sql="delete from policy_master where id='$_REQUEST[id]'";
	
	if (!mysql_query($sql,$dbconn))
	{
		die('Error: ' . mysql_error());
	}
	echo"<meta http-equiv=refresh content=\"0;url=manage_policies.php?action=view\">";
}
...
if (($_REQUEST['action']=='active') && ($_REQUEST['id']!=''))
{
		...
		$sql="update policy_master set status='$status' where id='$id'";
		if (!mysql_query($sql,$dbconn))
		{
			die('Error: ' . mysql_error());
		}
...
if ($_REQUEST['action']=='save')
{
	//$post_date = date('Y-m-d');
	 ...
	 $p_name=filter_input_string($_REQUEST['policy_name']);
		$export_attachment = '';
		$target_folder = '../policy/';
		$path_parts = "";
		$ext="";
		$target_path = "";
		$filetoupload="";
			...
			{	
				if (($_FILES["p_file"]["type"] == "application/pdf"))
				{
					...
					if(@move_uploaded_file($_FILES['p_file']['tmp_name'], $target_path))
					{
						...
						$sql="INSERT INTO policy_master (p_name,p_file,status,post_date) VALUES ('$p_name','$p_file','1','$post_date')";	
						if (!mysql_query($sql,$dbconn))
						{
							die('Error: ' . mysql_error());
						}
					...
					{
						echo "<script langauge=\"javascript\">alert(\"Sorry you can not upload this  file.\");location.href='manage_policies.php?action=add';</script>";
					return;
					}
			 }else
			 {
			 echo "<script langauge=\"javascript\">alert(\"Sorry you have select Invalid file.\");location.href='manage_policies.php?action=add';</script>";
			 }	
			}
...
<div id="main">
	<div class="content">
    	<div class="content_head"><a href="manage_policies.php?action=add"><div class="content_head_button">Add Policies</div></a> <a href="manage_policies.php?action=view"><div class="content_head_button">View Policies</div></a> </div>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="detail_txt" >
	...
	$result = mysql_query("SELECT * FROM policy_master where 1".$attach." ");
    $rCount=0;
    ...
    ?>  
 	<tr <?php if($i%2==0){echo "bgcolor='#CCCCCC'";}?>>
        ...
        <td><?php if($row['status'] == 1) { ?> <a style="text-decoration:none;" href="manage_policies.php?id=<?php echo $row['id']; ?>&status=0&action=active" onClick="return(window.confirm('Are you sure you want to Deactivate.'));"><img src="images/inactive.png" border="0" title="Inactive"/></a><?php } else { ?><a style="text-decoration:none;" href="manage_policies.php?id=<?php echo $row['id']; ?>&status=1&action=active" onClick="return(window.confirm('Are you sure you want to Activate.'));"><img src="images/active.png" border="0" title="Active"/></a><?php } ?></td>
        
        <!--<td><a href="manage_admin.php?action=edit&id=<?php echo $row['id']; ?>"><img src="images/edit.png" title="Edit" border="0" /></a></td>-->
        
        <td><a href="manage_policies.php?action=del&id=<?php echo $row['id']; ?>" onClick="return(window.confirm('Are you sure you want to delete?'));"><img src="images/delete.png" title="Delete" border="0" /></a></td>
 	</tr>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0"  >
    ...
    <tr>
    <td class="content_txt">Policy Name <span class="star">*</span></td>
    <td><input type="text" name="policy_name" id="policy_name" title="Please enter your name" class="show-tooltip input_txt" />    </td>
    </tr>
  
   
    <tr>
    <td class="content_txt">Browse. <span class="star">*</span></td>
    <td><input type="file" name="p_file" id="p_file" class="input_txt" />
           </td>
    </tr>
    
    <tr>
    <td class="content_txt">Post Date <span class="star">*</span></td>
    <td><input type="date" name="p_date" id="p_date" class="input_txt"  /></td>
    </tr>
