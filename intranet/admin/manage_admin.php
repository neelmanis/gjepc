...
if ($_REQUEST['action']=='save')
{
	...
	$result = @mysql_query("select * from admin_master where email_id='$email_id' and password='$password'");
	...
	{
		$sql="INSERT INTO admin_master (contact_name, address,mobile_no,email_id,password,role,admin_access,region_id,status,post_date) VALUES ('$contact_name', '$address','$mobile_no','$email_id','$password','$role','$admin_access_new','$region_id','1','$post_date')";	
		if (!mysql_query($sql,$dbconn))
		{
			die('Error: ' . mysql_error());
		}
...
if (($_REQUEST['action']=='update')&&($_REQUEST['id']!=''))
{
	...
	$sql="update admin_master set contact_name='$contact_name',address='$address', mobile_no='$mobile_no',email_id='$email_id',password='$password',role='$role',admin_access='$admin_access_new',region_id='$region_id' where id='$id'";
...
if(($_REQUEST['action']=='add') || ($_REQUEST['action']=='edit'))
{
  ...
  {
		$action='update';
		$result2 = mysql_query("SELECT *  FROM admin_master  where id='$_REQUEST[id]'");
...
<table width="100%" border="0" cellspacing="0" cellpadding="0"  >
	...
	?>
        ...
        <tr>
          ...
          <input type="checkbox" name="admin_access[]" id="admin_access[]" value="2"  <?php if(preg_match('/2/',$admin_access)){ echo ' checked="checked"'; } ?> />Manage Policies
          <input type="checkbox" name="admin_access[]" id="admin_access[]" value="3"  <?php if(preg_match('/3/',$admin_access)){ echo ' checked="checked"'; } ?> />Manage Gallary
          <input type="checkbox" name="admin_access[]" id="admin_access[]" value="4"  <?php if(preg_match('/4/',$admin_access)){ echo ' checked="checked"'; } ?> />Manage Polls & Forums
          <!--<input type="checkbox" name="admin_access[]" id="admin_access[]" value="5"  <?php if(preg_match('/5/',$admin_access)){ echo ' checked="checked"'; } ?> />Manage Traning Calender-->
          </td>
        </tr>
        </tbody>
    <tr>
