...
<tr style="font-size:20px; color:#CC9966; " ><td colspan="8"><b>Q.   <?php echo $row['question'];?></b></td></tr> 
    ...
    {	
		...
		//$pollans_master=mysql_fetch_array($total_result);
		 $total_cnt=mysql_num_rows($total_result);
		 
		while($pollans_master=mysql_fetch_array($total_result))
		{
			/************option count************/
		$answer_query="select * from pollanswer_master where q_id=".$row['id']." and o_id=".$row_op['id'];
		$result_ans=mysql_query($answer_query);
	
		$ans_cnt=mysql_num_rows($result_ans);
    ?> 
    
 	<tr <?php if($i%2==0){echo "bgcolor='#CCCCCC'";}?>>
        <td><?php echo $i;?></td>
		<td><?php echo getusername($pollans_master['user_id']); ?></td>
        <td><?php echo getoptionName($pollans_master['o_id']); ?></td>
