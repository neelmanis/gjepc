...
{
	$sql="delete from videos_master where id='$_REQUEST[id]'";
...
if (($_REQUEST['action']=='active') && ($_REQUEST['id']!=''))
{
		...
		$id=$_REQUEST['id'];
		$sql="update videos_master set status='$status' where id='$id'";
		if (!mysql_query($sql,$dbconn))
		{
			die('Error: ' . mysql_error());
		}
	...
	//;
	 $p_name=addslashes($_REQUEST['name']);
		...
		$filetoupload="";
			...
			{	
				if (($_FILES["p_file"]["type"] == "video/flv") || ($_FILES["p_file"]["type"] == "video/mp4") || ($_FILES["p_file"]["type"] == "audio/mp3") || ($_FILES["p_file"]["type"] == "audio/wma") )
				{
					if($_FILES['p_file']['size']<'52428800')
					{
					...
					{
						echo "<script langauge=\"javascript\">alert(\"Sorry you have select  file whose size exceeds 50MB.\");location.href='manage_videos.php?action=add';</script>";
					}
...
function checkdata()
{
	...
	{
		alert('Select Only mp4,mp3,flv,wma');
		document.getElementById('p_file').focus();
		return false;
	}
...
<div id="main">
	<div class="content">
    	<div class="content_head"><a href="manage_videos.php?action=add"><div class="content_head_button">Add Video</div></a> <a href="manage_videos.php?action=view"><div class="content_head_button">View Videos</div></a> </div>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="detail_txt" >
	...
	$result = mysql_query("SELECT * FROM videos_master where 1".$attach." ");
    $rCount=0;
    ...
    ?>  
 	<tr <?php if($i%2==0){echo "bgcolor='#CCCCCC'";}?>>
        ...
        <td><?php if($row['status'] == 1) { ?> <a style="text-decoration:none;" href="manage_videos.php?id=<?php echo $row['id']; ?>&status=0&action=active" onClick="return(window.confirm('Are you sure you want to Deactivate.'));"><img src="images/inactive.png" border="0" title="Inactive"/></a><?php } else { ?><a style="text-decoration:none;" href="manage_videos.php?id=<?php echo $row['id']; ?>&status=1&action=active" onClick="return(window.confirm('Are you sure you want to Activate.'));"><img src="images/active.png" border="0" title="Active"/></a><?php } ?></td>
        
        <!--<td><a href="manage_admin.php?action=edit&id=<?php echo $row['id']; ?>"><img src="images/edit.png" title="Edit" border="0" /></a></td>-->
        
        <td><a href="manage_videos.php?action=del&id=<?php echo $row['id']; ?>" onClick="return(window.confirm('Are you sure you want to delete?'));"><img src="images/delete.png" title="Delete" border="0" /></a></td>
 	</tr>
...
if(($_REQUEST['action']=='add') || ($_REQUEST['action']=='edit'))
{
  ...
  {
		$action='update';
		$result2 = mysql_query("SELECT *  FROM videos_master  where id='$_REQUEST[id]'");
