...
{
	$sql="delete from training_calender_master where id='$_REQUEST[id]'";
	
	if (!mysql_query($sql,$dbconn))
	{
		die('Error: ' . mysql_error());
	}
	echo"<meta http-equiv=refresh content=\"0;url=manage_traning_calender.php?action=view\">";
}
...
if (($_REQUEST['action']=='active') && ($_REQUEST['id']!=''))
{
		...
		$id=$_REQUEST['id'];
		$sql="update training_calender_master set status='$status' where id='$id'";
		if (!mysql_query($sql,$dbconn))
		{
			die('Error: ' . mysql_error());
		}
			...
			{	
				if (($_FILES["t_file"]["type"] == "application/pdf"))
				{
					...
					if(@move_uploaded_file($_FILES['t_file']['tmp_name'], $target_path))
					{
						...
						$sql="INSERT INTO training_calender_master (t_name,t_file,status,post_date) VALUES ('$t_name','$t_file','1','$post_date')";	
						if (!mysql_query($sql,$dbconn))
						{
							die('Error: ' . mysql_error());
						}
					...
					{
						echo "<script langauge=\"javascript\">alert(\"Sorry you can not upload this  file.\");location.href='manage_traning_calender.php?action=add';</script>";
					return;
					}
			 }else
			 {
			 echo "<script langauge=\"javascript\">alert(\"Sorry you have select Invalid file.\");location.href='manage_traning_calender.php?action=add';</script>";
			 }	
			}
...
<div id="main">
	<div class="content">
    	<div class="content_head"><a href="manage_traning_calender.php?action=add"><div class="content_head_button">Add Traning alender</div></a> <a href="manage_traning_calender.php?action=view"><div class="content_head_button">View Traning calender</div></a> </div>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="detail_txt" >
	...
	$result = mysql_query("SELECT * FROM training_calender_master where 1".$attach." ");
    $rCount=0;
    ...
    ?>  
 	<tr <?php if($i%2==0){echo "bgcolor='#CCCCCC'";}?>>
        ...
        <td><?php if($row['status'] == 1) { ?> <a style="text-decoration:none;" href="manage_traning_calender.php?id=<?php echo $row['id']; ?>&status=0&action=active" onClick="return(window.confirm('Are you sure you want to Deactivate.'));"><img src="images/inactive.png" border="0" title="Inactive"/></a><?php } else { ?><a style="text-decoration:none;" href="manage_traning_calender.php?id=<?php echo $row['id']; ?>&status=1&action=active" onClick="return(window.confirm('Are you sure you want to Activate.'));"><img src="images/active.png" border="0" title="Active"/></a><?php } ?></td>
        
        <!--<td><a href="manage_admin.php?action=edit&id=<?php echo $row['id']; ?>"><img src="images/edit.png" title="Edit" border="0" /></a></td>-->
        
        <td><a href="manage_traning_calender.php?action=del&id=<?php echo $row['id']; ?>" onClick="return(window.confirm('Are you sure you want to delete?'));"><img src="images/delete.png" title="Delete" border="0" /></a></td>
 	</tr>
