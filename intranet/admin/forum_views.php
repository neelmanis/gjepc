...
{
	$sql="delete from forum_answer where id='$_REQUEST[id]'";
	
	if (!mysql_query($sql,$dbconn))
	{
		die('Error: ' . mysql_error());
	}
	echo"<meta http-equiv=refresh content=\"0;url=forum_views.php?action=view&q_id=".$_REQUEST['q_id']."\">";
}
...
if (($_REQUEST['action']=='active') && ($_REQUEST['id']!=''))
{
		...
		$id=$_REQUEST['id'];
		$q_id=$_REQUEST['q_id'];
		$sql="update forum_answer set status='$status' where id='$id'";
		if (!mysql_query($sql,$dbconn))
		{
			die('Error: ' . mysql_error());
		}
		echo"<meta http-equiv=refresh content=\"0;url=forum_views.php?action=view&q_id=$q_id\">";
}
...
<div id="main">
	<div class="content">
    	<div class="content_head"><a href="manage_suggestions.php?action=view"><div class="content_head_button">View suggestions</div></a> <a href="manage_forum.php?action=view"><div class="content_head_button">Back To Forum</div></a></div>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="detail_txt" >
	...
	$result = mysql_query("SELECT * FROM forum_answer where q_id=$q_id ".$attach." ");
    $rCount=0;
    ...
    ?>  
 	<tr <?php if($i%2==0){echo "bgcolor='#CCCCCC'";}?>>
      ...
      <td>
        ...
        <td><?php if($row['status'] == 1) { ?> <a style="text-decoration:none;" href="forum_views.php?id=<?php echo $row['id']; ?>&status=0&action=active&q_id=<?php echo $row['q_id'];?>" onClick="return(window.confirm('Are you sure you want to Deactivate.'));"><img src="images/inactive.png" border="0" title="Inactive"/></a><?php } else { ?><a style="text-decoration:none;" href="forum_views.php?id=<?php echo $row['id']; ?>&status=1&action=active&q_id=<?php echo $row['q_id']; ?>" onClick="return(window.confirm('Are you sure you want to Activate.'));"><img src="images/active.png" border="0" title="Active"/></a><?php } ?></td>
        <!--<td><a href="forum_views.php?action=edit&id=<?php echo $row['id']; ?>"><img src="images/edit.png" title="Edit" border="0" /></a></td>-->
        
        <td><a href="forum_views.php?action=del&id=<?php echo $row['id']; ?>&q_id=<?php echo $row['q_id']; ?>" onClick="return(window.confirm('Are you sure you want to delete?'));"><img src="images/delete.png" title="Delete" border="0" /></a></td>
 	</tr>
