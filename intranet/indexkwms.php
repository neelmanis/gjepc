...
$uid = $_SESSION['user_id'];
  $query_pass = "select * from employee_details where id=$uid";
 
 $result_pass = mysql_query($query_pass) or trigger_error(mysql_error().$sql);
 $row_pass = mysql_fetch_array($result_pass);


 if($row_pass['change_pass_status']==0)
 {
...
<div class="container_wrap">
    ...
    <div class="announcement">
        ...
        <ul>
            ...
            <?php
	     $query ="select * from employee_details order by dob_str asc";
		...
		//echo $written_month = date("F", strtotime("date]"));
			...
			if($date==$ans[0] && $month==$ans[1])
			{
				...
				//echo "<li>".$birth_date."<br />";	
				echo "<li><span>Happy Birthday-".$row['employee_name']."</span></li>";
				
			}
			else
			{
				
				$upcoming_date = $date+"7";
				if($ans[0]<$upcoming_date && $ans[0]>$date && $month==$ans[1])
				{
				if($flag==0)
				{
				...
				$up_date = $ans[0];
				echo "<li>".$ans[0].ordinal($up_date)." ".$ans[1]."<br />".$row['employee_name']."<br />";	
				}
