<?php 
$pageTitle = "Gems And Jewellery Industry In India | Region-wise all commodities Import - GJEPC India";
...
include 'functions.php';
		...
		if($_REQUEST['Reset']=="Reset")
		{
		  ...
		  header("Location: region-wise-all-commodities-import.php");
		} else {
    ...
    <div class="row justify-content-center" >
			...
			<input type="hidden" name="search_type" id="search_type" value="SEARCH"/>
                ...
                <div class="form-group col-12 mb-4">
                    <p class="blue text-center">IMPORTS - REGION-WISE ALL COMMODITIES</p>
                </div>
                ...
                <div class="col-md-11 mb-3">
                    <div class="row">
                        ...
                        <div class="col-8">                           
							<select name="region" id="region" class="form-control search_bar" placeholder="Search Region">
		...
		/*	if($cur_month < 4) { Comment For New Year*/
		if($cur_month > 4) {
		...
		if(isset($commoditySel) && $commoditySel=="allCommodity")
		{
			if($commodity_level!='') 
			{				
				$sql="SELECT a.`financial_year`, a.`country_id`,a.`hs_code_id`, SUM(REPLACE(a.inr_value, ',', '')) as total_inr, `dollar_value`, SUM(REPLACE(a.dollar_value, ',', '')) as total_usd,b.*,c.`hs_code`,c.`level` FROM `statistics_integration_data` a ,statistics_integration_country_master b ,statistics_integration_hscode_master c WHERE 1 AND a.financial_year='$financial_year' AND a.trade_type='$trade_type'";
				...
				$sqlx="select hs_code from statistics_integration_hscode_master where level='$commodity_level'";
				...
				if($num>0){
						...
						$myArray[] = $row['hs_code'];
						$codes = $row['hs_code'];
						$sql.=" a.hs_code_id LIKE '$codes%' ";
						if($i<$num)
								$sql.=" OR ";
						$i++;
						}
						$sql.=" ) ";
						$sql.="and a.country_id=b.country_name and b.region_code='$region' and  a.hs_code_id=c.hs_code";
						$sql2 = $sql;
		...
		if(isset($commoditySel) && $commoditySel=="SpecificCommodity")
		{
			...
			$sql = "SELECT a.*,b.*,c.*,sum(REPLACE(a.inr_value, ',', '')) as `total_inr`,sum(REPLACE(a.dollar_value, ',', '')) as `total_usd` FROM `statistics_integration_data` a , statistics_integration_country_master b,statistics_integration_hscode_master c WHERE 1 and a.financial_year='$financial_year' and a.trade_type='Import' and a.country_id=b.country_name and a.hs_code_id=c.hs_code and c.level='8' and b.region_code='$region' group by c.commodity_category order by $orderBy desc";
			
			$sql2 = $sql;
			$replacedQuery = str_replace("group by c.commodity_category", "", $sql2);
			...
			$sql = "SELECT a.*,b.*,c.*,sum(REPLACE(a.inr_value, ',', '')) as `total_inr`,sum(REPLACE(a.dollar_value, ',', '')) as `total_usd` FROM `statistics_integration_data` a , statistics_integration_country_master b,statistics_integration_hscode_master c WHERE 1 and a.financial_year='$financial_year' and a.trade_type='Import' and a.country_id=b.country_name and a.hs_code_id=c.hs_code and c.level='8' and b.region_code='$region' and c.commodity_category='".$_POST['commodity']."'";
		...
		<div class="col-12">
                        <div class="row py-3" style="border-top: 1px solid #ccc">
                            <div class="col-12"><h2 class="title text-center mb-1"><span class="d-table mx-auto mb-3" style="width:35px;"><img src="assets/images/black_star.png" class="img-fluid d-block"></span>IMPORTS - REGION-WISE ALL COMMODITIES</h2></div>
                            ...
                            <div class="col-12">
								...
								<div class="row justify-content-center mt-4">
                                   		<div class="col-md-9 searchResultwrap">
                                        	<ul>
                                                	...
                                                	<div class="row">
                                                        ...
                                                        <div class="col-md-8 sResult">
															<?php 
																...
																{
																echo getCountryRegion($_POST['region'],$conn);								
																}
															?>
                                                        </div>
                                                    </div>
                                                ...
                                                <li>
                                                	<div class="row">
                                                    	<div class="col-md-4 sCategory">Specific Commodity </div>
                                                        <div class="col-md-8 sResult">
															<?php if($_POST['commodity']=="all"){ echo "All Category"; } else { echo $_POST['commodity']; } ?>
                                                        </div>
                                                    </div>
                                                </li>
												<?php }
                                ...
                                <button onclick="exportToExcel('tblexportData', 'region-wise-all-commodities-import')" class="cta gold_btn" title="Export To Excel"><i class="fa fa-file-excel-o" style="font-size: 40px;"></i></button>
                                </div>
                            </div>
							<div class="col-12">
			...
			/* Same */		if(getFinancialYear($conn)==$financial_year){ $sameYear = "YES"; } else { $sameYear = "NO"; }			
									while($row = $resultQ->fetch_assoc())
									{
										if($currency!="")
										{
											if($currency=="INR")
											{
											$getCurrentYearData  = getCurrentregionwiseAllINR_i($_REQUEST['financial_year'],$_REQUEST['region'],$row['hs_code'],$conn);
											...
											$getPreviousYearData = getLastYearRegionINR_i($last_finyr,$_REQUEST['region'],$row['hs_code'],$sameYear,$currentMonth,$conn);
											...
											$getCurrentYearData  = getCurrentregionwiseAllUSD_i($_REQUEST['financial_year'],$_REQUEST['region'],$row['hs_code'],$conn);
										...
										//	$newTotal_Paisa  = round($getCurrentYearData/$_POST['currency_usd'],2);
											...
											$getPreviousYearData = getLastYearRegionUSD_i($last_finyr,$_REQUEST['region'],$row['hs_code'],$sameYear,$currentMonth,$conn);
			...
			/* Same */		if(getFinancialYear($conn)==$financial_year){ $sameYear = "YES"; } else { $sameYear = "NO"; }				
									...
									{	//echo '<pre>'; print_r($row);								
										if($currency!="")
										{
											...
											$getPreviousYearData = getLastRegionwisecommodityINR_i($last_finyr,$row['commodity_category'],$_REQUEST['region'],$sameYear,$currentMonth,$conn);
											$getPreviousData	 = round($getPreviousYearData/$_POST['currency_inr'],2);
											$sub_lastYear_Paisa  = round($sub_lastYear_Paisa + $getPreviousYearData/$_POST['currency_inr'],2);											
										...
										//	$newTotal_Paisa  = round($row['total_usd']/$_POST['currency_usd'],2);	
											...
											$getPreviousYearData = getLastRegionwisecommodityUSD_i($last_finyr,$row['commodity_category'],$_REQUEST['region'],$sameYear,$currentMonth,$conn);
											$sub_lastYear_Paisa  = round($sub_lastYear_Paisa + $getPreviousYearData/$_POST['currency_usd'],2);
										//	$getPreviousData	 = round($getPreviousYearData/$_POST['currency_usd'],2);
											$number  = $getPreviousYearData/$_POST['currency_usd'];
											$getPreviousDataNOround  = number_format($number, 12, '.', '');
