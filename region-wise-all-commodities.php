<?php 
$pageTitle = "Gems And Jewellery Industry In India | Region-wise all commodities - GJEPC India";
...
include 'functions.php';
		...
		if($_REQUEST['Reset']=="Reset")
		{
		  ...
		  header("Location: region-wise-all-commodities.php");
		} else {
    ...
    <div class="row justify-content-center" >
			...
			<input type="hidden" name="search_type" id="search_type" value="SEARCH"/>
                ...
                <div class="form-group col-12 mb-4">
                    <p class="blue text-center">EXPORTS - REGION-WISE ALL COMMODITIES</p>
                </div>
		...
		<div class="col-12">
                        <div class="row py-3" style="border-top: 1px solid #ccc">
                            <div class="col-12"><h2 class="title text-center mb-1"><span class="d-table mx-auto mb-3" style="width:35px;"><img src="assets/images/black_star.png" class="img-fluid d-block"></span>EXPORTS - REGION-WISE ALL COMMODITIES</h2></div>
                            ...
                            <div class="col-12">
								...
								<div class="row justify-content-center mt-4">
                                   ...
                                   <button onclick="exportToExcel('tblexportData', 'region-wise-all-commodities-export')" class="cta gold_btn" title="Export To Excel"><i class="fa fa-file-excel-o" style="font-size: 40px;"></i></button>
                                   </div>
                            </div>
							<div class="col-12">
					...
					/* Same */		if(getFinancialYear($conn)==$financial_year){ $sameYear = "YES";	} else { $sameYear = "NO"; }		
									while($row = $resultQ->fetch_assoc())
									{
										if($currency!="")
										{
											if($currency=="INR")
											{
											$getCurrentYearData  = getCurrentregionwiseAllINR($_REQUEST['financial_year'],$_REQUEST['region'],$row['hs_code'],$conn);
											...
											$getPreviousYearData = getLastYearRegionINR($last_finyr,$_REQUEST['region'],$row['hs_code'],$sameYear,$currentMonth,$conn);
											...
											$getCurrentYearData  = getCurrentregionwiseAllUSD($_REQUEST['financial_year'],$_REQUEST['region'],$row['hs_code'],$conn);
										...
										//	$newTotal_Paisa  = round($getCurrentYearData/$_POST['currency_usd'],2);
											...
											$getPreviousYearData = getLastYearRegionUSD($last_finyr,$_REQUEST['region'],$row['hs_code'],$sameYear,$currentMonth,$conn);
				...
				/* Same */		if(getFinancialYear($conn)==$financial_year){ $sameYear = "YES";	} else { $sameYear = "NO"; }			
									while($row = $resultQ->fetch_assoc())
									{
										if($currency!="")
										{
											...
											$getPreviousYearData = getLastRegionwisecommodityINR($last_finyr,$row['commodity_category'],$_REQUEST['region'],$sameYear,$currentMonth,$conn);
											$getPreviousData	 = round($getPreviousYearData/$_POST['currency_inr'],2);
											$sub_lastYear_Paisa  = round($sub_lastYear_Paisa + $getPreviousYearData/$_POST['currency_inr'],2);											
										...
										//	$newTotal_Paisa  = round($row['total_usd']/$_POST['currency_usd'],2);
											...
											$getPreviousYearData = getLastRegionwisecommodityUSD($last_finyr,$row['commodity_category'],$_REQUEST['region'],$sameYear,$currentMonth,$conn);
											$sub_lastYear_Paisa  = round($sub_lastYear_Paisa + $getPreviousYearData/$_POST['currency_usd'],2);
										//	$getPreviousData	 = round($getPreviousYearData/$_POST['currency_usd'],2);
											$number  = $getPreviousYearData/$_POST['currency_usd'];
											$getPreviousDataNOround  = number_format($number, 12, '.', '');
