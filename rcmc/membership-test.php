...
if(isset($_REQUEST['registration_id']) && $_REQUEST['registration_id']!='')
{
	...
	$result_user = mysql_query($fetch_user);
	$result_panel = mysql_query($fetch_panel);
	$result_pin = mysql_query($fetch_pin);
	$result_add = mysql_query($fetch_address);
	$result_transaction = mysql_query($fetch_transaction);
	...
	$rows_panel = mysql_fetch_array($result_panel);
	$rows_pin = mysql_fetch_array($result_pin);
	$rows_address = mysql_fetch_array($result_add);
	$rows_transaction = mysql_fetch_array($result_transaction);
	...
	$company_name = strtoupper($rows_user['company_name']);
	...
	$state=strtoupper(getStateName($state));
	if($state == '')
	...
	$address = getRegionAddress($region_id);
	
	$reg_tel_no = getRegionTelno($region_id);
	$reg_email = getRegionEmail($region_id);
	$reg_fax = getRegionFax($region_id);
