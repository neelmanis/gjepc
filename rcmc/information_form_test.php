...
$bcountry = strtoupper(getFullCountryeName($branch_office_result['country']));
if($bcountry!=""){$bcountry=",".$bcountry;}else{$bcountry="";}
$bstate = strtoupper(getFullStateName($branch_office_result['state']));
...
$fcountry = strtoupper(getFullCountryeName($factory_office_result['country']));
if($fcountry!=""){$fcountry=",".$fcountry;}else{$fcountry="";}
$fstate = strtoupper(getFullStateName($factory_office_result['state']));
...
$person_office = mysql_query("SELECT name, father_name, address1, address2, address3, country, state, city, pincode, landline_no1, mobile_no, fax_no1, fax_no2, email_id FROM communication_address_master  WHERE registration_id = '$registration_id'  and type_of_address='$person_office'");
...
$pcountry = strtoupper(getFullCountryeName($person_office_result['country']));
if($pcountry!=""){$pcountry=",".$pcountry;}else{$pcountry="";}
$pstate = strtoupper(getFullStateName($person_office_result['state']));
...
$payment_result =  mysql_fetch_array(mysql_query("SELECT total_payable FROM challan_master where registration_id='$registration_id' and challan_financial_year='2015'"));
