...
//$company_name = strtoupper(filter($default['company_name']));
...
$information_region = $conn ->query("SELECT `region_full_name` FROM `region_master` WHERE `region_name` = '$region_id'");
$information_region_array =  $information_region->fetch_assoc();
...
$com = $conn ->query("SELECT * FROM communication_details_master WHERE registration_id = '$registration_id'");
$com_default = $com->fetch_assoc();
...
$registration_required1=$export_product_name;
	...
	if(preg_match('/Lab Grown Diamond/',$registration_required1))
    {
	   $lgd = 'Yes';
    }  else  {
       $lgd = '-';
...
$person_count = 0;
$person_office_print = array();

$person_office = $conn ->query("SELECT * FROM communication_address_master WHERE registration_id = '$registration_id' and type_of_address='$details_person_office'");
...
$approv = $conn ->query("SELECT * FROM approval_master WHERE registration_id='$registration_id'");
$certificate_renewal_data = $approv->fetch_assoc();
$applied_for_rcmc_certificate =	strtoupper($certificate_renewal_data['rcmc_certificate_apply']);
$membership_category =	strtoupper(getMemberType($certificate_renewal_data['registration_id'],$conn));
...
$invoice_date=date('d/m/Y',strtotime($certificate_renewal_data['invoice_date']));
...
if($rcmc_amendment_modification_reason!=""){
	$amendment_msg="<b>Amended RCMC : </b>$rcmc_amendment_modification_reason";
} else
	$amendment_msg="";
...
$signing_authority=$certificate_renewal_data['signing_authority'];
...
//Head Office
$head_office_print = "";
$head_office_result = $conn ->query("SELECT name, father_name, address1, address2, address3, country, state, city, pincode, landline_no1, mobile_no, fax_no1, fax_no2, email_id FROM communication_address_master  WHERE registration_id = '$registration_id'  and type_of_address='2' limit 0,2");
$person_count = 0;
while($head_office_result1 = $head_office_result->fetch_assoc()) {
...
$hcountry = strtoupper(getFullCountryeName($head_office_result1['country'],$conn));
$hstate = strtoupper(getFullStateName($head_office_result1['state'],$conn));
...
$ext_head_office_result = $conn ->query("SELECT name, father_name, address1, address2, address3, country, state, city, pincode, landline_no1, mobile_no, fax_no1, fax_no2, email_id FROM communication_address_master  WHERE registration_id = '$registration_id'  and type_of_address='2' limit 2,20");
$ext_head_office_count = 0;
while($ext_head_office_result1 = $ext_head_office_result->fetch_assoc()) {
...
$ext_hcountry = strtoupper(getFullCountryeName($ext_head_office_result1['country'],$conn));
$ext_hstate = strtoupper(getFullStateName($ext_head_office_result1['state'],$conn));
...
$registered_office_result = $conn ->query("SELECT * FROM communication_address_master WHERE registration_id = '$registration_id'  and type_of_address='6' limit 0,2");
$regis_count = 0;
while($registered_office_result1 =  $registered_office_result->fetch_assoc()) {
...
$rstate = strtoupper(getState($registered_office_result1['state'],$conn));
...
$registered_office_print .=  $raddress1.' '.$raddress2.' '. $raddress3.' '.$rcity.' '.$rstate.' '.$rcountry.' '.$rpincode.'<br/>' ;
}
...
$ext_registered_office_result = $conn ->query("SELECT * FROM communication_address_master WHERE registration_id = '$registration_id'  and type_of_address='6' limit 2,20");
$ext_registered_office_count = 0;
while($ext_registered_office_result1 =  $ext_registered_office_result->fetch_assoc()) {
...
$ext_rstate = strtoupper(getState($ext_registered_office_result1['state'],$conn));
...
$branch_office_print = "";
$branch_office_result = $conn ->query("SELECT * FROM  communication_address_master WHERE registration_id = '$registration_id'  and type_of_address='3' limit 0,2");
$brach_count = 0;
while($branch_office_result1 = $branch_office_result->fetch_assoc()) {
...
$bstate = strtoupper(getState($branch_office_result1['state'],$conn));
...
$branch_office_print .=  $baddress1.' '.$baddress2.' '. $baddress3.' '.$bcity.' '.$bstate.' '.$bcountry.' '.$bpincode.'<br/>' ;
}
...
$ext_branch_office_result = $conn ->query("SELECT * FROM  communication_address_master WHERE registration_id = '$registration_id'  and type_of_address='3' limit 2,60");
$ext_branch_office_count = 0;
while($ext_branch_office_result1 = $ext_branch_office_result->fetch_assoc()) {
...
$ext_bstate = strtoupper(getState($ext_branch_office_result1['state'],$conn));
...
$factor_office_print = "";
//$factor_office_result = $conn ->query("SELECT * FROM  communication_address_master WHERE registration_id = '$registration_id'  and type_of_address='4' limit 0,3"); // 12Jul18
$factor_office_result = $conn ->query("SELECT * FROM  communication_address_master WHERE registration_id = '$registration_id'  and type_of_address='4' limit 0,5");
$factory_count = 0;
while($factor_office_result1 = $factor_office_result->fetch_assoc()) {
...
$fstate = strtoupper(getState($factor_office_result1['state'],$conn));
$fcity = strtoupper($factor_office_result1['city']);
$fpincode = $factor_office_result1['pincode'];
$flandline_no1 = $factor_office_result1['landline_no1'];
$fmobile_no = $factor_office_result1['mobile_no'];
...
$ext_factor_office = $conn ->query("SELECT * FROM  communication_address_master WHERE registration_id = '$registration_id'  and type_of_address='4' limit 5,20");/* 01June 2018*/
/*$ext_factor_office = $conn ->query("SELECT * FROM  communication_address_master WHERE registration_id = '$registration_id'  and type_of_address='4' limit 2,20"); */
$ext_factor_office_count = 0;
while($ext_factor_office_result1 = $ext_factor_office->fetch_assoc()) {
...
$ext_fstate = strtoupper($ext_factor_office_result1['state'],$conn);
$ext_fcity = strtoupper($ext_factor_office_result1['city']);
$ext_fpincode = $ext_factor_office_result1['pincode'];
$ext_flandline_no1 = $ext_factor_office_result1['landline_no1'];
$ext_fmobile_no = $ext_factor_office_result1['mobile_no'];
$ext_ffax_no1 = $ext_factor_office_result1['fax_no1'];
$ext_ffax_no2 = $ext_factor_office_result1['fax_no2'];
      ...
      <tr>
        ...
        <td valign="top"><table width="620" border="0" cellspacing="0" cellpadding="0">
		  ...
		  <tr>
            ...
            <td align="center" valign="top">$lgd</td>
          </tr>
...
<table width="580" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
        <td colspan="2"><b>Note :</b> This import certificate is not a substitute for import License in respect of the items mentioned restricted under ITC(HS) and an import license, in addition to this Certificate, will have to obtained wherever required for such items.</td>
    </tr>
	<tr>
	 <td>&nbsp;&nbsp;</td>
	</tr>
	<tr><td style="background-color:#ccc;" valign="middle" align="center" height="20px">$amendment_msg</td></tr>
<tr>
...
{	
	...
	if($invoice_date<="30/11/2021")
		$signature = 'images/digital-sign-1.jpg';
	else
		$signature = 'images/digital-sign-11.jpg';
	//$signature = 'images/ms.jpg';
}
elseif($region_id=="RO-DEL")
{
	//if($signing_authority=="PS"){
		$certificate = 'file://'.realpath('suruchi_delhi.cer');
		$info = array(
			'Name' => 'PREETI SHARMA',
			'Location' => 'New Delhi',
			'Reason' => 'digital signature for rcmc',
			'ContactInfo' => 'preeti.sharma@gjepcindia.com',
			);
		$signature = 'images/preeti.jpg';
	/*} else {
...
elseif($region_id=="RO-SRT")
{
	...
	$info = array(
		...
		'Location' => 'Surat',
		'Reason' => 'digital signature for rcmc',
		'ContactInfo' => 'rajat.wani@gjepcindia.com',
		);
	$signature = 'images/rajat.jpg';
}
elseif($region_id=="RO-JAI")
{
	...
	$info = array(
		...
		'Location' => 'Jaipur',
		'Reason' => 'digital signature for rcmc',
		'ContactInfo' => 'nitin.khandelwal@gjepcindia.com',
		);
	$signature = 'images/nitin.jpg';
}
...
{		
	...
	$info = array(
		...
		'Location' => 'Kolkata',
		'Reason' => 'digital signature for rcmc',
		'ContactInfo' => 'kaushik@gjepcindia.com',
		);
	$signature = 'images/kaushik.jpg';
}
...
$pdf->Image($signature,120, '', 65, 33, '', '', '', false, 300, '', false, false, 0, false, false, false);
$pdf->writeHTML($txt3, true, false, true, false, '');
//$pdf->Image($signature, 120,60, 65, 25, 'JPG');
