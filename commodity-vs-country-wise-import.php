<?php 
$pageTitle = "Gems And Jewellery Industry In India | Commodity x Country-wise - GJEPC India";
$pageDescription  = "Gem Jewellery Export Promotion Council (GJEPC) is India's Apex body supported by the ministry of commerce and industry.";
?>
<?php 
...
if($_REQUEST['Reset']=="Reset")
{
		  ...
		  $_SESSION['country']         =""; 		  	  
		  $_SESSION['allcountry']      =""; 		  	  
		  $_SESSION['listscountry']    =""; 		  	  
		  $_SESSION['currency']        ="";
		  $_SESSION['currency_inr']    =""; 		  	  
		  $_SESSION['currency_usd']    =""; 
		  $_SESSION['display_records'] =""; 		  
		  
		  header("Location: commodity-vs-country-wise-import.php");
} else {
		...
		$listscountry	 = $_POST['listscountry'];
		$trade_type 	 = "Import";
		...
		$_SESSION['listscountry'] 	= $listscountry;
		...
		{  $signup_error = "Please Select Category."; $flag=0; }
		...
		{  $signup_error = "Please Select Display Records."; $flag=0; }
		else {
		$flag=1;
		} 
	}
...
<div class="container inner_container">
    ...
    <div class="row justify-content-center">
            ...
            <div class="row">
                ...
                <div class="form-group col-12 mb-4">
                    <p class="blue text-center">IMPORTS - COMMODITY X COUNTRY-WISE</p>
                </div>
                ...
                <div class="col-md-11">
                    <div class="row">
                        ...
                        <div class="col-12 mb-2" id="specific-commodity">
                        <div class="row">
                        <div class="col-3  d-flex align-items-center">
                        ...
                        <div class="col-8">
                        <select name="commodity" id="commodity" class="form-control" placeholder="Search Specific Commodity">
						<option value="">Select Category</option>
                        <?php 			
						$region_query = "select commodity_category from statistics_integration_hscode_master where status ='1' AND commodity_category!='' AND level='8' group by commodity_category order by commodity_category";
                        ...
                        <div class="col-8">                         
                          <select name="display_records" class="form-control">
                            ...
                            <option value="9" <?php if($_SESSION['display_records']=="9"){ echo "selected='selected'";}?>>Top 10</option>
                            <option value="19" <?php if($_SESSION['display_records']=="19"){ echo "selected='selected'";}?>>Top 20</option>
                            <option value="49" <?php if($_SESSION['display_records']=="49"){ echo "selected='selected'";}?>>Top 50</option>
                            </select>
                        </div>
                        </div>                       
						</div>
	...
	/*	if($cur_month > 4) { */ 
		...
		{
			if($_POST['commodity']!=''){
				$sqln = "select * from statistics_integration_hscode_master where status ='1' AND level='8' AND commodity_category='".$_POST['commodity']."'";
				$resultn = $conn ->query($sqln);
				$nums = $resultn->num_rows;
				if($nums>0){
					$sql.=" AND (";
					$myArray=array();
					$i=1;
					...
					$result_string = "'" . str_replace(",", "%' OR hs_code_id LIKE '", $sqln) . "%'";
					
					$hscodes = "(hs_code_id LIKE $result_string) "; 
					//echo '<br/>';
				}
			}
		}
		...
		*/
		/* for FY */ $replacedQuery2 = str_replace($financial_year, $last_finyr, $replacedQuery); 
		}
		...
		?>
            ...
            <div class="col-12">
                        <div class="row py-3"  style="border-top: 1px solid #ccc">
                            <div class="col-12"><h2 class="title text-center mb-1"><span class="d-table mx-auto mb-3" style="width:35px;"><img src="assets/images/black_star.png" class="img-fluid d-block"></span>Imports - Commodity x Country-wise</h2></div>
                            ...
                            <div class="col-12">
								...
								<div class="row justify-content-center mt-4">
                                   		<div class="col-md-9 searchResultwrap">
                                        	<ul>
                                                ...
                                                <li>
                                                	<div class="row">
                                                    	<div class="col-md-4 sCategory">Commodity Category</div>
                                ...
                                <div class="d-flex justify-content-start"> 
								   <button onclick="exportToExcel('tblexportData', 'commodity-vs-country-wise-import')" class="cta" title="Export To Excel"><i class="fa fa-file-excel-o" style="font-size: 40px;"></i></button>
                                </div>
								
								<div class="col-md-6 d-flex justify"> </div>
                            </div>
					...
					/* Same */		if(getFinancialYear($conn)==$financial_year){ $sameYear = "YES"; } else { $sameYear = "NO"; }
									...
									{										
										if($currency!="")
										{
											...
											$getPreviousFinancialYearData = getPreviousFinancialYearCountryINR_i($last_finyr,$row['country_id'],$hscodes,$sameYear,$currentMonth,$conn);
											$getPreviousData	= round($getPreviousFinancialYearData/$_POST['currency_inr'],2);
											$sub_lastYear_Paisa = $sub_lastYear_Paisa + $getPreviousFinancialYearData/$_POST['currency_inr'];
										...
										//	$newTotal_Paisa  = round($row['total_usd']/$_POST['currency_usd'],2);
											...
											$getPreviousFinancialYearData =  getPreviousFinancialYearCountryUSD_i($last_finyr,$row['country_id'],$hscodes,$sameYear,$currentMonth,$conn);
											$sub_lastYear_Paisa =  $sub_lastYear_Paisa + $getPreviousFinancialYearData/$_POST['currency_usd'];
										//	$getPreviousData	=  round($getPreviousFinancialYearData/$_POST['currency_usd'],2);
											$number  = $getPreviousFinancialYearData/$_POST['currency_usd'];
						...
						<td class="text-center"><?php if($currency=="INR"){ echo $sub_lastYear_Paisa=round($grandTotalLastINR,2); } else { echo $sub_lastYear_Paisa=round($grandTotalLastUSD,2); }?></td>
						<td class="text-center"><?php if($currency=="USD"){ echo $sub_total_Paisa=round($grandTotalUSD,2); } else { echo $sub_total_Paisa=round($grandTotalINR,2); }?></td>
