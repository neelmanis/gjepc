    ...
    public function preSend()
    {
        ...
        if (ini_get('mail.add_x_header') == 1
            ...
            and ((version_compare(PHP_VERSION, '7.0.0', '>=')
                    and version_compare(PHP_VERSION, '7.4.30', '<'))
                or (version_compare(PHP_VERSION, '7.4.30', '>=')
                    and version_compare(PHP_VERSION, '7.4.30', '<')))
        ) {
            trigger_error(
                'Your version of PHP is affected by a bug that may result in corrupted messages.' .
                ' To fix it, switch to sending using SMTP, disable the mail.add_x_header option in' .
                ' your php.ini, switch to MacOS or Linux, or upgrade your PHP to version 7.4.30+ or 7.4.30+.',
                E_USER_WARNING
            );
        }
        try {
