<?php 
$pageTitle = "Gems And Jewellery Industry In India | Commodity Wise Export - GJEPC India";
...
include 'functions.php';
	...
	if($_REQUEST['Reset']=="Reset")
	{
		  ...
		  header("Location: commodity-wise-export.php");
	} else {
		...
		if(isset($search_type)=="SEARCH")
		{
			...
			/*if($flag==1) { }
			$getValue="";
			if($rCount>0)  { $getValue=1; }
			if($rCount==0) { $getValue='NO'; } */
			}
		}
	}
?>
...
<div class="container inner_container">
    ...
    <div class="row justify-content-center">	
		...
		<input type="hidden" name="search_type" id="search_type" value="SEARCH"/>     
            ...
            <div class="form-group col-12 mb-4">
                <p class="blue text-center">EXPORTS - COMMODITY-WISE</p>
            </div> 
	...
	/*	if($cur_month > 4) { */
		...
		if(isset($commoditySel) && $commoditySel=="allCommodity")
		{
			if($commodity_level!='')
			{	
				$sql="SELECT `financial_year`, `country_id`, `hs_code_id`,`inr_value`, SUM(REPLACE(inr_value, ',', '')) as total_inr, `dollar_value`, SUM(REPLACE(dollar_value, ',', '')) as total_usd FROM `statistics_integration_data` WHERE 1 AND financial_year='$financial_year' AND trade_type='$trade_type'";
		...
		<div class="col-12">
                <div class="row py-3" style="border-top: 1px solid #ccc">
                            <div class="col-12"><h2 class="title text-center mb-1"><span class="d-table mx-auto mb-3" style="width:35px;"><img src="assets/images/black_star.png" class="img-fluid d-block"></span>EXPORTS - COMMODITY-WISE</h2></div>
                            ...
                            <div class="col-12">
								...
								<div class="row justify-content-center mt-4">
                                        	...
                                        	<ul>
                                            	<li>
                                                	<div class="row">
														...
														echo " APR-".getCurrentMonthName($conn); } ?>
														</div>
                                                    </div>
                                                </li>
												<?php
								   ...
								   <button onclick="exportToExcel('tblexportData', 'commodity-wise-export')" class="cta gold_btn" title="Export To Excel"><i class="fa fa-file-excel-o" style="font-size: 40px;"></i></button>
                                   </div>
                            </div>
						<div class="col-12">
					...
					/* Same */		if(getFinancialYear($conn)==$financial_year){ $sameYear = "YES";	} else { $sameYear = "NO"; }
									...
									{	//print_r($row);								
										if($currency!="")
										{
											...
											$getCurrentYearData  = getCurrentcommoditywiseINR($_REQUEST['financial_year'],$row['hs_code'],$conn);
											...
											$getPreviousYearData = getPreviouscommoditywiseINR($last_finyr,$row['hs_code'],$sameYear,$currentMonth,$conn);
											...
											$getCurrentYearData = getCurrentcommoditywiseUSD($_REQUEST['financial_year'],$row['hs_code'],$conn);
								...
								/* Same */	$getPreviousYearData = getPreviouscommoditywiseUSD($last_finyr,$row['hs_code'],$sameYear,$currentMonth,$conn);
					...
					/* Same */		if(getFinancialYear($conn)==$financial_year){ $sameYear = "YES";	} else { $sameYear = "NO"; }
									while($row = $resultQ->fetch_assoc())
									{
										...
										//	$getCurrentYearData  = getCurrentcommoditywiseINR($_REQUEST['financial_year'],$row['hs_code'],$conn);																															
							/* Same */	$getPreviousYearData = getPreviousSpecificcommoditywiseINR($last_finyr,$row['commodity_category'],$sameYear,$currentMonth,$conn);
											$getPreviousData	 = round($getPreviousYearData/$_POST['currency_inr'],2);
											$sub_lastYear_Paisa  = round($sub_lastYear_Paisa + $getPreviousYearData/$_POST['currency_inr'],2);											
										...
										//	$newTotal_Paisa  = round($row['total_usd']/$_POST['currency_usd'],2);
											...
											$getPreviousYearData = getPreviousSpecificcommoditywiseUSD($last_finyr,$row['commodity_category'],$sameYear,$currentMonth,$conn);
											$sub_lastYear_Paisa  = round($sub_lastYear_Paisa + $getPreviousYearData/$_POST['currency_usd'],2);
											//$getPreviousData	 = round($getPreviousYearData/$_POST['currency_usd'],2);
