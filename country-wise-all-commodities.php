...
include 'functions.php';
		...
		if($_REQUEST['Reset']=="Reset")
		{
		  ...
		  header("Location: country-wise-all-commodities.php");
		} else {
    ...
    <div class="row justify-content-center" >
			...
			<input type="hidden" name="search_type" id="search_type" value="SEARCH"/>
                ...
                <div class="form-group col-12 mb-4">
                    <p class="blue text-center">EXPORTS - COUNTRY-WISE ALL COMMODITIES</p>
                </div>
		...
		?>
            <div class="col-12">
                        <div class="row py-3"  style="border-top: 1px solid #ccc">
                            <div class="col-12"><h2 class="title text-center mb-1"><span class="d-table mx-auto mb-3" style="width:35px;"><img src="assets/images/black_star.png" class="img-fluid d-block"></span>Exports - Country-wise all commodities</h2></div>
                            ...
                            <div class="d-flex justify-content-start">
                               <button onclick="exportToExcel('tblexportData', 'country-wise-all-commodities')" class="cta gold_btn" title="Export To Excel"><i class="fa fa-file-excel-o" style="font-size: 40px;"></i></button>
                            </div> 
                            </div>
				...
				/* Same */			if(getFinancialYear($conn)==$financial_year){ $sameYear = "YES"; } else { $sameYear = "NO"; }													
									...
									{	//echo $grandTotalINR.'--'.$grandTotalUSD;
										if($currency!="")
										{
											...
											$getCurrentYearData  = getCurrentcountrywiseAllINR($_REQUEST['financial_year'],$_REQUEST['country'],$row['hs_code'],$conn);
											...
											$getPreviousYearData = getPreviousCountrywiseINR($last_finyr,$_REQUEST['country'],$row['hs_code'],$sameYear,$currentMonth,$conn);
											...
											$getCurrentYearData  = getCurrentcountrywiseAllUSD($_REQUEST['financial_year'],$_REQUEST['country'],$row['hs_code'],$conn);
										//	$newTotal_Paisa  = round($getCurrentYearData/$_POST['currency_usd'],2);
											...
											$getPreviousYearData = getPreviousCountrywiseUSD($last_finyr,$_REQUEST['country'],$row['hs_code'],$sameYear,$currentMonth,$conn);
				...
				/* Same */			if(getFinancialYear($conn)==$financial_year){ $sameYear = "YES"; } else { $sameYear = "NO"; }		
									...
									{	//echo '<pre>'; print_r($row);								
										if($currency!="")
										{
											...
											$getPreviousYearData = getPreviousCountrycommoditywiseINR($last_finyr,$row['commodity_category'],$_POST['country'],$sameYear,$currentMonth,$conn);
											$getPreviousData	 = round($getPreviousYearData/$_POST['currency_inr'],2);
											$sub_lastYear_Paisa  = round($sub_lastYear_Paisa + $getPreviousYearData/$_POST['currency_inr'],2);											
										...
										//	$newTotal_Paisa  = round($row['total_usd']/$_POST['currency_usd'],2);	
											...
											$getPreviousYearData = getPreviousCountrycommoditywiseUSD($last_finyr,$row['commodity_category'],$_POST['country'],$sameYear,$currentMonth,$conn);
											$sub_lastYear_Paisa  = round($sub_lastYear_Paisa + $getPreviousYearData/$_POST['currency_usd'],2);
										//	$getPreviousData	 = round($getPreviousYearData/$_POST['currency_usd'],2);
											$number  = $getPreviousYearData/$_POST['currency_usd'];
											$getPreviousDataNOround  = number_format($number, 12, '.', '');
